...
export async function ensureNode(options = {}) {
  ...
  const destRoot = join(runtimeRoot, slug);
  const marker = process.platform === 'win32'
    ? join(destRoot, 'node.exe')
    : join(destRoot, 'bin', 'node');
 ...
 * that satisfies the minimum can still be invisible to child processes. Then
 * corepack is missing and standalone pnpm falls back to its bundled older Node,
 * which fails the repo engines.node check.
 *
