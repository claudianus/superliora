...
} from './liora-frontier-targets.mjs';
import { evaluateHarnessRadarGate } from './liora-harness-radar.mjs';
import {
...
const DEFAULT_CRITERIA_PATH = `${WORKSPACE_DIR}/bench/sota-criteria.json`;
const DEFAULT_HARNESS_RADAR_PATH = `${WORKSPACE_DIR}/bench/harness-radar.json`;
const DEFAULT_OUTPUT_BASE = `${WORKSPACE_DIR}/evidence/liora-agent-sota-gate`;
const DEFAULT_EVIDENCE_ROOT = `${WORKSPACE_DIR}/evidence`;
...
const TUI_ULTRAWORK_WORKFLOW_GUIDANCE = 'SUPERLIORA_REAL_WORKFLOW_EVIDENCE';
const TUI_SCREEN_SIGNAL_PATTERNS = Object.freeze([
  { name: 'legacy-kimi-branding', pattern: /\b(?:super[- ]kimi|kimi code)\b/i },
...
const REQUIRED_FREE_WEB_RESEARCH_CONTRACTS = Object.freeze([
  ...
  { name: 'scrapling-compatible-internal-backend', pattern: /(?=[\s\S]*\bScrapling-class\b)(?=[\s\S]*\bbuilt-in WebSearch\b)(?=[\s\S]*\bFetchURL\b)(?=[\s\S]*\binternal providers\b)(?=[\s\S]*\bCSS selector\b)(?=[\s\S]*\badaptive element relocation\b)/i },
...
const REQUIRED_INTERNAL_WEB_RESEARCH_ARTIFACTS = Object.freeze([
  {
    name: 'local-fetch-url-selector-extraction',
    file: 'packages/agent-core/src/tools/providers/local-fetch-url.ts',
    pattern: /\bLOCAL_WEB_RESEARCH_CONTENT_SELECTORS\b[\s\S]*\bextractLocalMainContent\b[\s\S]*\bReadability\b/i,
  },
  {
    name: 'status-research-gate',
    file: 'apps/liora/src/tui/components/messages/status-panel.ts',
    pattern: /\bRESEARCH_GATE\b[\s\S]*\bWebSearch\b[\s\S]*\bFetchURL\b[\s\S]*\bMCP optional\b/i,
  },
]);
...
function usage() {
  return `Usage: node scripts/liora-agent-sota-gate.mjs [options]

Local SuperLiora SOTA evidence gate.
...
async function buildReport(options, outputDir, runId) {
  ...
  return {
    schemaVersion: 1,
    gate: 'superliora-agent-sota-gate',
    runId,
    status,
    reason:
      status === 'PASS'
...
function evaluateLoopScoreGate(scorecard) {
  return {
    name: 'superliora-loop-score',
    status: scorecard.status,
    required: true,
    reason:
      scorecard.status === 'PASS'
        ? `SuperLiora loop score ${scorecard.score}/${scorecard.maxScore} meets threshold ${scorecard.threshold}.`
        : `SuperLiora loop score ${scorecard.score}/${scorecard.maxScore} is below threshold ${scorecard.threshold}.`,
    observed: {
...
async function evaluateFreeWebResearchHarnessGate(criteria) {
  ...
  for (const surface of REQUIRED_INTERNAL_WEB_RESEARCH_ARTIFACTS) {
    ...
    if (!surfaceArtifact.exists || surfaceArtifact.bytes === 0) {
      missingInternalArtifacts.push(surface.name);
      continue;
    }
  ...
  if (missingInternalArtifacts.length > 0) {
    failures.push(`Internal web research artifacts are missing: ${missingInternalArtifacts.join(', ')}`);
  }
  return {
...
function isUsableSotaBaseline(summary) {
  const friction = extractTuiUxFriction(summary);
  return (
    summary?.gate === 'superliora-agent-sota-gate' &&
    summary.status === 'PASS' &&
    friction?.status === 'PASS' &&
...
async function evaluateWorkflowGate(summary) {
  ...
  if (summary.lioraHomeMode !== 'real-user-opt-in') {
    failures.push(`SUPERLIORA_HOME mode is ${String(summary.lioraHomeMode)}`);
  }
  ...
  return {
    ...
    observed: {
      ...
      lioraHomeMode: summary.lioraHomeMode,
      validationStatuses,
...
function recommendTuiNextActions(tuiGate, tuiUxDeltaGate, workflowGate, ultraworkGate) {
  ...
  if (workflowGate.status !== 'PASS') {
    addAction(
      ...
      'node scripts/qa-superliora-autonomous.mjs --phase tui-real-workflow --use-real-liora-home --evidence-root .superliora/evidence/<real-workflow-repair>',
    );
  }
    ...
    switch (penalty.name) {
      case 'failed-scenarios':
        addAction(
          ...
          'node scripts/qa-superliora-autonomous.mjs --phase tui-launch --evidence-root .superliora/evidence/<live-tui-after-fix>',
        );
        break;
      case 'screen-observation':
        addAction(
          ...
          { scenarios: scenarioNames(penalty.detail) },
          'node scripts/qa-superliora-autonomous.mjs --phase tui-launch --evidence-root .superliora/evidence/<screen-observation-after-fix>',
        );
        break;
      case 'input-trace':
        addAction(
          ...
          { scenarios: scenarioNames(penalty.detail) },
          'node scripts/qa-superliora-autonomous.mjs --phase tui-launch --evidence-root .superliora/evidence/<input-trace-after-fix>',
        );
        break;
      case 'exit-cleanup':
        addAction(
          ...
          penalty.detail,
          'node scripts/qa-superliora-autonomous.mjs --phase tui-launch --evidence-root .superliora/evidence/<exit-cleanup-after-fix>',
        );
        break;
      case 'duration':
        addAction(
  ...
  if (ultraworkGate === undefined) {
    addAction(
      ...
      'node scripts/qa-superliora-autonomous.mjs --phase tui-ultrawork-workflow --use-real-liora-home --evidence-root .superliora/evidence/<ultrawork-workflow-after>',
    );
    ...
    addAction(
      ...
      'node scripts/qa-superliora-autonomous.mjs --phase tui-ultrawork-workflow --use-real-liora-home --evidence-root .superliora/evidence/<ultrawork-workflow-repair>',
    );
  }
  ...
  if (actions.length === 0) {
    addAction(
      ...
      'node scripts/qa-superliora-autonomous.mjs --phase tui-ultrawork-workflow --use-real-liora-home --evidence-root .superliora/evidence/<ultrawork-full-cycle-after>',
    );
  }
...
function inspectTuiScreenText(scenario, output) {
  ...
  if (!hasLioraTuiChrome(normalized)) {
  ...
  switch (scenario) {
    case 'startup':
      if (!matchesAny(normalized, [/superliora|liora/i, /ask/i, /message/i, /editor/i, /auto/i])) {
        failures.push('startup capture does not show a SuperLiora startup/editor state');
      }
    ...
    case 'status':
      ...
      break;
    case 'clear':
      if (!matchesAny(normalized, [/superliora|liora/i, /message/i, /editor/i, /prompt/i])) {
        failures.push('clear capture does not show a returned SuperLiora prompt/editor state');
      }
      break;
    case 'autocomplete':
      if (
...
function hasLioraTuiChrome(output) {
...
async function evaluateNoWebUiSurfaceGate(inputs) {
  ...
  const webUiInputs = inputEntries
    .filter(([, inputPath]) => pathSegments(inputPath).includes('apps') && pathSegments(inputPath).includes('vis'))
    .map(([name, inputPath]) => `${name}=${inputPath}`);
  if (webUiInputs.length > 0) {
    failures.push(`gate inputs point at apps/vis: ${webUiInputs.join(', ')}`);
  }
  ...
  return {
    ...
    reason: failures.length === 0
      ? 'Gate success is based only on bench-system, bounded loop, and live terminal/TUI artifacts; the Ultrawork contract explicitly excludes apps/vis/browser UI success.'
      : failures.join('; '),
    observed: {
      ...
      contractBytes: contractArtifact.bytes,
      contractHasBoundary,
      excludedSuccessSurfaces: ['apps/vis', 'browser UI'],
    },
  };
}
...
function renderMarkdown(report) {
  ...
  if (report.tuiWorkflowProof !== undefined) {
    lines.push(
      ...
      `- mode: ${report.tuiWorkflowProof.lioraHomeMode}`,
  ...
  if (report.tuiUltraworkProof !== undefined) {
    lines.push(
      ...
      `- mode: ${report.tuiUltraworkProof.lioraHomeMode}`,
  ...
  if (report.freeWebResearchHarness !== undefined) {
    lines.push(
      ...
      '- advanced backend: Scrapling-compatible capabilities behind built-in providers for authorized public-page extraction',
      '- access boundary: public pages, user-provided authenticated sessions, and explicitly authorized test targets',
      `- internal artifacts: ${String(report.freeWebResearchHarness.internalArtifacts?.length ?? 0)}`,
