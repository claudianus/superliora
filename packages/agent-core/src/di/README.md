# `@superliora/agent-core` Dependency Injection Container
...
- **Testing** — `TestInstantiationService` (subpath
  `@superliora/agent-core/di/test`) exposes direct `.get` / `.stub` so
...
Test files import from the subpath `@superliora/agent-core/di/test`
(NOT the main package entry — keeps production bundles clean):

```ts
import { TestInstantiationService } from '@superliora/agent-core/di/test';
...
├── test.ts                      ← subpath barrel (`@superliora/agent-core/di/test`)
