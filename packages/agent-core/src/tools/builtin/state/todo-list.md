...
**Input (strict):** `todos` is `{ "title", "status": pending|in_progress|done }[]` only. Other fields/statuses rejected.
...
**Granularity:** verb + target (+ path). Split vague cards. Keep exactly one `in_progress` unless real parallel tracks exist.
