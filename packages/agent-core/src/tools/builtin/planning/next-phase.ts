...
You can only advance forward, never backward.`;
  ...
  private phaseInstructions(phase: string): string {
    const instructions: Record<string, string> = {
      interview: "Interview Phase: Act as an expert leader — teach brief insights, surface unknown-unknowns, and present Baseline + Upgrade choices backed by read-only research when needed. Elevate the UltraGoal; do not merely clarify gaps. When ambiguity <= 0.2, clarity floors pass, the UltraGoal is true/false verifiable, and required Seed gaps are closed, call NextPhase({ phase: 'design' }).",
      ...
      review: "Review Phase: Use read-only tools (Read, ReadMediaFile, Grep, Glob, LioraContext, WebSearch, FetchURL, SearchSkill, Skill, SearchExpert, TaskList, TaskOutput, Bash read-only inspection) plus TodoList progress tracking to re-read key files, re-check current external claims, and verify expert coverage before writing the plan. When verification is complete, call NextPhase({ phase: 'write' }).",
