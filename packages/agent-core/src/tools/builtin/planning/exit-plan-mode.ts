...
export class ExitPlanModeTool implements BuiltinTool<ExitPlanModeInput> {
  ...
  private async validateUltraPlanDrift(plan: string): Promise<UltraPlanDriftResult> {
    ...
    if (autoGenerated) {
      this.agent.telemetry.track('ultra_plan_drift_soft_gate', { combined: combinedDrift(metrics) });
      return {
        ok: true,
        metrics,
        warning:
          'The auto-generated Seed Spec has drift above the normal threshold. Exiting with a warning so the user can review the plan. If the plan is wrong, cancel it and re-enter UltraPlan to refine the Seed.',
      };
    }
