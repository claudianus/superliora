...
Ripgrep syntax (not POSIX) — escape `\{` for literal braces. Hidden/dotfiles are searched by default. `include_ignored=true` searches `.gitignore` paths (e.g. `node_modules`); sensitive files (such as `.env`) are always skipped.
