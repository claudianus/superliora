...
- Do not create unsolicited documentation files (`*.md`, `README`s, summaries) unless the user asks or a task/project instruction requires it (e.g. plan file, changeset).
- Read before overwriting. Ignore the Read/Edit line-number view — NEVER include line prefixes. Content is written literally, including line endings (`\n` stays LF, `\r\n` stays CRLF).
