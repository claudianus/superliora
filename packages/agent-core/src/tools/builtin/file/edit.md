...
- `old_string` must be unique unless `replace_all` is set. If ambiguous, add surrounding context. Use `replace_all` only when every occurrence should change (e.g. rename a symbol throughout the file).
