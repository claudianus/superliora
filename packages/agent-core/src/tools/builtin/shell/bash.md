...
- `jq`/`yq`/`python -m json.tool`/`ConvertTo-Json`/`ConvertFrom-Json`/`Import-Csv`/`ConvertFrom-Csv`/`Select-Object` (path dumps) whole-file dumps → `Read`; `Export-Csv`/`ConvertTo-Csv` path dumps → `Write`
