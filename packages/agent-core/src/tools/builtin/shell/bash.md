...
- reads: `cat`/`gcat`/`head`/`ghead`/`tail`/`bat`/`batcat`/`type`/`Get-Content`/`Get-Item` (file path dumps)/`Get-Content path | Format-List|Format-Table|Out-String|Format-Hex|Out-GridView|Select-Object|Select-Xml|ConvertTo-Json|ConvertFrom-Json|ConvertTo-Csv|ConvertTo-Html|ConvertTo-Xml|Import-Csv|Import-Clixml`/`cat path | pbcopy`/`Get-Content path | Set-Clipboard`/`type|cat path | clip`/`cat|type|Get-Content path | less|more|head|tail|nl`/`ConvertTo-Json`/`ConvertFrom-Json`/`Import-Csv`/`ConvertFrom-Csv`/`Import-Clixml` (path dumps)/`Select-Object` (path dumps)/`Format-List`/`Format-Table`/`Out-String`/`Format-Hex`/`Get-FileHash`/`Select-Xml`/`Out-GridView` (path dumps)/`glow`/`mdcat`/`rich`/`python -m rich.syntax`/`less`/`more`/`most`/`nl`/`w3m`/`lynx`/`elinks` (local path)/`zcat`/`gzcat`/`bzcat`/`xzcat`/`zstdcat`/`rev`/`paste` (single file)/`sort`/`uniq`/`shuf` (single file)/`look word file`/`iconv … file`/`sed -n`/`awk`/`base64`/`hexdump`/`fmt`/`pr`/`fold`/`jq`/`yq`/`python -m json.tool`/`git show <rev>:<path>`/`svn cat`/`hg cat`/`pbcopy < path`/`Set-Clipboard -Path`/`xclip path`/`xsel path`
