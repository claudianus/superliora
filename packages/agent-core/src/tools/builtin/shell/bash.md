...
- pattern find / `fd`/`fdfind` / `rg --files` / `Get-ChildItem -Recurse -Filter` / `dir /s` / `where /r` → `Glob` (plain `ls`/`dir`/`gci` of a directory OK)
...
- search/list: `grep`/`rg`/`rg --files`/`ag`/`ack`/`ugrep`/`git grep`/`git ls-files`/`find`/`fd`/`fdfind`/`Select-String`/`findstr`/`Get-ChildItem -Recurse -Filter`/`dir /s`/`where /r`
