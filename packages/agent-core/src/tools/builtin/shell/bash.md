...
**Background:** `run_in_background=true` returns a task ID (requires `description`). You are notified when the task completes. Default timeout {{ DEFAULT_BACKGROUND_TIMEOUT_S }}s, max {{ MAX_BACKGROUND_TIMEOUT_S }}s; `disable_timeout=true` only when needed. After starting, return control to the user; use `TaskOutput` (`block=true` to wait). `TaskStop` to cancel. Users inspect tasks via `/tasks`.
