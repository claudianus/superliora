...
- `jq`/`yq`/`python -m json.tool`/`ConvertTo-Json`/`ConvertFrom-Json`/`Import-Csv`/`ConvertFrom-Csv`/`Import-Clixml`/`Select-Object`/`Format-Hex`/`Get-FileHash`/`Select-Xml` (path dumps) whole-file dumps → `Read`; `Export-Csv`/`ConvertTo-Csv`/`Export-Clixml` path dumps → `Write`
