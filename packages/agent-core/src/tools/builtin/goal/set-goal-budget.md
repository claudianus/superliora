...
Compound times convert to one unit first ("2 hours and 3 minutes" → `value: 123, unit: "minutes"`). Time budgets: 1 second–24 hours (outside rejected). Turn and token budgets need only be positive and are rounded to the nearest whole number (minimum 1).
