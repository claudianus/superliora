...
const PROJECT_BRAND_DIRS = ['.superliora/skills'] as const;
...
export interface SkillPathContext {
  ...
  /**
   * Brand data dir — `SUPERLIORA_HOME`, or `<userHomeDir>/.superliora` by default.
   * User brand skills live directly under here as `skills/`, so this path
   * carries no `.superliora` segment of its own (that would double the prefix).
   */
...
export async function resolveSkillRoots(
  ...
  const brandHomeDir = options.paths.brandHomeDir ?? path.join(userHomeDir, '.superliora');
