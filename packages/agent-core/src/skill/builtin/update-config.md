...
kimi-code has two TOML config files, both under `<SUPERLIORA_HOME>/`, both snake_case, but with different ownership — decide which one the user means before doing anything.

The runtime resolves the data directory as `SUPERLIORA_HOME` first, falling back to `~/.superliora`. Before doing anything, resolve the actual directory with Bash so you don't write to the wrong place. Check whether `SUPERLIORA_HOME` is set and fall back to `~/.superliora` when it is empty:
...
Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<SUPERLIORA_HOME>` means that resolved root — **never assume `~/.superliora`**.
...
- Location: `<SUPERLIORA_HOME>/config.toml` or `<SUPERLIORA_HOME>/tui.toml`. For other scopes/files, defer to the official docs.
...
3. **Copy out a candidate (do not create from scratch)**: use **Bash** to copy the target verbatim — `cp config.toml config-new.toml` (same directory, `-new` suffix; for tui.toml, `cp tui.toml tui-new.toml`). **Leave the original untouched for now.**
   - Only when the target doesn't exist (nothing to copy) should you use **Write** to create a minimal skeleton candidate (e.g. just the comment line `# <SUPERLIORA_HOME>/config.toml`).
...
5. **Validate the candidate** (see Capability 3, via `liora doctor`). **If anything fails, keep Editing the candidate and re-validate, looping until it all passes.**
...
Use **`liora doctor`** to validate the candidate you wrote — it doesn't start the TUI and doesn't modify any file; it runs kimi's own parser + schema (syntax and schema together), so it's the authoritative check. Pick the subcommand by which file you changed, and pass the **candidate** path explicitly:

- changed `config.toml` → `liora doctor config <config-new.toml path>`
- changed `tui.toml` → `liora doctor tui <tui-new.toml path>`
...
Then do two checks `liora doctor` can't:
...
> To also check whether the currently **active** config is OK overall, run `liora doctor` with no path (it checks the default `config.toml` + `tui.toml`, showing a missing one as skipped).
