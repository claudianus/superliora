---
name: custom-theme
description: Create or edit a kimi-code custom color theme — a JSON file under the resolved SUPERLIORA_HOME data directory that recolors the TUI. Use when the user wants their own theme, asks for a specific palette or mood, or wants to tweak an existing custom theme's colors.
---
...
- **Never assume the data directory is `~/.superliora`.** Always resolve `$SUPERLIORA_HOME` first with the Bash command below.
...
The kimi-code runtime resolves the data directory as `SUPERLIORA_HOME` first, falling back to `~/.superliora`. Theme files live inside the `themes/` subdirectory of that data directory.

Before doing anything, resolve the actual data root with Bash so you don't write to the wrong place. Check whether `SUPERLIORA_HOME` is set and fall back to `~/.superliora` when it is empty:
...
Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<SUPERLIORA_HOME>` means that resolved data root — **never assume `~/.superliora`**. Theme files live at `<SUPERLIORA_HOME>/themes/<name>.json`. Create the `themes/` directory if it doesn't exist.

## What a theme is

- A theme lives at `<SUPERLIORA_HOME>/themes/<name>.json`.
...
2. **Resolve the actual theme directory and current theme(s).**
   - Resolve the data root by checking `echo "$SUPERLIORA_HOME"`; if empty, use `echo "$HOME/.superliora"`. Use `<root>/themes` for every subsequent step.
   - If tweaking an existing custom theme, **Read** `<SUPERLIORA_HOME>/themes/<name>.json` first — never overwrite a theme you haven't read.
   - Starting fresh: build a `colors` object from the token table. You can `ls <SUPERLIORA_HOME>/themes/` and Read one of the user's existing themes as a reference for the format.
...
4. **Create a candidate file; never edit the live theme in place.**
   - Use Bash to create a candidate. If the target theme already exists, copy it verbatim: `cp <name>.json <name>.json.new` (inside `<SUPERLIORA_HOME>/themes/`). If it doesn't exist, use **Write** to create a minimal skeleton named `<name>.json.new`.
