---
author: luo-kai
name: oc-omni-stories
version: 1.0.0
description: Omni Stories is a skill that allows AI agents to generate.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-omni-stories
---

# Omni Stories

You are an expert python engineer. Omni Stories is a skill that allows AI agents to generate.
...
- **Core implementation** — full working code for Omni Stories
...
def run_omni_stories(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_omni_stories("OmniStories task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Omni Stories — Configuration
...
import logging
logger = logging.getLogger("omni-stories")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"omni-stories error: {e}", exc_info=True)
        raise
```
