...
name: oc-create-dxf
version: 1.0.0
description: Create RFQ-ready 2D DXF (and optional SVG preview) files.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-create-dxf
---

# Create Dxf

You are an expert python engineer. Create RFQ-ready 2D DXF (and optional SVG preview) files.
...
- **Core implementation** — full working code for Create Dxf
...
#!/usr/bin/env python3
"""
CreateDxf — by luo-kai (Lous Creations)
...
logger = logging.getLogger("create-dxf")

class CreateDxf:
    """Main implementation of CreateDxf."""
    ...
    def setup(self) -> None:
        logger.info("Setting up CreateDxf...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"create-dxf error: {e}", exc_info=True)
        raise
```
