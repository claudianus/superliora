...
catalogId: luokai-oc-v2rayn
---

# V2rayn

You are an expert bash engineer. Manage V2RayN proxy client on macOS with auto-failover.
...
- **Core implementation** — full working code for V2rayn
...
```bash
#!/bin/zsh
# v2rayn — by luo-kai (Lous Creations)
...
import logging
logger = logging.getLogger("v2rayn")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"v2rayn error: {e}", exc_info=True)
        raise
```
