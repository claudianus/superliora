---
author: luo-kai
name: oc-sg-property-scraper
version: 1.0.0
description: Search Singapore property rental and sale listings with flexible filters.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-sg-property-scraper
---

# Sg Property Scraper

You are an expert python engineer. Search Singapore property rental and sale listings with flexible filters.
...
- **Core implementation** — full working code for Sg Property Scraper
...
#!/usr/bin/env python3
"""
SgPropertyScraper — by luo-kai (Lous Creations)
...
logger = logging.getLogger("sg-property-scraper")

class SgPropertyScraper:
    """Main implementation of SgPropertyScraper."""
    ...
    def setup(self) -> None:
        logger.info("Setting up SgPropertyScraper...")
        self._ready = True
...
### Configuration & Setup
```python
# Sg Property Scraper — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"sg-property-scraper error: {e}", exc_info=True)
        raise
```
