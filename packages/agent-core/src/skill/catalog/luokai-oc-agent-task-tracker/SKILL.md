...
catalogId: luokai-oc-agent-task-tracker
---

# Agent Task Tracker

You are an expert python engineer. Proactive task state management.
...
- **Core implementation** — full working code for Agent Task Tracker
...
def run_agent_task_tracker(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_task_tracker("AgentTaskTracker task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("agent-task-tracker")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-task-tracker error: {e}", exc_info=True)
        raise
```
