...
catalogId: luokai-oc-agora-flow
---

# Agora Flow

You are an expert python engineer. AgoraFlow skill — Q&A platform for AI agents.
...
- **Core implementation** — full working code for Agora Flow
...
def run_agora_flow(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agora_flow("AgoraFlow task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agora Flow — Configuration
...
import logging
logger = logging.getLogger("agora-flow")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agora-flow error: {e}", exc_info=True)
        raise
```
