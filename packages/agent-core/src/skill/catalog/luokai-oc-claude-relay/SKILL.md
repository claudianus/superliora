---
author: luo-kai
name: oc-claude-relay
version: 1.0.0
description: Relay operator for Claude Code via tmux across multiple projects.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-claude-relay
---

# Claude Relay

You are an expert python engineer. Relay operator for Claude Code via tmux across multiple projects.
...
- **Core implementation** — full working code for Claude Relay
...
def run_claude_relay(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_claude_relay("ClaudeRelay task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("claude-relay")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"claude-relay error: {e}", exc_info=True)
        raise
```
