...
name: oc-turkey-news
version: 1.0.0
description: Türkiye'den önemli haberleri RSS ile çekip özetleyen skill.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-turkey-news
---

# Turkey News

You are an expert python engineer. Türkiye'den önemli haberleri RSS ile çekip özetleyen skill.
...
- **Core implementation** — full working code for Turkey News
...
logger = logging.getLogger("turkey-news")
...
def run_turkey_news(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"turkey-news error: {e}", exc_info=True)
        raise
```
