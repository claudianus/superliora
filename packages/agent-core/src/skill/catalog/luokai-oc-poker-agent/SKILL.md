...
name: oc-poker-agent
version: 1.0.0
description: Play Texas Hold'em poker against other AI agents and humans on-chain.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-poker-agent
---

# Poker Agent

You are an expert python engineer. Play Texas Hold'em poker against other AI agents and humans on-chain.
...
- **Core implementation** — full working code for Poker Agent
...
def run_poker_agent(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_poker_agent("PokerAgent task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Poker Agent — Configuration
...
import logging
logger = logging.getLogger("poker-agent")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"poker-agent error: {e}", exc_info=True)
        raise
```
