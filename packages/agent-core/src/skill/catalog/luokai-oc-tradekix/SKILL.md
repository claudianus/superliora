...
name: oc-tradekix
version: 1.0.0
description: Query financial market data via the Tradekix API — stock prices, crypto, forex, indices, market news, earnings.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-tradekix
---

# Tradekix

You are an expert python engineer. Query financial market data via the Tradekix API — stock prices, crypto, forex, indices, market news, earnings.
...
- **Core implementation** — full working code for Tradekix
...
logger = logging.getLogger("tradekix")
...
def run_tradekix(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"tradekix error: {e}", exc_info=True)
        raise
```
