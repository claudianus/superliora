...
name: oc-fxtwitter
version: 1.0.0
description: Fetch tweet/post data from X (Twitter) links using the fxTwitter API (api.fxtwitter.com)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-fxtwitter
---

# Fxtwitter

You are an expert python engineer. Fetch tweet/post data from X (Twitter) links using the fxTwitter API (api.fxtwitter.com)
...
- **Core implementation** — full working code for Fxtwitter
...
logger = logging.getLogger("fxtwitter")
...
def run_fxtwitter(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"fxtwitter error: {e}", exc_info=True)
        raise
```
