...
catalogId: luokai-oc-udp-messenger
---
...
- **Core implementation** — full working code for Udp Messenger
...
def run_udp_messenger(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_udp_messenger("UdpMessenger task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Udp Messenger — Configuration
...
import logging
logger = logging.getLogger("udp-messenger")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"udp-messenger error: {e}", exc_info=True)
        raise
```
