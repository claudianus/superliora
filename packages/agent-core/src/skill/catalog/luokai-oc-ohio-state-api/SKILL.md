---
author: luo-kai
name: oc-ohio-state-api
version: 1.0.0
description: Get public data from The Ohio State University Content APIs (content.osu.edu) across campus services (bus.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-ohio-state-api
---

# Ohio State Api

You are an expert python engineer. Get public data from The Ohio State University Content APIs (content.osu.edu) across campus services (bus.
...
- **Core implementation** — full working code for Ohio State Api
...
logger = logging.getLogger("ohio-state-api")
...
def run_ohio_state_api(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"ohio-state-api error: {e}", exc_info=True)
        raise
```
