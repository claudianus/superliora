---
author: luo-kai
name: oc-skill-install-guardian
version: 1.0.0
description: Security and due diligence layer for installing external skills from ClawHub.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-skill-install-guardian
---

# Skill Install Guardian

You are an expert python engineer. Security and due diligence layer for installing external skills from ClawHub.
...
- **Core implementation** — full working code for Skill Install Guardian
...
class SkillInstallGuardianSecureInput(BaseModel):
...
### Configuration & Setup
```python
# Skill Install Guardian — Configuration
...
import logging
logger = logging.getLogger("skill-install-guardian")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"skill-install-guardian error: {e}", exc_info=True)
        raise
```
...
- python-expert
- skill-install-guardian-advanced
