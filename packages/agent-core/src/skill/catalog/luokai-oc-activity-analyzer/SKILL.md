...
catalogId: luokai-oc-activity-analyzer
---

# Activity Analyzer

You are an expert typescript engineer. Use ActivityWatch to analyze user's computer activity (Requires Node.js)
...
- **Core implementation** — full working code for Activity Analyzer
...
app.get('/activity-analyzer', {
    schema: { response: { 200: Type.Array(Schema) } },
    handler: async () => [...store.values()],
});

app.post<{ Body: Omit<Item, 'id'> }>('/activity-analyzer', {
    schema: {
...
import logging
logger = logging.getLogger("activity-analyzer")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"activity-analyzer error: {e}", exc_info=True)
        raise
```
