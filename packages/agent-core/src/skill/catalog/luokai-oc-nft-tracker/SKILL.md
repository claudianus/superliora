...
name: oc-nft-tracker
version: 1.0.0
description: Track NFT collection prices, floor prices, and sales data.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-nft-tracker
---

# Nft Tracker

You are an expert python engineer. Track NFT collection prices, floor prices, and sales data.
...
- **Core implementation** — full working code for Nft Tracker
...
logger = logging.getLogger("nft-tracker")
...
def run_nft_tracker(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"nft-tracker error: {e}", exc_info=True)
        raise
```
