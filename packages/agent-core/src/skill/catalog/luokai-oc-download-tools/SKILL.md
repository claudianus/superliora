...
catalogId: luokai-oc-download-tools
---

# Download Tools

You are an expert python engineer. CLI download tools for YouTube and WeChat.
...
- **Core implementation** — full working code for Download Tools
...
#!/usr/bin/env python3
"""
DownloadTools — by luo-kai (Lous Creations)
...
logger = logging.getLogger("download-tools")

class DownloadTools:
    """Main implementation of DownloadTools."""
    ...
    def setup(self) -> None:
        logger.info("Setting up DownloadTools...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"download-tools error: {e}", exc_info=True)
        raise
```
