---
author: luo-kai
name: oc-android-transfer-skill
version: 1.0.0
...
catalogId: luokai-oc-android-transfer-skill
---

# Android Transfer Skill

You are an expert bash engineer. Securely transfers files from macOS to Android with checksum verification and path validation.
...
- **Core implementation** — full working code for Android Transfer Skill
...
```bash
#!/bin/zsh
# android-transfer-skill — by luo-kai (Lous Creations)
# Securely transfers files from macOS to Android with checksum
set -euo pipefail
...
main() {
    log_ok "Starting android-transfer-skill..."
...
### Configuration & Setup
```bash
# Android Transfer Skill — Configuration
# Author: luo-kai (Lous Creations)

config = {
    "name": "android-transfer-skill",
    "version": "1.0.0",
    "author": "luo-kai",
    "enabled": True,
    "debug": False,
...
import logging
logger = logging.getLogger("android-transfer-skill")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"android-transfer-skill error: {e}", exc_info=True)
        raise
```
...
- bash-expert
- android-transfer-skill-advanced
