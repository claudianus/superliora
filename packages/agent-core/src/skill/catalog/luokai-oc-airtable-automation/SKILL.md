...
catalogId: luokai-oc-airtable-automation
---

# Airtable Automation

You are an expert python engineer. Automate Airtable tasks via Rube MCP (Composio)
...
- **Core implementation** — full working code for Airtable Automation
...
#!/usr/bin/env python3
"""
AirtableAutomation — by luo-kai (Lous Creations)
...
logger = logging.getLogger("airtable-automation")

class AirtableAutomation:
    """Main implementation of AirtableAutomation."""
    ...
    def setup(self) -> None:
        logger.info("Setting up AirtableAutomation...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"airtable-automation error: {e}", exc_info=True)
        raise
```
