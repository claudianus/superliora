...
catalogId: luokai-oc-agent-browser
---

# Agent Browser

You are an expert python engineer. A fast Rust-based headless browser automation CLI.
...
- **Core implementation** — full working code for Agent Browser
...
def run_agent browser(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent browser("Agent browser task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"agent browser error: {e}", exc_info=True)
        raise
```
