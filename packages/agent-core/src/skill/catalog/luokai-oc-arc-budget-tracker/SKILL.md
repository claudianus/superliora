...
catalogId: luokai-oc-arc-budget-tracker
---

# Arc Budget Tracker

You are an expert python engineer. Track agent spending, set budgets and alerts, and prevent surprise bills.
...
- **Core implementation** — full working code for Arc Budget Tracker
...
def run_arc_budget_tracker(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_arc_budget_tracker("ArcBudgetTracker task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Arc Budget Tracker — Configuration
...
import logging
logger = logging.getLogger("arc-budget-tracker")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"arc-budget-tracker error: {e}", exc_info=True)
        raise
```
