...
req = urllib.request.Request('https://api.maton.ai/google-play/androidpublisher/v3/applications/{packageName}/inappproducts')
...
```
https://api.maton.ai/google-play/{native-api-path}
```

Maton proxies requests to `androidpublisher.googleapis.com` and automatically injects your OAuth token.
...
python <<'EOF'
import urllib.request, os, json
req = urllib.request.Request('https://api.maton.ai/connections?app=google-play&status=ACTIVE')
req.add_header('Authorization', f'Bearer {os.environ["MATON_API_KEY"]}')
print(json.dumps(json.load(urllib.request.urlopen(req)), indent=2))
EOF
```
...
const response = await fetch(
  `https://api.maton.ai/google-play/androidpublisher/v3/applications/${packageName}/inappproducts`,
  {
    headers: {
...
response = requests.get(
    f'https://api.maton.ai/google-play/androidpublisher/v3/applications/{package_name}/inappproducts',
    headers=headers
)
...
- Correct: `https://api.maton.ai/google-play/androidpublisher/v3/applications/{packageName}/inappproducts`
- Incorrect: `https://api.maton.ai/androidpublisher/v3/applications/{packageName}/inappproducts`
