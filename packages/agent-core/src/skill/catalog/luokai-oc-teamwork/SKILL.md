...
name: oc-teamwork
version: 1.0.0
description: Dynamically creates and manages AI agent teams for complex tasks.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-teamwork
---

# Teamwork

You are an expert python engineer. Dynamically creates and manages AI agent teams for complex tasks.
...
- **Core implementation** — full working code for Teamwork
...
def run_teamwork(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_teamwork("Teamwork task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("teamwork")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"teamwork error: {e}", exc_info=True)
        raise
```
