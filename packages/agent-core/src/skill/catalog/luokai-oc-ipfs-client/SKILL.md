...
name: oc-ipfs-client
version: 1.0.0
description: Read-only IPFS queries — fetch files, inspect metadata, explore DAGs, and resolve IPNS names via local or public.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-ipfs-client
---

# Ipfs Client

You are an expert python engineer. Read-only IPFS queries — fetch files, inspect metadata, explore DAGs, and resolve IPNS names via local or public.
...
- **Core implementation** — full working code for Ipfs Client
...
logger = logging.getLogger("ipfs-client")
...
def run_ipfs_client(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"ipfs-client error: {e}", exc_info=True)
        raise
```
