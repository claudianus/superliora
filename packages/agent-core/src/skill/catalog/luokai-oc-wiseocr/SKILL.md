...
name: oc-wiseocr
version: 1.0.0
description: Convert PDF files to Markdown using WiseOCR API (powered by WiseDiag)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-wiseocr
---

# Wiseocr

You are an expert python engineer. Convert PDF files to Markdown using WiseOCR API (powered by WiseDiag)
...
- **Core implementation** — full working code for Wiseocr
...
#!/usr/bin/env python3
"""
Wiseocr — by luo-kai (Lous Creations)
Convert PDF files to Markdown using WiseOCR API (powered by WiseDiag)
"""
...
logger = logging.getLogger("wiseocr")

class Wiseocr:
    """Main implementation of Wiseocr."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Wiseocr...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"wiseocr error: {e}", exc_info=True)
        raise
```
