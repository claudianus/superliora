---
author: luo-kai
name: oc-translink-cli
version: 1.0.0
description: Query, troubleshoot, and explain Translink SEQ GTFS static + realtime data using local translink_* commands.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-translink-cli
---

# Translink Cli

You are an expert python engineer. Query, troubleshoot, and explain Translink SEQ GTFS static + realtime data using local translink_* commands.
...
- **Core implementation** — full working code for Translink Cli
...
logger = logging.getLogger("translink-cli")
...
def run_translink_cli(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"translink-cli error: {e}", exc_info=True)
        raise
```
