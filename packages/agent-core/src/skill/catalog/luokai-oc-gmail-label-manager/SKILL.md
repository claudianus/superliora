---
author: luo-kai
name: oc-gmail-label-manager
version: 1.0.0
description: This skill automates the organization of unread Gmail messages by applying labels, removing unnecessary labels.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-gmail-label-manager
---

# Gmail Label Manager

You are an expert python engineer. This skill automates the organization of unread Gmail messages by applying labels, removing unnecessary labels.
...
- **Core implementation** — full working code for Gmail Label Manager
...
#!/usr/bin/env python3
"""
GmailLabelManager — by luo-kai (Lous Creations)
This skill automates the organization of unread Gmail messages by applying label
"""
...
logger = logging.getLogger("gmail-label-manager")

class GmailLabelManager:
    """Main implementation of GmailLabelManager."""
    ...
    def setup(self) -> None:
        logger.info("Setting up GmailLabelManager...")
        self._ready = True
...
### Configuration & Setup
```python
# Gmail Label Manager — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"gmail-label-manager error: {e}", exc_info=True)
        raise
```
