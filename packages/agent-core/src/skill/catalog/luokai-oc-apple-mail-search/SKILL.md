...
catalogId: luokai-oc-apple-mail-search
---

# Apple Mail Search

You are an expert bash engineer. Fast Apple Mail search via SQLite on macOS.
...
- **Core implementation** — full working code for Apple Mail Search
...
```bash
#!/bin/zsh
# apple-mail-search — by luo-kai (Lous Creations)
...
main() {
    log_ok "Starting apple-mail-search..."
...
### Configuration & Setup
```bash
# Apple Mail Search — Configuration
...
import logging
logger = logging.getLogger("apple-mail-search")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"apple-mail-search error: {e}", exc_info=True)
        raise
```
