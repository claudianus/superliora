---
author: luo-kai
name: oc-presale-one-pass-orchestrator
version: 1.0.0
description: Run or supervise a one-pass Codex implementation with preflight gates, stepwise plan execution, and strict QA.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-presale-one-pass-orchestrator
---

# Presale One Pass Orchestrator

You are an expert python engineer. Run or supervise a one-pass Codex implementation with preflight gates, stepwise plan execution, and strict QA.
...
- **Core implementation** — full working code for Presale One Pass Orchestrator
...
def run_presale_one_pass_orchestrator(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_presale_one_pass_orchestrator("PresaleOnePassOrchestrator task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Presale One Pass Orchestrator — Configuration
...
import logging
logger = logging.getLogger("presale-one-pass-orchestrator")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"presale-one-pass-orchestrator error: {e}", exc_info=True)
        raise
```
...
- python-expert
- presale-one-pass-orchestrator-advanced
