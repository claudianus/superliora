---
author: luo-kai
name: oc-macro-regime-detector
version: 1.0.0
description: Detect the current macro regime (Risk-On, Risk-Off, Inflationary, Deflationary, Stagflation) using multi-source.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-macro-regime-detector
---

# Macro Regime Detector

You are an expert python engineer. Detect the current macro regime (Risk-On, Risk-Off, Inflationary, Deflationary, Stagflation) using multi-source.
...
- **Core implementation** — full working code for Macro Regime Detector
...
#!/usr/bin/env python3
"""
MacroRegimeDetector — by luo-kai (Lous Creations)
...
logger = logging.getLogger("macro-regime-detector")

class MacroRegimeDetector:
    """Main implementation of MacroRegimeDetector."""
    ...
    def setup(self) -> None:
        logger.info("Setting up MacroRegimeDetector...")
        self._ready = True
...
### Configuration & Setup
```python
# Macro Regime Detector — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"macro-regime-detector error: {e}", exc_info=True)
        raise
```
...
- python-expert
- macro-regime-detector-advanced
