...
name: oc-riskofficer
version: 1.0.0
description: Manage investment portfolios, calculate risk metrics.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-riskofficer
---

# Riskofficer

You are an expert swift engineer. Manage investment portfolios, calculate risk metrics.
...
- **Core implementation** — full working code for Riskofficer
...
class RiskofficerViewModel {
    ...
    func load() async {
        ...
        do {
            let url = URL(string: "https://api.example.com/riskofficer")!
...
struct RiskofficerView: View {
    @State private var vm = RiskofficerViewModel()
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"riskofficer error: {e}", exc_info=True)
        raise
```
