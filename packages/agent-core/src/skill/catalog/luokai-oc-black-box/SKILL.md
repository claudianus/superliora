...
catalogId: luokai-oc-black-box
---

# Black Box

You are an expert python engineer. Indestructible audit logs for agent actions, stored in TiDB Zero.
...
- **Core implementation** — full working code for Black Box
...
def run_black_box(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_black_box("BlackBox task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"black-box error: {e}", exc_info=True)
        raise
```
