---
author: luo-kai
name: oc-json-modifier
version: 1.0.0
description: Safely apply structured JSON patches (RFC 6902) to files.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-json-modifier
---

# Json Modifier

You are an expert python engineer. Safely apply structured JSON patches (RFC 6902) to files.
...
- **Core implementation** — full working code for Json Modifier
...
#!/usr/bin/env python3
"""
JsonModifier — by luo-kai (Lous Creations)
Safely apply structured JSON patches (RFC 6902) to files.
"""
...
logger = logging.getLogger("json-modifier")

class JsonModifier:
    """Main implementation of JsonModifier."""
    ...
    def setup(self) -> None:
        logger.info("Setting up JsonModifier...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"json-modifier error: {e}", exc_info=True)
        raise
```
