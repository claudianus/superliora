---
author: luo-kai
name: oc-yr-no-weather
version: 1.0.0
description: Fetch weather forecasts from the Norwegian Meteorological Institute (MET) using the yr.no API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-yr-no-weather
---

# Yr No Weather

You are an expert python engineer. Fetch weather forecasts from the Norwegian Meteorological Institute (MET) using the yr.no API.
...
- **Core implementation** — full working code for Yr No Weather
...
#!/usr/bin/env python3
"""
YrNoWeather — by luo-kai (Lous Creations)
Fetch weather forecasts from the Norwegian Meteorological Institute (MET) using 
"""
...
logger = logging.getLogger("yr-no-weather")

class YrNoWeather:
    """Main implementation of YrNoWeather."""
    ...
    def setup(self) -> None:
        logger.info("Setting up YrNoWeather...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"yr-no-weather error: {e}", exc_info=True)
        raise
```
