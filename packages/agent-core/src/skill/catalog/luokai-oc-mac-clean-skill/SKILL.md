...
catalogId: luokai-oc-mac-clean-skill
---

# Mac Clean Skill

You are an expert bash engineer. Cleans up system caches, trash, and old downloads on macOS.
...
- **Core implementation** — full working code for Mac Clean Skill
...
```bash
#!/bin/zsh
# mac-clean-skill — by luo-kai (Lous Creations)
...
main() {
    log_ok "Starting mac-clean-skill..."
...
### Configuration & Setup
```bash
# Mac Clean Skill — Configuration
# Author: luo-kai (Lous Creations)

config = {
    "name": "mac-clean-skill",
    "version": "1.0.0",
    "author": "luo-kai",
    "enabled": True,
    "debug": False,
...
import logging
logger = logging.getLogger("mac-clean-skill")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"mac-clean-skill error: {e}", exc_info=True)
        raise
```
