...
name: oc-feed-digest
version: 1.0.0
description: Agentic feed digest using the feed CLI.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-feed-digest
---

# Feed Digest

You are an expert python engineer. Agentic feed digest using the feed CLI.
...
- **Core implementation** — full working code for Feed Digest
...
def run_feed_digest(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_feed_digest("FeedDigest task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Feed Digest — Configuration
...
import logging
logger = logging.getLogger("feed-digest")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"feed-digest error: {e}", exc_info=True)
        raise
```
