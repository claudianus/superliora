---
author: luo-kai
name: oc-travel-agent
version: 1.0.0
description: Find, book, and change flights for your human via email.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-travel-agent
---

# Travel Agent

You are an expert python engineer. Find, book, and change flights for your human via email.
...
- **Core implementation** — full working code for Travel Agent
...
def run_travel_agent(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_travel_agent("TravelAgent task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"travel-agent error: {e}", exc_info=True)
        raise
```
