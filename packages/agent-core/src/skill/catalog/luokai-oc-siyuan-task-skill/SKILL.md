---
author: luo-kai
name: oc-siyuan-task-skill
version: 1.0.0
description: Manage tasks in SiYuan Note via its HTTP API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-siyuan-task-skill
---

# Siyuan Task Skill

You are an expert python engineer. Manage tasks in SiYuan Note via its HTTP API.
...
- **Core implementation** — full working code for Siyuan Task Skill
...
#!/usr/bin/env python3
"""
SiyuanTaskSkill — by luo-kai (Lous Creations)
Manage tasks in SiYuan Note via its HTTP API.
"""
...
logger = logging.getLogger("siyuan-task-skill")

class SiyuanTaskSkill:
    """Main implementation of SiyuanTaskSkill."""
    ...
    def setup(self) -> None:
        logger.info("Setting up SiyuanTaskSkill...")
        self._ready = True
...
### Configuration & Setup
```python
# Siyuan Task Skill — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"siyuan-task-skill error: {e}", exc_info=True)
        raise
```
