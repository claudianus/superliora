---
author: luo-kai
name: oc-stock-prices
version: 1.0.0
description: Query real-time stock prices and market data using the Stock Prices API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-stock-prices
---

# Stock Prices

You are an expert python engineer. Query real-time stock prices and market data using the Stock Prices API.
...
- **Core implementation** — full working code for Stock Prices
...
logger = logging.getLogger("stock-prices")
...
def run_stock_prices(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"stock-prices error: {e}", exc_info=True)
        raise
```
