...
name: oc-mt5-httpapi
version: 1.0.0
description: MetaTrader 5 trading via REST API — get market data, place/modify/close orders, manage positions, pull history.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mt5-httpapi
---

# Mt5 Httpapi

You are an expert python engineer. MetaTrader 5 trading via REST API — get market data, place/modify/close orders, manage positions, pull history.
...
- **Core implementation** — full working code for Mt5 Httpapi
...
logger = logging.getLogger("mt5-httpapi")
...
def run_mt5_httpapi(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mt5-httpapi error: {e}", exc_info=True)
        raise
```
