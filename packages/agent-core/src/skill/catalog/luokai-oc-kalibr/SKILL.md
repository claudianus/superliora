...
name: oc-kalibr
version: 1.0.0
description: Ship agents that fix themselves.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-kalibr
---

# Kalibr

You are an expert python engineer. Ship agents that fix themselves.
...
- **Core implementation** — full working code for Kalibr
...
def run_kalibr(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_kalibr("Kalibr task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"kalibr error: {e}", exc_info=True)
        raise
```
