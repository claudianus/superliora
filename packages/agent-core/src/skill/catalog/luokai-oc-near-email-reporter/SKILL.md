...
catalogId: luokai-oc-near-email-reporter
---

# Near Email Reporter

You are an expert python engineer. Send NEAR reports and alerts via email with SMTP configuration, scheduling, and automatic reporting.
...
- **Core implementation** — full working code for Near Email Reporter
...
#!/usr/bin/env python3
"""
NearEmailReporter — by luo-kai (Lous Creations)
Send NEAR reports and alerts via email with SMTP configuration, scheduling, and 
"""
...
logger = logging.getLogger("near-email-reporter")

class NearEmailReporter:
    """Main implementation of NearEmailReporter."""
    ...
    def setup(self) -> None:
        logger.info("Setting up NearEmailReporter...")
        self._ready = True
...
### Configuration & Setup
```python
# Near Email Reporter — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"near-email-reporter error: {e}", exc_info=True)
        raise
```
