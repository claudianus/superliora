...
catalogId: luokai-oc-amazon-orders
---

# Amazon Orders

You are an expert python engineer. Download and query your Amazon order history via an unofficial Python API and CLI.
...
- **Core implementation** — full working code for Amazon Orders
...
app = FastAPI(title="AmazonOrders", version="1.0.0")

class AmazonOrdersItem(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True)
    name: Annotated[str, Field(min_length=1, max_length=200)]
    description: str | None = None
    tags: list[str] = []

class AmazonOrdersResponse(AmazonOrdersItem):
    id: str

store: dict[str, AmazonOrdersResponse] = {}

@app.get("/amazon-orders", response_model=list[AmazonOrdersResponse])
async def list_items(
    skip: int = Query(0, ge=0),
    limit: int = Query(20, ge=1, le=100),
) -> list[AmazonOrdersResponse]:
    return list(store.values())[skip:skip + limit]

@app.post("/amazon-orders", response_model=AmazonOrdersResponse, status_code=201)
async def create_item(payload: AmazonOrdersItem) -> AmazonOrdersResponse:
    import uuid
    item = AmazonOrdersResponse(id=str(uuid.uuid4()), **payload.model_dump())
    store[item.id] = item
    return item

@app.get("/amazon-orders/{item_id}")
async def get_item(item_id: str) -> AmazonOrdersResponse:
...
import logging
logger = logging.getLogger("amazon-orders")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"amazon-orders error: {e}", exc_info=True)
        raise
```
