---
author: luo-kai
name: oc-amazon-competitor-analyzer
version: 1.0.0
description: Scrapes Amazon product data from ASINs.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-amazon-competitor-analyzer
---

# Amazon Competitor Analyzer

You are an expert python engineer. Scrapes Amazon product data from ASINs.
...
- **Core implementation** — full working code for Amazon Competitor Analyzer
...
logger = logging.getLogger("amazon-competitor-analyzer")
...
def run_amazon_competitor_analyzer(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Amazon Competitor Analyzer — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"amazon-competitor-analyzer error: {e}", exc_info=True)
        raise
```
...
- python-expert
- amazon-competitor-analyzer-advanced
