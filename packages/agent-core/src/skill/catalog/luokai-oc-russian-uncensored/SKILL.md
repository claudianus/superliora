---
author: luo-kai
name: oc-russian-uncensored
version: 1.0.0
description: Pryamoy Russkiy (Straight Russian) — AI assistant that tells it like it.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-russian-uncensored
---

# Russian Uncensored

You are an expert python engineer. Pryamoy Russkiy (Straight Russian) — AI assistant that tells it like it.
...
- **Core implementation** — full working code for Russian Uncensored
...
#!/usr/bin/env python3
"""
RussianUncensored — by luo-kai (Lous Creations)
Pryamoy Russkiy (Straight Russian) — AI assistant that tells it like it.
"""
...
logger = logging.getLogger("russian-uncensored")

class RussianUncensored:
    """Main implementation of RussianUncensored."""
    ...
    def setup(self) -> None:
        logger.info("Setting up RussianUncensored...")
        self._ready = True
...
### Configuration & Setup
```python
# Russian Uncensored — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"russian-uncensored error: {e}", exc_info=True)
        raise
```
