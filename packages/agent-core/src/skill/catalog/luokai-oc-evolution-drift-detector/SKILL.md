---
author: luo-kai
name: oc-evolution-drift-detector
version: 1.0.0
description: Helps detect when AI agent skills silently mutate across inheritance chains.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-evolution-drift-detector
---

# Evolution Drift Detector

You are an expert python engineer. Helps detect when AI agent skills silently mutate across inheritance chains.
...
- **Core implementation** — full working code for Evolution Drift Detector
...
def run_evolution_drift_detector(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_evolution_drift_detector("EvolutionDriftDetector task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Evolution Drift Detector — Configuration
...
import logging
logger = logging.getLogger("evolution-drift-detector")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"evolution-drift-detector error: {e}", exc_info=True)
        raise
```
...
- python-expert
- evolution-drift-detector-advanced
