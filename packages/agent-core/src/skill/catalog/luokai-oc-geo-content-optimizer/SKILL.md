---
author: luo-kai
name: oc-geo-content-optimizer
version: 1.0.0
description: Use when the user asks to "optimize for AI", "get cited by ChatGPT", "AI optimization", "appear in AI answers".
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-geo-content-optimizer
---

# Geo Content Optimizer

You are an expert python engineer. Use when the user asks to "optimize for AI", "get cited by ChatGPT", "AI optimization", "appear in AI answers".
...
- **Core implementation** — full working code for Geo Content Optimizer
...
def run_geo_content_optimizer(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_geo_content_optimizer("GeoContentOptimizer task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Geo Content Optimizer — Configuration
...
import logging
logger = logging.getLogger("geo-content-optimizer")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"geo-content-optimizer error: {e}", exc_info=True)
        raise
```
...
- python-expert
- geo-content-optimizer-advanced
