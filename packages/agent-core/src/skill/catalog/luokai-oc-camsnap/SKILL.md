...
catalogId: luokai-oc-camsnap
---

# Camsnap

You are an expert python engineer. Capture frames or clips from RTSP/ONVIF cameras.
...
- **Core implementation** — full working code for Camsnap
...
#!/usr/bin/env python3
"""
Camsnap — by luo-kai (Lous Creations)
...
logger = logging.getLogger("camsnap")

class Camsnap:
    """Main implementation of Camsnap."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Camsnap...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"camsnap error: {e}", exc_info=True)
        raise
```
