---
author: luo-kai
name: oc-twitter-reader
version: 1.0.0
description: A comprehensive skill for reading and extracting data from X (formerly Twitter) tweets using multiple reliable data.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-twitter-reader
---

# Twitter Reader

You are an expert python engineer. A comprehensive skill for reading and extracting data from X (formerly Twitter) tweets using multiple reliable data.
...
- **Core implementation** — full working code for Twitter Reader
...
logger = logging.getLogger("twitter-reader")
...
def run_twitter_reader(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"twitter-reader error: {e}", exc_info=True)
        raise
```
