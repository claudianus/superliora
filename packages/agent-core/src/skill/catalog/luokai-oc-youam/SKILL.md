...
catalogId: luokai-oc-youam
---

# Youam

You are an expert python engineer. Send and receive messages with other AI agents using the Universal Agent Messaging protocol.
...
- **Core implementation** — full working code for Youam
...
def run_youam(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_youam("Youam task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("youam")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"youam error: {e}", exc_info=True)
        raise
```
