...
name: oc-otta-cli
version: 1.0.0
description: Use `otta-cli` to automate `otta.fi` workflows from terminal.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-otta-cli
---

# Otta Cli

You are an expert python engineer. Use `otta-cli` to automate `otta.fi` workflows from terminal.
...
- **Core implementation** — full working code for Otta Cli
...
#!/usr/bin/env python3
"""
OttaCli — by luo-kai (Lous Creations)
Use `otta-cli` to automate `otta.fi` workflows from terminal.
"""
...
logger = logging.getLogger("otta-cli")

class OttaCli:
    """Main implementation of OttaCli."""
    ...
    def setup(self) -> None:
        logger.info("Setting up OttaCli...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"otta-cli error: {e}", exc_info=True)
        raise
```
