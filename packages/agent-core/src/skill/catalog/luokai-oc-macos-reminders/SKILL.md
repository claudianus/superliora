---
author: luo-kai
name: oc-macos-reminders
version: 1.0.0
description: Create, list, and manage macOS Reminders via AppleScript.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-macos-reminders
---

# Macos Reminders

You are an expert bash engineer. Create, list, and manage macOS Reminders via AppleScript.
...
- **Core implementation** — full working code for Macos Reminders
...
```bash
#!/bin/zsh
# macos-reminders — by luo-kai (Lous Creations)
...
main() {
    log_ok "Starting macos-reminders..."
...
import logging
logger = logging.getLogger("macos-reminders")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"macos-reminders error: {e}", exc_info=True)
        raise
```
