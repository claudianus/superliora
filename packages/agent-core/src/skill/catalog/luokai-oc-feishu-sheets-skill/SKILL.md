---
author: luo-kai
name: oc-feishu-sheets-skill
version: 1.0.0
description: Feishu online spreadsheet (Sheets) operations including create, read, write, append data, manage worksheets.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-feishu-sheets-skill
---

# Feishu Sheets Skill

You are an expert python engineer. Feishu online spreadsheet (Sheets) operations including create, read, write, append data, manage worksheets.
...
- **Core implementation** — full working code for Feishu Sheets Skill
...
logger = logging.getLogger("feishu-sheets-skill")
...
def run_feishu_sheets_skill(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Feishu Sheets Skill — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"feishu-sheets-skill error: {e}", exc_info=True)
        raise
```
