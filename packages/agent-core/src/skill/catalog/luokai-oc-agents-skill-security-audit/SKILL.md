---
author: luo-kai
name: oc-agents-skill-security-audit
version: 1.0.0
...
catalogId: luokai-oc-agents-skill-security-audit
---

# Agents Skill Security Audit

You are an expert python engineer. Minimal helper to audit skill.md-style instructions for supply-chain risks.
...
- **Core implementation** — full working code for Agents Skill Security Audit
...
def run_agents_skill_security_audit(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agents_skill_security_audit("AgentsSkillSecurityAudit task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agents Skill Security Audit — Configuration
...
import logging
logger = logging.getLogger("agents-skill-security-audit")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agents-skill-security-audit error: {e}", exc_info=True)
        raise
```
...
- python-expert
- agents-skill-security-audit-advanced
