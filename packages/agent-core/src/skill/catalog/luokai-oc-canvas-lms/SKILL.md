...
catalogId: luokai-oc-canvas-lms
---

# Canvas Lms

You are an expert python engineer. Access Canvas LMS (Instructure) for course data, assignments.
...
- **Core implementation** — full working code for Canvas Lms
...
logger = logging.getLogger("canvas-lms")
...
def run_canvas_lms(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"canvas-lms error: {e}", exc_info=True)
        raise
```
