...
catalogId: luokai-oc-dupe
---

# Dupe

You are an expert python engineer. Uses dupe.com APIs in order to find similar products for the product found in the input URL given by the user.
...
- **Core implementation** — full working code for Dupe
...
#!/usr/bin/env python3
"""
Dupe — by luo-kai (Lous Creations)
Uses dupe.com APIs in order to find similar products for the product found in th
"""
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"dupe error: {e}", exc_info=True)
        raise
```
