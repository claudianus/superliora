...
catalogId: luokai-oc-box-cli
---

# Box Cli

You are an expert python engineer. Box CLI skill for working with files, folders, metadata,.
...
- **Core implementation** — full working code for Box Cli
...
logger = logging.getLogger("box-cli")
...
def run_box_cli(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"box-cli error: {e}", exc_info=True)
        raise
```
