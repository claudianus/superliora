...
catalogId: luokai-oc-pipelock
---

# Pipelock

You are an expert python engineer. Secure agent HTTP requests through a scanning proxy that catches credential leaks, SSRF, and prompt injection.
...
- **Core implementation** — full working code for Pipelock
...
def run_pipelock(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_pipelock("Pipelock task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("pipelock")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"pipelock error: {e}", exc_info=True)
        raise
```
