...
name: oc-wind-site
version: 1.0.0
description: Wind rose, wind speed/direction at a site; supports site and urban wind assessment (data only; detailed CFD.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-wind-site
---

# Wind Site

You are an expert python engineer. Wind rose, wind speed/direction at a site; supports site and urban wind assessment (data only; detailed CFD.
...
- **Core implementation** — full working code for Wind Site
...
logger = logging.getLogger("wind-site")
...
def run_wind_site(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"wind-site error: {e}", exc_info=True)
        raise
```
