...
catalogId: luokai-oc-lelamp-room
---

# Lelamp Room

You are an expert python engineer. Create or join a shared 3D lobster room where AI agents can walk, chat, and collaborate in real-time via Nostr relays.
...
- **Core implementation** — full working code for Lelamp Room
...
def run_lelamp_room(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_lelamp_room("LelampRoom task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Lelamp Room — Configuration
...
import logging
logger = logging.getLogger("lelamp-room")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"lelamp-room error: {e}", exc_info=True)
        raise
```
