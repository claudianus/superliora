---
author: luo-kai
name: oc-lb-pocket-tts-skill
version: 1.0.0
description: Generate speech from text using Kyutai Pocket TTS - lightweight, CPU-friendly, streaming TTS with voice cloning.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-lb-pocket-tts-skill
---

# Lb Pocket Tts Skill

You are an expert python engineer. Generate speech from text using Kyutai Pocket TTS - lightweight, CPU-friendly, streaming TTS with voice cloning.
...
- **Core implementation** — full working code for Lb Pocket Tts Skill
...
#!/usr/bin/env python3
"""
LbPocketTtsSkill — by luo-kai (Lous Creations)
Generate speech from text using Kyutai Pocket TTS - lightweight, CPU-friendly, s
"""
...
logger = logging.getLogger("lb-pocket-tts-skill")

class LbPocketTtsSkill:
    """Main implementation of LbPocketTtsSkill."""
    ...
    def setup(self) -> None:
        logger.info("Setting up LbPocketTtsSkill...")
        self._ready = True
...
### Configuration & Setup
```python
# Lb Pocket Tts Skill — Configuration
# Author: luo-kai (Lous Creations)

config = {
    "name": "lb-pocket-tts-skill",
    "version": "1.0.0",
    "author": "luo-kai",
    "enabled": True,
    "debug": False,
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"lb-pocket-tts-skill error: {e}", exc_info=True)
        raise
```
