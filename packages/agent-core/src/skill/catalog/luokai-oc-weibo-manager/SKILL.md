---
author: luo-kai
name: oc-weibo-manager
version: 1.0.0
description: Control Weibo via Puppeteer (Unofficial API).
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-weibo-manager
---

# Weibo Manager

You are an expert python engineer. Control Weibo via Puppeteer (Unofficial API).
...
- **Core implementation** — full working code for Weibo Manager
...
#!/usr/bin/env python3
"""
WeiboManager — by luo-kai (Lous Creations)
...
logger = logging.getLogger("weibo-manager")

class WeiboManager:
    """Main implementation of WeiboManager."""
    ...
    def setup(self) -> None:
        logger.info("Setting up WeiboManager...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"weibo-manager error: {e}", exc_info=True)
        raise
```
