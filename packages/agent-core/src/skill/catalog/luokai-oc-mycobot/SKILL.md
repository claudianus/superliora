...
name: oc-mycobot
version: 1.0.0
description: WhatsApp Business API integration with managed OAuth.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mycobot
---

# Mycobot

You are an expert python engineer. WhatsApp Business API integration with managed OAuth.
...
- **Core implementation** — full working code for Mycobot
...
class MycobotSecureInput(BaseModel):
...
import logging
logger = logging.getLogger("mycobot")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"mycobot error: {e}", exc_info=True)
        raise
```
