---
author: luo-kai
name: oc-intelligent-router
version: 1.0.0
description: Intelligent model routing for sub-agent task delegation.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-intelligent-router
---

# Intelligent Router

You are an expert python engineer. Intelligent model routing for sub-agent task delegation.
...
- **Core implementation** — full working code for Intelligent Router
...
def run_intelligent_router(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_intelligent_router("IntelligentRouter task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("intelligent-router")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"intelligent-router error: {e}", exc_info=True)
        raise
```
