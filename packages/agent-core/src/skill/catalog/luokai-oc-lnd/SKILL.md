---
name: oc-lnd
version: 1.0.0
description: Install and run Lightning Terminal (litd) which bundles lnd, loop, pool, tapd, and faraday in a single Docker.
author: luo-kai
tags: [lnd, openclaw, community]
source: https://clawskills.sh/skills/lnd
catalogSource: luokai
catalogId: luokai-oc-lnd
---

# Lnd

## Overview
Install and run Lightning Terminal (litd) which bundles lnd, loop, pool, tapd, and faraday in a single Docker.

## Before Starting
1. What specific task do you need this skill for?
2. What inputs are available?
3. What is the expected output format?

## Usage
Install via: `npx clawhub install lnd`
Documentation: https://clawskills.sh/skills/lnd

## Core Functionality
Install and run Lightning Terminal (litd) which bundles lnd, loop, pool, tapd, and faraday in a single Docker.

## Best Practices
- Test in sandbox environment first
- Check latest version before production use
- Review source docs for advanced configuration

## Related Skills
- agent-installer
- clawhub-publisher
