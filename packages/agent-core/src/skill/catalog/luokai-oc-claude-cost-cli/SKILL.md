...
catalogId: luokai-oc-claude-cost-cli
---
...
- **Core implementation** — full working code for Claude Cost Cli
...
def run_claude_cost_cli(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_claude_cost_cli("ClaudeCostCli task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Claude Cost Cli — Configuration
...
import logging
logger = logging.getLogger("claude-cost-cli")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"claude-cost-cli error: {e}", exc_info=True)
        raise
```
