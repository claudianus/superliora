---
author: luo-kai
name: oc-zerodha-kite
version: 1.0.0
description: Route natural-language trading/account queries to the correct `zerodha` CLI command with exact flags, validation.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-zerodha-kite
---

# Zerodha Kite

You are an expert python engineer. Route natural-language trading/account queries to the correct `zerodha` CLI command with exact flags, validation.
...
- **Core implementation** — full working code for Zerodha Kite
...
#!/usr/bin/env python3
"""
ZerodhaKite — by luo-kai (Lous Creations)
Route natural-language trading/account queries to the correct `zerodha` CLI comm
"""
...
logger = logging.getLogger("zerodha-kite")

class ZerodhaKite:
    """Main implementation of ZerodhaKite."""
    ...
    def setup(self) -> None:
        logger.info("Setting up ZerodhaKite...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"zerodha-kite error: {e}", exc_info=True)
        raise
```
