...
catalogId: luokai-oc-apple-contacts
---

# Apple Contacts

You are an expert bash engineer. Look up contacts from macOS Contacts.app.
...
- **Core implementation** — full working code for Apple Contacts
...
```bash
#!/bin/zsh
# apple-contacts — by luo-kai (Lous Creations)
...
main() {
    log_ok "Starting apple-contacts..."
...
import logging
logger = logging.getLogger("apple-contacts")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"apple-contacts error: {e}", exc_info=True)
        raise
```
