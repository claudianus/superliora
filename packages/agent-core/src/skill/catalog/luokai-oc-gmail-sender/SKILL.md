---
author: luo-kai
name: oc-gmail-sender
version: 1.0.0
description: Send emails via Gmail SMTP using Google App Password.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-gmail-sender
---

# Gmail Sender

You are an expert python engineer. Send emails via Gmail SMTP using Google App Password.
...
- **Core implementation** — full working code for Gmail Sender
...
#!/usr/bin/env python3
"""
GmailSender — by luo-kai (Lous Creations)
...
logger = logging.getLogger("gmail-sender")

class GmailSender:
    """Main implementation of GmailSender."""
    ...
    def setup(self) -> None:
        logger.info("Setting up GmailSender...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"gmail-sender error: {e}", exc_info=True)
        raise
```
