---
author: luo-kai
name: oc-macos-calendar
version: 1.0.0
description: Create, list, and manage macOS Calendar events via AppleScript.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-macos-calendar
---

# Macos Calendar

You are an expert bash engineer. Create, list, and manage macOS Calendar events via AppleScript.
...
- **Core implementation** — full working code for Macos Calendar
...
```bash
#!/bin/zsh
# macos-calendar — by luo-kai (Lous Creations)
# Create, list, and manage macOS Calendar events via AppleScri
set -euo pipefail
...
main() {
    log_ok "Starting macos-calendar..."
...
import logging
logger = logging.getLogger("macos-calendar")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"macos-calendar error: {e}", exc_info=True)
        raise
```
