...
name: oc-arccos-golf
version: 1.0.0
description: Analyze Arccos Golf performance data including club distances, strokes gained metrics, scoring patterns.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-arccos-golf
---

# Arccos Golf

You are an expert python engineer. Analyze Arccos Golf performance data including club distances, strokes gained metrics, scoring patterns.
...
- **Core implementation** — full working code for Arccos Golf
...
logger = logging.getLogger("arccos-golf")
...
def run_arccos_golf(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"arccos-golf error: {e}", exc_info=True)
        raise
```
