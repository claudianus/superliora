---
author: luo-kai
name: oc-sentry-issues
version: 1.0.0
description: Fetch and analyze issues from Sentry error tracking.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-sentry-issues
---

# Sentry Issues

You are an expert python engineer. Fetch and analyze issues from Sentry error tracking.
...
- **Core implementation** — full working code for Sentry Issues
...
#!/usr/bin/env python3
"""
SentryIssues — by luo-kai (Lous Creations)
...
logger = logging.getLogger("sentry-issues")

class SentryIssues:
    """Main implementation of SentryIssues."""
    ...
    def setup(self) -> None:
        logger.info("Setting up SentryIssues...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"sentry-issues error: {e}", exc_info=True)
        raise
```
