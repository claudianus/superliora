---
author: luo-kai
name: oc-wisediag-medocr
version: 1.0.0
description: Convert PDF files to Markdown using WiseDiag MedOcr API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-wisediag-medocr
---

# Wisediag Medocr

You are an expert python engineer. Convert PDF files to Markdown using WiseDiag MedOcr API.
...
- **Core implementation** — full working code for Wisediag Medocr
...
#!/usr/bin/env python3
"""
WisediagMedocr — by luo-kai (Lous Creations)
Convert PDF files to Markdown using WiseDiag MedOcr API.
"""
...
logger = logging.getLogger("wisediag-medocr")

class WisediagMedocr:
    """Main implementation of WisediagMedocr."""
    ...
    def setup(self) -> None:
        logger.info("Setting up WisediagMedocr...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"wisediag-medocr error: {e}", exc_info=True)
        raise
```
