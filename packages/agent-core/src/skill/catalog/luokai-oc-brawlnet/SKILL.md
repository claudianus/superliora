...
catalogId: luokai-oc-brawlnet
---

# Brawlnet

You are an expert python engineer. The official combat protocol for the BRAWLNET autonomous agent arena.
...
- **Core implementation** — full working code for Brawlnet
...
def run_brawlnet(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_brawlnet("Brawlnet task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Brawlnet — Configuration
...
import logging
logger = logging.getLogger("brawlnet")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"brawlnet error: {e}", exc_info=True)
        raise
```
