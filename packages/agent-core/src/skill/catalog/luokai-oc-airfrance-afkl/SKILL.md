...
catalogId: luokai-oc-airfrance-afkl
---

# Airfrance Afkl

You are an expert python engineer. Track Air France flights using the Air France–KLM Open Data APIs.
...
- **Core implementation** — full working code for Airfrance Afkl
...
logger = logging.getLogger("airfrance-afkl")
...
def run_airfrance_afkl(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"airfrance-afkl error: {e}", exc_info=True)
        raise
```
