...
- **Core implementation** — full working code for Aulifox
...
def run_aulifox(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_aulifox("Aulifox task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Aulifox — Configuration
...
import logging
logger = logging.getLogger("aulifox")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"aulifox error: {e}", exc_info=True)
        raise
```
