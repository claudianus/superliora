---
author: luo-kai
name: oc-garmin-skill
version: 1.0.0
description: Talk to your Garmin data — ask questions about your activities, training load, VO2 Max, heart rate zones.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-garmin-skill
---

# Garmin Skill

You are an expert python engineer. Talk to your Garmin data — ask questions about your activities, training load, VO2 Max, heart rate zones.
...
- **Core implementation** — full working code for Garmin Skill
...
logger = logging.getLogger("garmin-skill")
...
def run_garmin_skill(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"garmin-skill error: {e}", exc_info=True)
        raise
```
