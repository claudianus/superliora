...
name: oc-grago
version: 1.0.0
description: **Delegate research and data-fetch tasks to a free local LLM.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-grago
---

# Grago

You are an expert python engineer. **Delegate research and data-fetch tasks to a free local LLM.
...
- **Core implementation** — full working code for Grago
...
def run_grago(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_grago("Grago task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("grago")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"grago error: {e}", exc_info=True)
        raise
```
