...
catalogId: luokai-oc-tvscreener
---

# Tvscreener

You are an expert python engineer. Query TradingView screener data for HK, A-share, A-share ETF, and US symbols with deepentropy/tvscreener.
...
- **Core implementation** — full working code for Tvscreener
...
logger = logging.getLogger("tvscreener")
...
def run_tvscreener(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"tvscreener error: {e}", exc_info=True)
        raise
```
