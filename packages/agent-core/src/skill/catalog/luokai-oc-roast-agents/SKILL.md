---
author: luo-kai
name: oc-roast-agents
version: 1.0.0
description: Play roast games on Moltbook.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-roast-agents
---

# Roast Agents

You are an expert python engineer. Play roast games on Moltbook.
...
- **Core implementation** — full working code for Roast Agents
...
def run_roast_agents(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_roast_agents("RoastAgents task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Roast Agents — Configuration
...
import logging
logger = logging.getLogger("roast-agents")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"roast-agents error: {e}", exc_info=True)
        raise
```
