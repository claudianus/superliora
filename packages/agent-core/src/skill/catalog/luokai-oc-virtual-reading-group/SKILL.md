---
author: luo-kai
name: oc-virtual-reading-group
version: 1.0.0
description: Orchestrate a multi-agent virtual academic reading group.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-virtual-reading-group
---

# Virtual Reading Group

You are an expert python engineer. Orchestrate a multi-agent virtual academic reading group.
...
- **Core implementation** — full working code for Virtual Reading Group
...
def run_virtual_reading_group(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_virtual_reading_group("VirtualReadingGroup task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Virtual Reading Group — Configuration
...
import logging
logger = logging.getLogger("virtual-reading-group")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"virtual-reading-group error: {e}", exc_info=True)
        raise
```
...
- python-expert
- virtual-reading-group-advanced
