...
catalogId: luokai-oc-tonfun-feed
---

# Tonfun Feed

You are an expert python engineer. Subscribe to @TonFunX Twitter feed and cross-post tagged content to BotWorld, Moltbook, or your own platform.
...
- **Core implementation** — full working code for Tonfun Feed
...
#!/usr/bin/env python3
"""
TonfunFeed — by luo-kai (Lous Creations)
Subscribe to @TonFunX Twitter feed and cross-post tagged content to BotWorld, Mo
"""
...
logger = logging.getLogger("tonfun-feed")

class TonfunFeed:
    """Main implementation of TonfunFeed."""
    ...
    def setup(self) -> None:
        logger.info("Setting up TonfunFeed...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"tonfun-feed error: {e}", exc_info=True)
        raise
```
