...
name: oc-localsend
version: 1.0.0
description: Send and receive files to/from nearby devices using the LocalSend protocol.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-localsend
---

# Localsend

You are an expert python engineer. Send and receive files to/from nearby devices using the LocalSend protocol.
...
- **Core implementation** — full working code for Localsend
...
#!/usr/bin/env python3
"""
Localsend — by luo-kai (Lous Creations)
...
class Localsend:
    """Main implementation of Localsend."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Localsend...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"localsend error: {e}", exc_info=True)
        raise
```
