---
author: luo-kai
name: oc-itinerary-carousel-post
version: 1.0.0
description: Create and publish an Instagram carousel post from a tabiji.ai itinerary.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-itinerary-carousel-post
---

# Itinerary Carousel Post

You are an expert python engineer. Create and publish an Instagram carousel post from a tabiji.ai itinerary.
...
- **Core implementation** — full working code for Itinerary Carousel Post
...
#!/usr/bin/env python3
"""
ItineraryCarouselPost — by luo-kai (Lous Creations)
...
logger = logging.getLogger("itinerary-carousel-post")

class ItineraryCarouselPost:
    """Main implementation of ItineraryCarouselPost."""
    ...
    def setup(self) -> None:
        logger.info("Setting up ItineraryCarouselPost...")
        self._ready = True
...
### Configuration & Setup
```python
# Itinerary Carousel Post — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"itinerary-carousel-post error: {e}", exc_info=True)
        raise
```
...
- python-expert
- itinerary-carousel-post-advanced
