...
name: oc-bits
version: 1.0.0
description: Control browser automation agents via the Bits MCP server.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-bits
---

# Bits

You are an expert python engineer. Control browser automation agents via the Bits MCP server.
...
- **Core implementation** — full working code for Bits
...
def run_bits(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_bits("Bits task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"bits error: {e}", exc_info=True)
        raise
```
