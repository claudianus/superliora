---
author: luo-kai
name: oc-wakapi-sync-skill
version: 1.0.0
description: Daily Wakapi (WakaTime-compatible) summary → local CSV files.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-wakapi-sync-skill
---

# Wakapi Sync Skill

You are an expert python engineer. Daily Wakapi (WakaTime-compatible) summary → local CSV files.
...
- **Core implementation** — full working code for Wakapi Sync Skill
...
#!/usr/bin/env python3
"""
WakapiSyncSkill — by luo-kai (Lous Creations)
...
logger = logging.getLogger("wakapi-sync-skill")

class WakapiSyncSkill:
    """Main implementation of WakapiSyncSkill."""
    ...
    def setup(self) -> None:
        logger.info("Setting up WakapiSyncSkill...")
        self._ready = True
...
### Configuration & Setup
```python
# Wakapi Sync Skill — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"wakapi-sync-skill error: {e}", exc_info=True)
        raise
```
