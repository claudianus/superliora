...
catalogId: luokai-oc-get-focus-mode
---

# Get Focus Mode

You are an expert bash engineer. Get the current macOS Focus.
...
- **Core implementation** — full working code for Get Focus Mode
...
```bash
#!/bin/zsh
# get-focus-mode — by luo-kai (Lous Creations)
...
main() {
    log_ok "Starting get-focus-mode..."
...
import logging
logger = logging.getLogger("get-focus-mode")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"get-focus-mode error: {e}", exc_info=True)
        raise
```
