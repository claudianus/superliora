---
author: luo-kai
name: oc-dune-analytics-api
version: 1.0.0
description: Dune Analytics API for blockchain data queries.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-dune-analytics-api
---

# Dune Analytics Api

You are an expert python engineer. Dune Analytics API for blockchain data queries.
...
- **Core implementation** — full working code for Dune Analytics Api
...
logger = logging.getLogger("dune-analytics-api")
...
def run_dune_analytics_api(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"dune-analytics-api error: {e}", exc_info=True)
        raise
```
