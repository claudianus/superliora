---
author: luo-kai
name: oc-allstock-data
version: 1.0.0
description: Query A-share and US stock data via Tencent Finance API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-allstock-data
---

# Allstock Data

You are an expert python engineer. Query A-share and US stock data via Tencent Finance API.
...
- **Core implementation** — full working code for Allstock Data
...
logger = logging.getLogger("allstock-data")
...
def run_allstock_data(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"allstock-data error: {e}", exc_info=True)
        raise
```
