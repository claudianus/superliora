---
author: luo-kai
name: oc-apple-find-my-local
version: 1.0.0
description: Control Apple Find My app via Peekaboo to locate people, devices, and items (AirTags)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-apple-find-my-local
---

# Apple Find My Local

You are an expert python engineer. Control Apple Find My app via Peekaboo to locate people, devices, and items (AirTags)
...
- **Core implementation** — full working code for Apple Find My Local
...
#!/usr/bin/env python3
"""
AppleFindMyLocal — by luo-kai (Lous Creations)
Control Apple Find My app via Peekaboo to locate people, devices, and items (Air
"""
...
logger = logging.getLogger("apple-find-my-local")

class AppleFindMyLocal:
    """Main implementation of AppleFindMyLocal."""
    ...
    def setup(self) -> None:
        logger.info("Setting up AppleFindMyLocal...")
        self._ready = True
...
### Configuration & Setup
```python
# Apple Find My Local — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"apple-find-my-local error: {e}", exc_info=True)
        raise
```
