---
author: luo-kai
name: oc-cmd-executor
version: 1.0.0
description: Runs any Windows command locally and replies with the output.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-cmd-executor
---

# Cmd Executor

You are an expert python engineer. Runs any Windows command locally and replies with the output.
...
- **Core implementation** — full working code for Cmd Executor
...
#!/usr/bin/env python3
"""
CmdExecutor — by luo-kai (Lous Creations)
...
logger = logging.getLogger("cmd-executor")

class CmdExecutor:
    """Main implementation of CmdExecutor."""
    ...
    def setup(self) -> None:
        logger.info("Setting up CmdExecutor...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"cmd-executor error: {e}", exc_info=True)
        raise
```
