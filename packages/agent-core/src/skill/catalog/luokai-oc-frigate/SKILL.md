...
catalogId: luokai-oc-frigate
---

# Frigate

You are an expert python engineer. Access Frigate NVR cameras with session-based authentication.
...
- **Core implementation** — full working code for Frigate
...
class FrigateSecureInput(BaseModel):
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"frigate error: {e}", exc_info=True)
        raise
```
