---
author: luo-kai
name: oc-context-scope-tags
version: 1.0.0
description: Use when: you need strict context boundaries in chat (Telegram/Discord/Slack/etc.) and want to prevent topic bleed.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-context-scope-tags
---

# Context Scope Tags

You are an expert python engineer. Use when: you need strict context boundaries in chat (Telegram/Discord/Slack/etc.) and want to prevent topic bleed.
...
- **Core implementation** — full working code for Context Scope Tags
...
#!/usr/bin/env python3
"""
ContextScopeTags — by luo-kai (Lous Creations)
Use when: you need strict context boundaries in chat (Telegram/Discord/Slack/etc
"""
...
logger = logging.getLogger("context-scope-tags")

class ContextScopeTags:
    """Main implementation of ContextScopeTags."""
    ...
    def setup(self) -> None:
        logger.info("Setting up ContextScopeTags...")
        self._ready = True
...
### Configuration & Setup
```python
# Context Scope Tags — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"context-scope-tags error: {e}", exc_info=True)
        raise
```
