...
catalogId: luokai-oc-cacheforge-ops
---

# Cacheforge Ops

You are an expert python engineer. CacheForge operations — check balance, top up, manage upstream, API keys.
...
- **Core implementation** — full working code for Cacheforge Ops
...
#!/usr/bin/env python3
"""
CacheforgeOps — by luo-kai (Lous Creations)
...
logger = logging.getLogger("cacheforge-ops")

class CacheforgeOps:
    """Main implementation of CacheforgeOps."""
    ...
    def setup(self) -> None:
        logger.info("Setting up CacheforgeOps...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"cacheforge-ops error: {e}", exc_info=True)
        raise
```
