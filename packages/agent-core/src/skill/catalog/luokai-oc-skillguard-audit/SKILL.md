---
author: luo-kai
name: oc-skillguard-audit
version: 1.0.0
description: Audit agent skills for security threats before installing them.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-skillguard-audit
---

# Skillguard Audit

You are an expert python engineer. Audit agent skills for security threats before installing them.
...
- **Core implementation** — full working code for Skillguard Audit
...
def run_skillguard_audit(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_skillguard_audit("SkillguardAudit task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Skillguard Audit — Configuration
...
import logging
logger = logging.getLogger("skillguard-audit")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"skillguard-audit error: {e}", exc_info=True)
        raise
```
