---
author: luo-kai
name: oc-expanso-edge
version: 1.0.0
description: Data processing pipelines for OpenClaw.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-expanso-edge
---

# Expanso Edge

You are an expert python engineer. Data processing pipelines for OpenClaw.
...
- **Core implementation** — full working code for Expanso Edge
...
logger = logging.getLogger("expanso-edge")
...
def run_expanso_edge(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"expanso-edge error: {e}", exc_info=True)
        raise
```
