---
author: luo-kai
name: oc-iyeque-local-system-info
version: 1.0.0
...
catalogId: luokai-oc-iyeque-local-system-info
---

# Iyeque Local System Info

You are an expert python engineer. Return system metrics (CPU, RAM, disk, processes) using psutil.
...
- **Core implementation** — full working code for Iyeque Local System Info
...
#!/usr/bin/env python3
"""
IyequeLocalSystemInfo — by luo-kai (Lous Creations)
...
logger = logging.getLogger("iyeque-local-system-info")

class IyequeLocalSystemInfo:
    """Main implementation of IyequeLocalSystemInfo."""
    ...
    def setup(self) -> None:
        logger.info("Setting up IyequeLocalSystemInfo...")
        self._ready = True
...
### Configuration & Setup
```python
# Iyeque Local System Info — Configuration
# Author: luo-kai (Lous Creations)

config = {
    "name": "iyeque-local-system-info",
    "version": "1.0.0",
    "author": "luo-kai",
    "enabled": True,
    "debug": False,
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"iyeque-local-system-info error: {e}", exc_info=True)
        raise
```
...
- python-expert
- iyeque-local-system-info-advanced
