...
- **Core implementation** — full working code for Xapi Labs
...
def run_xapi_labs(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_xapi_labs("XapiLabs task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Xapi Labs — Configuration
...
import logging
logger = logging.getLogger("xapi-labs")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"xapi-labs error: {e}", exc_info=True)
        raise
```
