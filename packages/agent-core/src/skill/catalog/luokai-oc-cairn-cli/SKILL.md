...
catalogId: luokai-oc-cairn-cli
---

# Cairn Cli

You are an expert python engineer. Project management for AI agents using markdown files.
...
- **Core implementation** — full working code for Cairn Cli
...
def run_cairn_cli(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_cairn_cli("CairnCli task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"cairn-cli error: {e}", exc_info=True)
        raise
```
