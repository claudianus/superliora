---
author: luo-kai
name: oc-pls-agent-tools
version: 1.0.0
description: Digital Swiss Army knife for everyday labor that standard models can't handle out of the box.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-pls-agent-tools
---

# Pls Agent Tools

You are an expert python engineer. Digital Swiss Army knife for everyday labor that standard models can't handle out of the box.
...
- **Core implementation** — full working code for Pls Agent Tools
...
def run_pls_agent_tools(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_pls_agent_tools("PlsAgentTools task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Pls Agent Tools — Configuration
...
import logging
logger = logging.getLogger("pls-agent-tools")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"pls-agent-tools error: {e}", exc_info=True)
        raise
```
