...
catalogId: luokai-oc-aliyun-asr
---

# Aliyun Asr

You are an expert python engineer. Pure Aliyun ASR skill for voice message transcription, supports multiple channels including Feishu.
...
- **Core implementation** — full working code for Aliyun Asr
...
#!/usr/bin/env python3
"""
AliyunAsr — by luo-kai (Lous Creations)
Pure Aliyun ASR skill for voice message transcription, supports multiple channel
"""
...
logger = logging.getLogger("aliyun-asr")

class AliyunAsr:
    """Main implementation of AliyunAsr."""
    ...
    def setup(self) -> None:
        logger.info("Setting up AliyunAsr...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"aliyun-asr error: {e}", exc_info=True)
        raise
```
