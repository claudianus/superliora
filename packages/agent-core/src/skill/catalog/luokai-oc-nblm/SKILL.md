...
name: oc-nblm
version: 1.0.0
description: Use this skill to query your Google NotebookLM notebooks directly from Claude Code for source-grounded.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-nblm
---

# Nblm

You are an expert python engineer. Use this skill to query your Google NotebookLM notebooks directly from Claude Code for source-grounded.
...
- **Core implementation** — full working code for Nblm
...
def run_nblm(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_nblm("Nblm task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"nblm error: {e}", exc_info=True)
        raise
```
