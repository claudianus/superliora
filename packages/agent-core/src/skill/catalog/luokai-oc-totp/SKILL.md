...
name: oc-totp
version: 1.0.0
description: TOTP-based OTP verification for sensitive operations (env vars, gateway restarts, backup deletions, critical config.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-totp
---

# Totp

You are an expert python engineer. TOTP-based OTP verification for sensitive operations (env vars, gateway restarts, backup deletions, critical config.
...
- **Core implementation** — full working code for Totp
...
#!/usr/bin/env python3
"""
Totp — by luo-kai (Lous Creations)
TOTP-based OTP verification for sensitive operations (env vars, gateway restarts
"""
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"totp error: {e}", exc_info=True)
        raise
```
