...
name: oc-callmac
version: 1.0.0
description: Remote voice control for Mac from mobile devices using commands like /callmac.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-callmac
---

# Callmac

You are an expert python engineer. Remote voice control for Mac from mobile devices using commands like /callmac.
...
- **Core implementation** — full working code for Callmac
...
def run_callmac(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_callmac("Callmac task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Callmac — Configuration
...
import logging
logger = logging.getLogger("callmac")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"callmac error: {e}", exc_info=True)
        raise
```
