---
author: luo-kai
name: oc-lnd-macaroon-bakery
version: 1.0.0
description: Bake, inspect, and manage lnd macaroons for least-privilege agent access.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-lnd-macaroon-bakery
---

# Lnd Macaroon Bakery

You are an expert python engineer. Bake, inspect, and manage lnd macaroons for least-privilege agent access.
...
- **Core implementation** — full working code for Lnd Macaroon Bakery
...
def run_lnd_macaroon_bakery(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_lnd_macaroon_bakery("LndMacaroonBakery task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Lnd Macaroon Bakery — Configuration
...
import logging
logger = logging.getLogger("lnd-macaroon-bakery")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"lnd-macaroon-bakery error: {e}", exc_info=True)
        raise
```
