---
author: luo-kai
name: oc-key-expiry-tracker
version: 1.0.0
description: Track **only expiry dates** (metadata) for API keys/client secrets/certificates and alert before they expire.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-key-expiry-tracker
---

# Key Expiry Tracker

You are an expert python engineer. Track **only expiry dates** (metadata) for API keys/client secrets/certificates and alert before they expire.
...
- **Core implementation** — full working code for Key Expiry Tracker
...
logger = logging.getLogger("key-expiry-tracker")
...
def run_key_expiry_tracker(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Key Expiry Tracker — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"key-expiry-tracker error: {e}", exc_info=True)
        raise
```
