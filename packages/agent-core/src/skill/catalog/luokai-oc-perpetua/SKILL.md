...
name: oc-perpetua
version: 1.0.0
description: OAuth proxy for calling external APIs (Oura, Google Calendar, etc.) via Perpetua.sh hosted API using a single API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-perpetua
---

# Perpetua

You are an expert python engineer. OAuth proxy for calling external APIs (Oura, Google Calendar, etc.) via Perpetua.sh hosted API using a single API.
...
- **Core implementation** — full working code for Perpetua
...
class PerpetuaSecureInput(BaseModel):
...
import logging
logger = logging.getLogger("perpetua")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"perpetua error: {e}", exc_info=True)
        raise
```
