...
name: oc-aa
version: 1.0.0
description: This skill enables the agent to **automatically answer Gmail messages on behalf of a client**.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-aa
---

# Aa

You are an expert python engineer. This skill enables the agent to **automatically answer Gmail messages on behalf of a client**.
...
- **Core implementation** — full working code for Aa
...
def run_aa(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_aa("Aa task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"aa error: {e}", exc_info=True)
        raise
```
