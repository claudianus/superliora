...
catalogId: luokai-oc-agent-mail-cli
---

# Agent Mail Cli

You are an expert python engineer. Email inbox for AI agents.
...
- **Core implementation** — full working code for Agent Mail Cli
...
def run_agent_mail_cli(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_mail_cli("AgentMailCli task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("agent-mail-cli")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-mail-cli error: {e}", exc_info=True)
        raise
```
