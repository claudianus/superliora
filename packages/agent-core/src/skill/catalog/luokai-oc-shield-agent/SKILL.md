---
author: luo-kai
name: oc-shield-agent
version: 1.0.0
description: Autonomous DeFi smart contract vulnerability scanner.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-shield-agent
---

# Shield Agent

You are an expert python engineer. Autonomous DeFi smart contract vulnerability scanner.
...
- **Core implementation** — full working code for Shield Agent
...
def run_shield_agent(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_shield_agent("ShieldAgent task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Shield Agent — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"shield-agent error: {e}", exc_info=True)
        raise
```
