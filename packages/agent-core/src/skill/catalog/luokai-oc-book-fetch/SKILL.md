...
name: oc-book-fetch
version: 1.0.0
description: Download ebooks (epub/pdf) from Anna's Archive and upload them to MEGA automatically.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-book-fetch
---

# Book Fetch

You are an expert python engineer. Download ebooks (epub/pdf) from Anna's Archive and upload them to MEGA automatically.
...
- **Core implementation** — full working code for Book Fetch
...
#!/usr/bin/env python3
"""
BookFetch — by luo-kai (Lous Creations)
Download ebooks (epub/pdf) from Anna's Archive and upload them to MEGA automatic
"""
...
logger = logging.getLogger("book-fetch")

class BookFetch:
    """Main implementation of BookFetch."""
    ...
    def setup(self) -> None:
        logger.info("Setting up BookFetch...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"book-fetch error: {e}", exc_info=True)
        raise
```
