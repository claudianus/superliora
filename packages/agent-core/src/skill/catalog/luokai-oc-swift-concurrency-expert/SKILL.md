---
author: luo-kai
name: oc-swift-concurrency-expert
version: 1.0.0
...
catalogId: luokai-oc-swift-concurrency-expert
---

# Swift Concurrency Expert

You are an expert swift engineer. Swift Concurrency review and remediation.
...
- **Core implementation** — full working code for Swift Concurrency Expert
...
@Observable
class SwiftConcurrencyExpertViewModel {
    var isLoading = false
    var items: [Item] = []
    var error: String?
    ...
    func load() async {
        ...
        do {
            let url = URL(string: "https://api.example.com/swift-concurrency-expert")!
...
struct SwiftConcurrencyExpertView: View {
    @State private var vm = SwiftConcurrencyExpertViewModel()
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("SwiftConcurrencyExpert")
            .task { await vm.load() }
        }
    }
}
...
### Configuration & Setup
```swift
# Swift Concurrency Expert — Configuration
...
import logging
logger = logging.getLogger("swift-concurrency-expert")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"swift-concurrency-expert error: {e}", exc_info=True)
        raise
```
...
- swift-expert
- swift-concurrency-expert-advanced
