---
author: luo-kai
name: oc-swiftfindrefs
version: 1.0.0
description: Use swiftfindrefs (IndexStoreDB) to list every Swift source.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-swiftfindrefs
---

# Swiftfindrefs

You are an expert swift engineer. Use swiftfindrefs (IndexStoreDB) to list every Swift source.
...
- **Core implementation** — full working code for Swiftfindrefs
...
class SwiftfindrefsViewModel {
    ...
    func load() async {
        ...
        do {
            let url = URL(string: "https://api.example.com/swiftfindrefs")!
...
struct SwiftfindrefsView: View {
    @State private var vm = SwiftfindrefsViewModel()
...
import logging
logger = logging.getLogger("swiftfindrefs")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"swiftfindrefs error: {e}", exc_info=True)
        raise
```
