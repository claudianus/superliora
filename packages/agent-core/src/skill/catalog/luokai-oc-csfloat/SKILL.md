...
catalogId: luokai-oc-csfloat
---

# Csfloat

You are an expert python engineer. Queries csfloat.com for data on skins.
...
- **Core implementation** — full working code for Csfloat
...
def run_csfloat(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"csfloat error: {e}", exc_info=True)
        raise
```
