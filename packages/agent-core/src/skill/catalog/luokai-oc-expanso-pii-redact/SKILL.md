...
catalogId: luokai-oc-expanso-pii-redact
---

# Expanso Pii Redact

You are an expert python engineer. "Redact PII from text, replacing sensitive data with placeholders".
...
- **Core implementation** — full working code for Expanso Pii Redact
...
logger = logging.getLogger("expanso-pii-redact")
...
def run_expanso_pii_redact(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Expanso Pii Redact — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"expanso-pii-redact error: {e}", exc_info=True)
        raise
```
