---
author: luo-kai
name: oc-free-a-share-real-time-data
version: 1.0.0
description: Fetch China A-share stock market data (bars, realtime quotes, tick-by-tick transactions) via mootdx/TDX protocol.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-free-a-share-real-time-data
---

# Free A Share Real Time Data

You are an expert python engineer. Fetch China A-share stock market data (bars, realtime quotes, tick-by-tick transactions) via mootdx/TDX protocol.
...
- **Core implementation** — full working code for Free A Share Real Time Data
...
logger = logging.getLogger("free-a-share-real-time-data")
...
def run_free_a_share_real_time_data(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Free A Share Real Time Data — Configuration
# Author: luo-kai (Lous Creations)

config = {
    "name": "free-a-share-real-time-data",
    "version": "1.0.0",
    "author": "luo-kai",
    "enabled": True,
    "debug": False,
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"free-a-share-real-time-data error: {e}", exc_info=True)
        raise
```
...
- python-expert
- free-a-share-real-time-data-advanced
