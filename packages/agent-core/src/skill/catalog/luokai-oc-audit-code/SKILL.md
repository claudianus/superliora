...
catalogId: luokai-oc-audit-code
---

# Audit Code

You are an expert python engineer. Security-focused code review for hardcoded secrets, dangerous calls, and common vulnerabilities.
...
- **Core implementation** — full working code for Audit Code
...
class AuditCodeSecureInput(BaseModel):
...
import logging
logger = logging.getLogger("audit-code")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"audit-code error: {e}", exc_info=True)
        raise
```
