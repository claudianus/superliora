...
catalogId: luokai-oc-shellmail
---

# Shellmail

You are an expert python engineer. Email API for AI agents.
...
- **Core implementation** — full working code for Shellmail
...
def run_shellmail(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_shellmail("Shellmail task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Shellmail — Configuration
...
import logging
logger = logging.getLogger("shellmail")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"shellmail error: {e}", exc_info=True)
        raise
```
