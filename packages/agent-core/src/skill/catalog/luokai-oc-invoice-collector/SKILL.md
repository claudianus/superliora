---
author: luo-kai
name: oc-invoice-collector
version: 1.0.0
description: Collect invoices/receipts from Gmail and send a summary email with attachments.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-invoice-collector
---

# Invoice Collector

You are an expert python engineer. Collect invoices/receipts from Gmail and send a summary email with attachments.
...
- **Core implementation** — full working code for Invoice Collector
...
#!/usr/bin/env python3
"""
InvoiceCollector — by luo-kai (Lous Creations)
...
logger = logging.getLogger("invoice-collector")

class InvoiceCollector:
    """Main implementation of InvoiceCollector."""
    ...
    def setup(self) -> None:
        logger.info("Setting up InvoiceCollector...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"invoice-collector error: {e}", exc_info=True)
        raise
```
