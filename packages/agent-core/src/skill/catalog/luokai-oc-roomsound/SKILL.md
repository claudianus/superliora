...
name: oc-roomsound
version: 1.0.0
description: RoomSound gives your agent the skill to play audio to your speakers.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-roomsound
---

# Roomsound

You are an expert python engineer. RoomSound gives your agent the skill to play audio to your speakers.
...
- **Core implementation** — full working code for Roomsound
...
def run_roomsound(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_roomsound("Roomsound task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Roomsound — Configuration
...
import logging
logger = logging.getLogger("roomsound")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"roomsound error: {e}", exc_info=True)
        raise
```
