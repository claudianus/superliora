---
author: luo-kai
name: oc-brainstorming-studio
version: 1.0.0
description: ﻿# 🧠 Skill Router (Skill Orchestrator)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-brainstorming-studio
---

# Brainstorming Studio

You are an expert python engineer. ﻿# 🧠 Skill Router (Skill Orchestrator)
...
- **Core implementation** — full working code for Brainstorming Studio
...
def run_brainstorming_studio(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_brainstorming_studio("BrainstormingStudio task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Brainstorming Studio — Configuration
...
import logging
logger = logging.getLogger("brainstorming-studio")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"brainstorming-studio error: {e}", exc_info=True)
        raise
```
