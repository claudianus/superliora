---
author: luo-kai
name: oc-open-chrome-tabs
version: 1.0.0
description: Read currently open browser tabs from Chrome or other Chromium browsers (Arc, Brave, Edge, etc.)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-open-chrome-tabs
---

# Open Chrome Tabs

You are an expert python engineer. Read currently open browser tabs from Chrome or other Chromium browsers (Arc, Brave, Edge, etc.)
...
- **Core implementation** — full working code for Open Chrome Tabs
...
#!/usr/bin/env python3
"""
OpenChromeTabs — by luo-kai (Lous Creations)
Read currently open browser tabs from Chrome or other Chromium browsers (Arc, Br
"""
...
logger = logging.getLogger("open-chrome-tabs")

class OpenChromeTabs:
    """Main implementation of OpenChromeTabs."""
    ...
    def setup(self) -> None:
        logger.info("Setting up OpenChromeTabs...")
        self._ready = True
...
### Configuration & Setup
```python
# Open Chrome Tabs — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"open-chrome-tabs error: {e}", exc_info=True)
        raise
```
