...
catalogId: luokai-oc-apple-notes
---

# Apple Notes

You are an expert bash engineer. Manage Apple Notes via the `memo` CLI on macOS.
...
- **Core implementation** — full working code for Apple Notes
...
```bash
#!/bin/zsh
# apple-notes — by luo-kai (Lous Creations)
...
import logging
logger = logging.getLogger("apple-notes")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"apple-notes error: {e}", exc_info=True)
        raise
```
