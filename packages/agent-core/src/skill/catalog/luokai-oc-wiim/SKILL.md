...
name: oc-wiim
version: 1.0.0
description: Control WiiM audio devices (play, pause, stop, next, prev, volume, mute, play URLs, presets)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-wiim
---

# Wiim

You are an expert typescript engineer. Control WiiM audio devices (play, pause, stop, next, prev, volume, mute, play URLs, presets)
...
- **Core implementation** — full working code for Wiim
...
export const Wiim = memo(function Wiim({ className, onSelect }: WiimProps) {
    const [selected, setSelected] = useState<string | null>(null);

    const { data, isLoading, error } = useQuery({
        queryKey: ['wiim'],
        queryFn: () => fetch('/api/wiim').then((r) => r.json()),
        staleTime: 30_000,
    });
...
import logging
logger = logging.getLogger("wiim")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"wiim error: {e}", exc_info=True)
        raise
```
