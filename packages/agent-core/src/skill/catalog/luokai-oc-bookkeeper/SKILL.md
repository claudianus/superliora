...
name: oc-bookkeeper
version: 1.0.0
description: Meta-skill for pre-accounting automation by orchestrating gmail, deepread-ocr, stripe-api, and xero.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-bookkeeper
---

# Bookkeeper

You are an expert python engineer. Meta-skill for pre-accounting automation by orchestrating gmail, deepread-ocr, stripe-api, and xero.
...
- **Core implementation** — full working code for Bookkeeper
...
def run_bookkeeper(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_bookkeeper("Bookkeeper task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"bookkeeper error: {e}", exc_info=True)
        raise
```
