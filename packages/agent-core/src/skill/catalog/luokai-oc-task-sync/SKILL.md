...
name: oc-task-sync
version: 1.0.0
description: Synchronize TickTick (Dida) and Google Tasks bidirectionally, including list/project mapping, task content sync.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-task-sync
---

# Task Sync

You are an expert python engineer. Synchronize TickTick (Dida) and Google Tasks bidirectionally, including list/project mapping, task content sync.
...
- **Core implementation** — full working code for Task Sync
...
#!/usr/bin/env python3
"""
TaskSync — by luo-kai (Lous Creations)
Synchronize TickTick (Dida) and Google Tasks bidirectionally, including list/pro
"""
...
logger = logging.getLogger("task-sync")

class TaskSync:
    """Main implementation of TaskSync."""
    ...
    def setup(self) -> None:
        logger.info("Setting up TaskSync...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"task-sync error: {e}", exc_info=True)
        raise
```
