...
catalogId: luokai-oc-find-slills
---

# Find Slills

You are an expert python engineer. Helps users discover and install agent skills when they ask questions like "how do I do X", "find a skill for X".
...
- **Core implementation** — full working code for Find Slills
...
def run_find_slills(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_find_slills("FindSlills task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Find Slills — Configuration
...
import logging
logger = logging.getLogger("find-slills")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"find-slills error: {e}", exc_info=True)
        raise
```
