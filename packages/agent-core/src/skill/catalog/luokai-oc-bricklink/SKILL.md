...
name: oc-bricklink
version: 1.0.0
description: BrickLink Store API helper/CLI (OAuth 1.0 request signing).
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-bricklink
---

# Bricklink

You are an expert python engineer. BrickLink Store API helper/CLI (OAuth 1.0 request signing).
...
- **Core implementation** — full working code for Bricklink
...
class BricklinkSecureInput(BaseModel):
...
import logging
logger = logging.getLogger("bricklink")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"bricklink error: {e}", exc_info=True)
        raise
```
