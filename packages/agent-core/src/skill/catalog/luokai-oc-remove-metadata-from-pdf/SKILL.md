---
author: luo-kai
name: oc-remove-metadata-from-pdf
version: 1.0.0
description: Remove metadata from one or multiple PDFs by uploading them to the Solutions API, polling until completion.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-remove-metadata-from-pdf
---

# Remove Metadata From Pdf

You are an expert python engineer. Remove metadata from one or multiple PDFs by uploading them to the Solutions API, polling until completion.
...
- **Core implementation** — full working code for Remove Metadata From Pdf
...
logger = logging.getLogger("remove-metadata-from-pdf")
...
def run_remove_metadata_from_pdf(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Remove Metadata From Pdf — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"remove-metadata-from-pdf error: {e}", exc_info=True)
        raise
```
...
- python-expert
- remove-metadata-from-pdf-advanced
