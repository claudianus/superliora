...
catalogId: luokai-oc-aviation-weather
---

# Aviation Weather

You are an expert python engineer. Fetch aviation weather data (METAR, TAF, PIREPs)
...
- **Core implementation** — full working code for Aviation Weather
...
def run_aviation_weather(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"aviation-weather error: {e}", exc_info=True)
        raise
```
