---
author: luo-kai
name: oc-ned-analytics
version: 1.0.0
description: Query your Shopify store's sales, profitability, customers, and marketing data through Ned's API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-ned-analytics
---

# Ned Analytics

You are an expert python engineer. Query your Shopify store's sales, profitability, customers, and marketing data through Ned's API.
...
- **Core implementation** — full working code for Ned Analytics
...
logger = logging.getLogger("ned-analytics")
...
def run_ned_analytics(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"ned-analytics error: {e}", exc_info=True)
        raise
```
