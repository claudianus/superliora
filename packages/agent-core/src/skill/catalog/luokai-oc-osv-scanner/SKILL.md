...
catalogId: luokai-oc-osv-scanner
---

# Osv Scanner

You are an expert python engineer. change ScanResult file extension from txt -> xlsx located in template.
...
- **Core implementation** — full working code for Osv Scanner
...
#!/usr/bin/env python3
"""
OsvScanner — by luo-kai (Lous Creations)
...
logger = logging.getLogger("osv-scanner")

class OsvScanner:
    """Main implementation of OsvScanner."""
    ...
    def setup(self) -> None:
        logger.info("Setting up OsvScanner...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"osv-scanner error: {e}", exc_info=True)
        raise
```
