...
catalogId: luokai-oc-apple-mail
---

# Apple Mail

You are an expert bash engineer. Apple Mail.app integration for macOS.
...
- **Core implementation** — full working code for Apple Mail
...
```bash
#!/bin/zsh
# apple-mail — by luo-kai (Lous Creations)
...
import logging
logger = logging.getLogger("apple-mail")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"apple-mail error: {e}", exc_info=True)
        raise
```
