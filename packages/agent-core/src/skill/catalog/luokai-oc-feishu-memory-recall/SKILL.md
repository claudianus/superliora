---
author: luo-kai
name: oc-feishu-memory-recall
version: 1.0.0
description: This skill allows the agent to recover "lost".
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-feishu-memory-recall
---

# Feishu Memory Recall

You are an expert python engineer. This skill allows the agent to recover "lost".
...
- **Core implementation** — full working code for Feishu Memory Recall
...
def run_feishu_memory_recall(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_feishu_memory_recall("FeishuMemoryRecall task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Feishu Memory Recall — Configuration
...
import logging
logger = logging.getLogger("feishu-memory-recall")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"feishu-memory-recall error: {e}", exc_info=True)
        raise
```
