---
author: luo-kai
name: oc-expanso-text-summarize
version: 1.0.0
...
catalogId: luokai-oc-expanso-text-summarize
---

# Expanso Text Summarize

You are an expert python engineer. Summarize text into 3-5 bullet points using AI.
...
- **Core implementation** — full working code for Expanso Text Summarize
...
#!/usr/bin/env python3
"""
ExpansoTextSummarize — by luo-kai (Lous Creations)
...
logger = logging.getLogger("expanso-text-summarize")

class ExpansoTextSummarize:
    """Main implementation of ExpansoTextSummarize."""
    ...
    def setup(self) -> None:
        logger.info("Setting up ExpansoTextSummarize...")
        self._ready = True
...
### Configuration & Setup
```python
# Expanso Text Summarize — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"expanso-text-summarize error: {e}", exc_info=True)
        raise
```
...
- python-expert
- expanso-text-summarize-advanced
