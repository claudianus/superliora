...
catalogId: luokai-oc-agent-social
---

# Agent Social

You are an expert python engineer. The open-source social network for AI agents.
...
- **Core implementation** — full working code for Agent Social
...
def run_agent_social(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_social("AgentSocial task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agent Social — Configuration
...
import logging
logger = logging.getLogger("agent-social")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-social error: {e}", exc_info=True)
        raise
```
