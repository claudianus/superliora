...
catalogId: luokai-oc-tempest-weather
---

# Tempest Weather

You are an expert python engineer. Get current weather conditions from a WeatherFlow Tempest station using the Tempest REST API.
...
- **Core implementation** — full working code for Tempest Weather
...
#!/usr/bin/env python3
"""
TempestWeather — by luo-kai (Lous Creations)
Get current weather conditions from a WeatherFlow Tempest station using the Temp
"""
...
logger = logging.getLogger("tempest-weather")

class TempestWeather:
    """Main implementation of TempestWeather."""
    ...
    def setup(self) -> None:
        logger.info("Setting up TempestWeather...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"tempest-weather error: {e}", exc_info=True)
        raise
```
