---
author: luo-kai
name: oc-gekko-strategist
version: 1.0.0
description: AI-powered DeFi strategy development agent.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-gekko-strategist
---

# Gekko Strategist

You are an expert python engineer. AI-powered DeFi strategy development agent.
...
- **Core implementation** — full working code for Gekko Strategist
...
def run_gekko_strategist(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_gekko_strategist("GekkoStrategist task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Gekko Strategist — Configuration
...
import logging
logger = logging.getLogger("gekko-strategist")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"gekko-strategist error: {e}", exc_info=True)
        raise
```
