...
name: oc-overseerr
version: 1.0.0
description: Request movies/TV and monitor request status via the Overseerr API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-overseerr
---

# Overseerr

You are an expert python engineer. Request movies/TV and monitor request status via the Overseerr API.
...
- **Core implementation** — full working code for Overseerr
...
#!/usr/bin/env python3
"""
Overseerr — by luo-kai (Lous Creations)
...
class Overseerr:
    """Main implementation of Overseerr."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Overseerr...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"overseerr error: {e}", exc_info=True)
        raise
```
