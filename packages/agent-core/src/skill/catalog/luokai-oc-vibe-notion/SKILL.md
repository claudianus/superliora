...
catalogId: luokai-oc-vibe-notion
---

# Vibe Notion

You are an expert python engineer. Interact with Notion using the unofficial private API - pages, databases, blocks, search, users, comments.
...
- **Core implementation** — full working code for Vibe Notion
...
logger = logging.getLogger("vibe-notion")
...
def run_vibe_notion(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"vibe-notion error: {e}", exc_info=True)
        raise
```
