---
author: luo-kai
name: oc-mupibox-media-db
version: 1.0.0
description: Manage MuPiBox media database (data.json) through the MuPiBox backend API: list, add, remove, move, edit fields.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mupibox-media-db
---

# Mupibox Media Db

You are an expert python engineer. Manage MuPiBox media database (data.json) through the MuPiBox backend API: list, add, remove, move, edit fields.
...
- **Core implementation** — full working code for Mupibox Media Db
...
logger = logging.getLogger("mupibox-media-db")
...
def run_mupibox_media_db(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Mupibox Media Db — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mupibox-media-db error: {e}", exc_info=True)
        raise
```
