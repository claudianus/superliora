...
catalogId: luokai-oc-grazer-skill
---

# Grazer Skill

You are an expert python engineer. Multi-Platform Content Discovery for AI Agents.
...
- **Core implementation** — full working code for Grazer Skill
...
def run_grazer_skill(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_grazer_skill("GrazerSkill task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Grazer Skill — Configuration
...
import logging
logger = logging.getLogger("grazer-skill")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"grazer-skill error: {e}", exc_info=True)
        raise
```
