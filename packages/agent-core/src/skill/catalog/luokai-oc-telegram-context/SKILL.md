---
author: luo-kai
name: oc-telegram-context
version: 1.0.0
description: Toggle-enabled skill that fetches Telegram message history at session start for conversational continuity.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-telegram-context
---

# Telegram Context

You are an expert python engineer. Toggle-enabled skill that fetches Telegram message history at session start for conversational continuity.
...
- **Core implementation** — full working code for Telegram Context
...
#!/usr/bin/env python3
"""
TelegramContext — by luo-kai (Lous Creations)
Toggle-enabled skill that fetches Telegram message history at session start for 
"""
...
logger = logging.getLogger("telegram-context")

class TelegramContext:
    """Main implementation of TelegramContext."""
    ...
    def setup(self) -> None:
        logger.info("Setting up TelegramContext...")
        self._ready = True
...
### Configuration & Setup
```python
# Telegram Context — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"telegram-context error: {e}", exc_info=True)
        raise
```
