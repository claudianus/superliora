...
name: oc-lel-mail
version: 1.0.0
description: Send and read email via a combination of python and bash scripts which makes use of the main agent for reasoning.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-lel-mail
---

# Lel Mail

You are an expert python engineer. Send and read email via a combination of python and bash scripts which makes use of the main agent for reasoning.
...
- **Core implementation** — full working code for Lel Mail
...
def run_lel_mail(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_lel_mail("LelMail task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Lel Mail — Configuration
...
import logging
logger = logging.getLogger("lel-mail")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"lel-mail error: {e}", exc_info=True)
        raise
```
