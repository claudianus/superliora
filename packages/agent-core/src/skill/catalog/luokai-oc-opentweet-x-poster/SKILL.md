...
catalogId: luokai-oc-opentweet-x-poster
---

# Opentweet X Poster

You are an expert python engineer. Post to X (Twitter) using the OpenTweet API.
...
- **Core implementation** — full working code for Opentweet X Poster
...
#!/usr/bin/env python3
"""
OpentweetXPoster — by luo-kai (Lous Creations)
Post to X (Twitter) using the OpenTweet API.
"""
...
logger = logging.getLogger("opentweet-x-poster")

class OpentweetXPoster:
    """Main implementation of OpentweetXPoster."""
    ...
    def setup(self) -> None:
        logger.info("Setting up OpentweetXPoster...")
        self._ready = True
...
### Configuration & Setup
```python
# Opentweet X Poster — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"opentweet-x-poster error: {e}", exc_info=True)
        raise
```
