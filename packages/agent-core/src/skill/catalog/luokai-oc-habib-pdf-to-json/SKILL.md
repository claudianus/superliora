---
author: luo-kai
name: oc-habib-pdf-to-json
version: 1.0.0
description: Extract structured data from construction PDFs.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-habib-pdf-to-json
---

# Habib Pdf To Json

You are an expert python engineer. Extract structured data from construction PDFs.
...
- **Core implementation** — full working code for Habib Pdf To Json
...
logger = logging.getLogger("habib-pdf-to-json")
...
def run_habib_pdf_to_json(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Habib Pdf To Json — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"habib-pdf-to-json error: {e}", exc_info=True)
        raise
```
