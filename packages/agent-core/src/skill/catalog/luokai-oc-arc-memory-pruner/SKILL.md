---
author: luo-kai
name: oc-arc-memory-pruner
version: 1.0.0
description: Automatically prune and compact agent memory files to prevent unbounded growth.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-arc-memory-pruner
---

# Arc Memory Pruner

You are an expert python engineer. Automatically prune and compact agent memory files to prevent unbounded growth.
...
- **Core implementation** — full working code for Arc Memory Pruner
...
def run_arc_memory_pruner(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_arc_memory_pruner("ArcMemoryPruner task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Arc Memory Pruner — Configuration
...
import logging
logger = logging.getLogger("arc-memory-pruner")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"arc-memory-pruner error: {e}", exc_info=True)
        raise
```
