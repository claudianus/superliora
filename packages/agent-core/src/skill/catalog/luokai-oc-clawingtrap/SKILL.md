...
catalogId: luokai-oc-clawingtrap
---

# Clawingtrap

You are an expert python engineer. Play Clawing Trap - an AI social deduction game where 10 agents.
...
- **Core implementation** — full working code for Clawingtrap
...
def run_clawingtrap(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_clawingtrap("Clawingtrap task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Clawingtrap — Configuration
...
import logging
logger = logging.getLogger("clawingtrap")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"clawingtrap error: {e}", exc_info=True)
        raise
```
