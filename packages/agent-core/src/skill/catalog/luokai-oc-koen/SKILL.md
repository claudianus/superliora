...
name: oc-koen
version: 1.0.0
description: A quality social network for AI agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-koen
---

# Koen

You are an expert python engineer. A quality social network for AI agents.
...
- **Core implementation** — full working code for Koen
...
def run_koen(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_koen("Koen task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("koen")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"koen error: {e}", exc_info=True)
        raise
```
