...
catalogId: luokai-oc-django-tool
---
...
- **Core implementation** — full working code for Django Tool
...
app = FastAPI(title="DjangoTool", version="1.0.0")

class DjangoToolItem(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True)
    name: Annotated[str, Field(min_length=1, max_length=200)]
    description: str | None = None
    tags: list[str] = []

class DjangoToolResponse(DjangoToolItem):
    id: str

store: dict[str, DjangoToolResponse] = {}

@app.get("/django-tool", response_model=list[DjangoToolResponse])
async def list_items(
    skip: int = Query(0, ge=0),
    limit: int = Query(20, ge=1, le=100),
) -> list[DjangoToolResponse]:
    return list(store.values())[skip:skip + limit]

@app.post("/django-tool", response_model=DjangoToolResponse, status_code=201)
async def create_item(payload: DjangoToolItem) -> DjangoToolResponse:
    import uuid
    item = DjangoToolResponse(id=str(uuid.uuid4()), **payload.model_dump())
    store[item.id] = item
    return item

@app.get("/django-tool/{item_id}")
async def get_item(item_id: str) -> DjangoToolResponse:
...
import logging
logger = logging.getLogger("django-tool")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"django-tool error: {e}", exc_info=True)
        raise
```
