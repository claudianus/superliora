...
catalogId: luokai-oc-braindb
---

# Braindb

You are an expert python engineer. Persistent, semantic memory for AI agents.
...
- **Core implementation** — full working code for Braindb
...
def run_braindb(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_braindb("Braindb task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Braindb — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"braindb error: {e}", exc_info=True)
        raise
```
