---
author: luo-kai
name: oc-paper-notion-summarizer
version: 1.0.0
description: Fetch paper metadata by title or arXiv/DOI link, create a deep structured summary, and post it as a Notion page.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-paper-notion-summarizer
---

# Paper Notion Summarizer

You are an expert python engineer. Fetch paper metadata by title or arXiv/DOI link, create a deep structured summary, and post it as a Notion page.
...
- **Core implementation** — full working code for Paper Notion Summarizer
...
logger = logging.getLogger("paper-notion-summarizer")
...
def run_paper_notion_summarizer(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Paper Notion Summarizer — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"paper-notion-summarizer error: {e}", exc_info=True)
        raise
```
...
- python-expert
- paper-notion-summarizer-advanced
