...
catalogId: luokai-oc-startup-agent
---
...
- **Core implementation** — full working code for Startup Agent
...
def run_startup_agent(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_startup_agent("StartupAgent task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"startup-agent error: {e}", exc_info=True)
        raise
```
