...
catalogId: luokai-oc-anime-lookup
---

# Anime Lookup

You are an expert python engineer. CLI for AI agents to search and lookup anime info for their humans.
...
- **Core implementation** — full working code for Anime Lookup
...
def run_anime_lookup(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_anime_lookup("AnimeLookup task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Anime Lookup — Configuration
...
import logging
logger = logging.getLogger("anime-lookup")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"anime-lookup error: {e}", exc_info=True)
        raise
```
