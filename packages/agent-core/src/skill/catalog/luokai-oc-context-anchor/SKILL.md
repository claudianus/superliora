...
catalogId: luokai-oc-context-anchor
---

# Context Anchor

You are an expert python engineer. Recover from context compaction by scanning memory files.
...
- **Core implementation** — full working code for Context Anchor
...
#!/usr/bin/env python3
"""
ContextAnchor — by luo-kai (Lous Creations)
...
logger = logging.getLogger("context-anchor")

class ContextAnchor:
    """Main implementation of ContextAnchor."""
    ...
    def setup(self) -> None:
        logger.info("Setting up ContextAnchor...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"context-anchor error: {e}", exc_info=True)
        raise
```
