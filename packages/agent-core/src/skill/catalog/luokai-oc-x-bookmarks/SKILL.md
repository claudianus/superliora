...
name: oc-x-bookmarks
version: 1.0.0
description: Fetch, summarize, and manage X/Twitter bookmarks via bird CLI or X API v2.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-x-bookmarks
---

# X Bookmarks

You are an expert python engineer. Fetch, summarize, and manage X/Twitter bookmarks via bird CLI or X API v2.
...
- **Core implementation** — full working code for X Bookmarks
...
#!/usr/bin/env python3
"""
XBookmarks — by luo-kai (Lous Creations)
...
logger = logging.getLogger("x-bookmarks")

class XBookmarks:
    """Main implementation of XBookmarks."""
    ...
    def setup(self) -> None:
        logger.info("Setting up XBookmarks...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"x-bookmarks error: {e}", exc_info=True)
        raise
```
