---
name: oc-iyeque-audio-processing
version: 1.0.0
description: Audio ingestion, analysis, transformation, and generation (Transcribe, TTS, VAD, Features).
author: luo-kai
tags: [iyeque-audio-processing, openclaw, community]
source: https://clawskills.sh/skills/iyeque-audio-processing
catalogSource: luokai
catalogId: luokai-oc-iyeque-audio-processing
---

# Iyeque Audio Processing

## Overview
Audio ingestion, analysis, transformation, and generation (Transcribe, TTS, VAD, Features).

## Before Starting
1. What specific task do you need this skill for?
2. What inputs are available?
3. What is the expected output format?

## Usage
This skill integrates with the OpenClaw/ClawHub ecosystem.
Install via: `npx clawhub install iyeque-audio-processing`

## Core Functionality
Refer to the original skill documentation at: https://clawskills.sh/skills/iyeque-audio-processing

This skill enables: Audio ingestion, analysis, transformation, and generation (Transcribe, TTS, VAD, Features).

## Best Practices
- Always test in a sandbox environment first
- Check the skill's latest version before deploying
- Review the source documentation for advanced configuration

## Related Skills
- agent-installer
- clawhub-publisher
- openclaw-setup-expert
