...
catalogId: luokai-oc-vet-repo
---

# Vet Repo

You are an expert python engineer. Scan repository agent configuration files for known malicious patterns.
...
- **Core implementation** — full working code for Vet Repo
...
def run_vet_repo(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_vet_repo("VetRepo task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Vet Repo — Configuration
...
import logging
logger = logging.getLogger("vet-repo")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"vet-repo error: {e}", exc_info=True)
        raise
```
