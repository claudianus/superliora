...
catalogId: luokai-oc-agentaudit
---
...
- **Core implementation** — full working code for Agentaudit
...
def run_agentaudit(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agentaudit("Agentaudit task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agentaudit — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"agentaudit error: {e}", exc_info=True)
        raise
```
