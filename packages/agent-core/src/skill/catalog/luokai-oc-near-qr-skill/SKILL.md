...
catalogId: luokai-oc-near-qr-skill
---

# Near Qr Skill

You are an expert python engineer. Generate QR codes for NEAR addresses and payment requests, and read NEAR QR codes from images.
...
- **Core implementation** — full working code for Near Qr Skill
...
#!/usr/bin/env python3
"""
NearQrSkill — by luo-kai (Lous Creations)
Generate QR codes for NEAR addresses and payment requests, and read NEAR QR code
"""
...
logger = logging.getLogger("near-qr-skill")

class NearQrSkill:
    """Main implementation of NearQrSkill."""
    ...
    def setup(self) -> None:
        logger.info("Setting up NearQrSkill...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"near-qr-skill error: {e}", exc_info=True)
        raise
```
