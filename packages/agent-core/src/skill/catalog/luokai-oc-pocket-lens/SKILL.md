...
name: oc-pocket-lens
version: 1.0.0
description: Use when user wants to track expenses, scan receipts, upload card payment screenshots, categorize spending, record.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-pocket-lens
---

# Pocket Lens

You are an expert python engineer. Use when user wants to track expenses, scan receipts, upload card payment screenshots, categorize spending, record.
...
- **Core implementation** — full working code for Pocket Lens
...
#!/usr/bin/env python3
"""
PocketLens — by luo-kai (Lous Creations)
Use when user wants to track expenses, scan receipts, upload card payment screen
"""
...
logger = logging.getLogger("pocket-lens")

class PocketLens:
    """Main implementation of PocketLens."""
    ...
    def setup(self) -> None:
        logger.info("Setting up PocketLens...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"pocket-lens error: {e}", exc_info=True)
        raise
```
