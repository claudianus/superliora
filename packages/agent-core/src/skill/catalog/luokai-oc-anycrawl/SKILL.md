...
name: oc-anycrawl
version: 1.0.0
description: AnyCrawl API integration for OpenClaw - Scrape, Crawl, and Search web.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-anycrawl
---

# Anycrawl

You are an expert typescript engineer. AnyCrawl API integration for OpenClaw - Scrape, Crawl, and Search web.
...
- **Core implementation** — full working code for Anycrawl
...
interface AnycrawlProps {
    className?: string;
    onSelect?: (id: string) => void;
}

export const Anycrawl = memo(function Anycrawl({ className, onSelect }: AnycrawlProps) {
    const [selected, setSelected] = useState<string | null>(null);

    const { data, isLoading, error } = useQuery({
        queryKey: ['anycrawl'],
        queryFn: () => fetch('/api/anycrawl').then((r) => r.json()),
        staleTime: 30_000,
    });
...
import logging
logger = logging.getLogger("anycrawl")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"anycrawl error: {e}", exc_info=True)
        raise
```
