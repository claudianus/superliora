...
catalogId: luokai-oc-anki-connect
---

# Anki Connect

You are an expert python engineer. Interact with Anki flashcard decks via the AnkiConnect REST API.
...
- **Core implementation** — full working code for Anki Connect
...
#!/usr/bin/env python3
"""
AnkiConnect — by luo-kai (Lous Creations)
...
logger = logging.getLogger("anki-connect")

class AnkiConnect:
    """Main implementation of AnkiConnect."""
    ...
    def setup(self) -> None:
        logger.info("Setting up AnkiConnect...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"anki-connect error: {e}", exc_info=True)
        raise
```
