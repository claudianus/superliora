...
name: oc-mukt
version: 1.0.0
description: Zero-cost OpenRouter responder that auto-discovers the best currently free model, retries on failures, and returns.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mukt
---

# Mukt

You are an expert python engineer. Zero-cost OpenRouter responder that auto-discovers the best currently free model, retries on failures, and returns.
...
- **Core implementation** — full working code for Mukt
...
#!/usr/bin/env python3
"""
Mukt — by luo-kai (Lous Creations)
Zero-cost OpenRouter responder that auto-discovers the best currently free model
"""
...
logger = logging.getLogger("mukt")

class Mukt:
    """Main implementation of Mukt."""
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mukt error: {e}", exc_info=True)
        raise
```
