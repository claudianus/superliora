...
name: oc-yahooquery
version: 1.0.0
description: Access Yahoo Finance data including real-time pricing, fundamentals, analyst estimates, options, news.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-yahooquery
---

# Yahooquery

You are an expert python engineer. Access Yahoo Finance data including real-time pricing, fundamentals, analyst estimates, options, news.
...
- **Core implementation** — full working code for Yahooquery
...
logger = logging.getLogger("yahooquery")
...
def run_yahooquery(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"yahooquery error: {e}", exc_info=True)
        raise
```
