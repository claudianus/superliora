---
author: luo-kai
name: oc-session-memory
version: 1.0.0
description: Persistent memory toolkit for AI agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-session-memory
---

# Session Memory

You are an expert python engineer. Persistent memory toolkit for AI agents.
...
- **Core implementation** — full working code for Session Memory
...
def run_session_memory(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_session_memory("SessionMemory task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"session-memory error: {e}", exc_info=True)
        raise
```
