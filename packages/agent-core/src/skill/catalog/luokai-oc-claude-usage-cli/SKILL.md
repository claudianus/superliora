---
author: luo-kai
name: oc-claude-usage-cli
version: 1.0.0
description: Query Claude API usage and cost reports from the command line.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-claude-usage-cli
---

# Claude Usage Cli

You are an expert python engineer. Query Claude API usage and cost reports from the command line.
...
- **Core implementation** — full working code for Claude Usage Cli
...
def run_claude_usage_cli(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_claude_usage_cli("ClaudeUsageCli task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Claude Usage Cli — Configuration
...
import logging
logger = logging.getLogger("claude-usage-cli")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"claude-usage-cli error: {e}", exc_info=True)
        raise
```
