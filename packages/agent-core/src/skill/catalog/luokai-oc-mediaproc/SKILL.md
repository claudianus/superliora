...
name: oc-mediaproc
version: 1.0.0
description: Process media files (video, audio, images) via a locked-down SSH container.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mediaproc
---

# Mediaproc

You are an expert python engineer. Process media files (video, audio, images) via a locked-down SSH container.
...
- **Core implementation** — full working code for Mediaproc
...
#!/usr/bin/env python3
"""
Mediaproc — by luo-kai (Lous Creations)
...
logger = logging.getLogger("mediaproc")

class Mediaproc:
    """Main implementation of Mediaproc."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Mediaproc...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mediaproc error: {e}", exc_info=True)
        raise
```
