...
catalogId: luokai-oc-geomanic
---

# Geomanic

You are an expert python engineer. Query and manage GPS travel data from Geomanic — your privacy-first GPS tracking platform.
...
- **Core implementation** — full working code for Geomanic
...
logger = logging.getLogger("geomanic")
...
def run_geomanic(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"geomanic error: {e}", exc_info=True)
        raise
```
