---
author: luo-kai
name: oc-localudpmessenger
version: 1.0.0
description: Use when agents need to communicate over the local network — "send message to agent", "discover agents", "check.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-localudpmessenger
---

# Localudpmessenger

You are an expert python engineer. Use when agents need to communicate over the local network — "send message to agent", "discover agents", "check.
...
- **Core implementation** — full working code for Localudpmessenger
...
def run_localudpmessenger(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_localudpmessenger("Localudpmessenger task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Localudpmessenger — Configuration
...
import logging
logger = logging.getLogger("localudpmessenger")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"localudpmessenger error: {e}", exc_info=True)
        raise
```
