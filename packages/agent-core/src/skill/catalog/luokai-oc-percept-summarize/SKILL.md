...
catalogId: luokai-oc-percept-summarize
---

# Percept Summarize

You are an expert python engineer. Automatic conversation summaries with entity extraction and relationship mapping.
...
- **Core implementation** — full working code for Percept Summarize
...
#!/usr/bin/env python3
"""
PerceptSummarize — by luo-kai (Lous Creations)
...
logger = logging.getLogger("percept-summarize")

class PerceptSummarize:
    """Main implementation of PerceptSummarize."""
    ...
    def setup(self) -> None:
        logger.info("Setting up PerceptSummarize...")
        self._ready = True
...
### Configuration & Setup
```python
# Percept Summarize — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"percept-summarize error: {e}", exc_info=True)
        raise
```
