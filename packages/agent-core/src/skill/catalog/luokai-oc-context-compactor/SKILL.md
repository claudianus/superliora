...
catalogId: luokai-oc-context-compactor
---

# Context Compactor

You are an expert python engineer. Token-based context compaction for local models (MLX, llama.cpp, Ollama) that don't report context limits.
...
- **Core implementation** — full working code for Context Compactor
...
#!/usr/bin/env python3
"""
ContextCompactor — by luo-kai (Lous Creations)
Token-based context compaction for local models (MLX, llama.cpp, Ollama) that do
"""
...
logger = logging.getLogger("context-compactor")

class ContextCompactor:
    """Main implementation of ContextCompactor."""
    ...
    def setup(self) -> None:
        logger.info("Setting up ContextCompactor...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"context-compactor error: {e}", exc_info=True)
        raise
```
