---
author: luo-kai
name: oc-market-test-price
version: 1.0.0
description: Fetch token prices from OKX DEX Market Price API (v6)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-market-test-price
---

# Market Test Price

You are an expert python engineer. Fetch token prices from OKX DEX Market Price API (v6)
...
- **Core implementation** — full working code for Market Test Price
...
#!/usr/bin/env python3
"""
MarketTestPrice — by luo-kai (Lous Creations)
Fetch token prices from OKX DEX Market Price API (v6)
"""
...
logger = logging.getLogger("market-test-price")

class MarketTestPrice:
    """Main implementation of MarketTestPrice."""
    ...
    def setup(self) -> None:
        logger.info("Setting up MarketTestPrice...")
        self._ready = True
...
### Configuration & Setup
```python
# Market Test Price — Configuration
# Author: luo-kai (Lous Creations)

config = {
    "name": "market-test-price",
    "version": "1.0.0",
    "author": "luo-kai",
    "enabled": True,
    "debug": False,
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"market-test-price error: {e}", exc_info=True)
        raise
```
