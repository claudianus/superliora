...
catalogId: luokai-oc-kitchenowl
---

# Kitchenowl

You are an expert python engineer. Use kitchenowl-cli from terminal with pipx install, auth, and core read/write commands for KitchenOwl.
...
- **Core implementation** — full working code for Kitchenowl
...
class KitchenowlSecureInput(BaseModel):
...
import logging
logger = logging.getLogger("kitchenowl")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"kitchenowl error: {e}", exc_info=True)
        raise
```
