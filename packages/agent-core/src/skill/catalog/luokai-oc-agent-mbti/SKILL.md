...
name: oc-agent-mbti
version: 1.0.0
description: AI Agent personality diagnosis and configuration system based on MBTI framework.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-agent-mbti
---

# Agent Mbti

You are an expert python engineer. AI Agent personality diagnosis and configuration system based on MBTI framework.
...
- **Core implementation** — full working code for Agent Mbti
...
def run_agent_mbti(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_mbti("AgentMbti task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agent Mbti — Configuration
...
import logging
logger = logging.getLogger("agent-mbti")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-mbti error: {e}", exc_info=True)
        raise
```
