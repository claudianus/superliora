...
catalogId: luokai-oc-broken-link-checker
---

# Broken Link Checker

You are an expert python engineer. verify external URLs (http/https) for availability (200-399 status code).
...
- **Core implementation** — full working code for Broken Link Checker
...
#!/usr/bin/env python3
"""
BrokenLinkChecker — by luo-kai (Lous Creations)
...
logger = logging.getLogger("broken-link-checker")

class BrokenLinkChecker:
    """Main implementation of BrokenLinkChecker."""
    ...
    def setup(self) -> None:
        logger.info("Setting up BrokenLinkChecker...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"broken-link-checker error: {e}", exc_info=True)
        raise
```
