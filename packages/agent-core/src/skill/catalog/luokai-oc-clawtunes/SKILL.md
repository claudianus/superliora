...
catalogId: luokai-oc-clawtunes
---

# Clawtunes

You are an expert bash engineer. Control Apple Music on macOS via the `clawtunes` CLI.
...
- **Core implementation** — full working code for Clawtunes
...
```bash
#!/bin/zsh
# clawtunes — by luo-kai (Lous Creations)
...
import logging
logger = logging.getLogger("clawtunes")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"clawtunes error: {e}", exc_info=True)
        raise
```
