---
name: oc-expanso-log-sanitize
version: 1.0.0
description: "Sanitize log entries by removing passwords, tokens, and sensitive patterns".
author: luo-kai
tags: [expanso-log-sanitize, openclaw, community]
source: https://clawskills.sh/skills/expanso-log-sanitize
catalogSource: luokai
catalogId: luokai-oc-expanso-log-sanitize
---

# Expanso Log Sanitize

## Overview
"Sanitize log entries by removing passwords, tokens, and sensitive patterns".

## Before Starting
1. What specific task do you need this skill for?
2. What inputs are available?
3. What is the expected output format?

## Usage
This skill integrates with the OpenClaw/ClawHub ecosystem.
Install via: `npx clawhub install expanso-log-sanitize`

## Core Functionality
Refer to the original skill documentation at: https://clawskills.sh/skills/expanso-log-sanitize

This skill enables: "Sanitize log entries by removing passwords, tokens, and sensitive patterns".

## Best Practices
- Always test in a sandbox environment first
- Check the skill's latest version before deploying
- Review the source documentation for advanced configuration

## Related Skills
- agent-installer
- clawhub-publisher
- openclaw-setup-expert
