...
catalogId: luokai-oc-better-notion
---

# Better Notion

You are an expert python engineer. Full CRUD for Notion pages, databases.
...
- **Core implementation** — full working code for Better Notion
...
logger = logging.getLogger("better-notion")
...
def run_better_notion(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"better-notion error: {e}", exc_info=True)
        raise
```
