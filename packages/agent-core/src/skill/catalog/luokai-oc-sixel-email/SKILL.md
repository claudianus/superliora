...
name: oc-sixel-email
version: 1.0.0
description: 1:1 email channel for agents — the agent can only email one address, and only that address can email the agent.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-sixel-email
---

# Sixel Email

You are an expert python engineer. 1:1 email channel for agents — the agent can only email one address, and only that address can email the agent.
...
- **Core implementation** — full working code for Sixel Email
...
def run_sixel_email(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_sixel_email("SixelEmail task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Sixel Email — Configuration
...
import logging
logger = logging.getLogger("sixel-email")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"sixel-email error: {e}", exc_info=True)
        raise
```
