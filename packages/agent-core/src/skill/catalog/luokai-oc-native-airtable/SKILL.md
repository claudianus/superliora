---
author: luo-kai
name: oc-native-airtable
version: 1.0.0
description: Read Airtable bases, tables, and records directly via the Airtable API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-native-airtable
---

# Native Airtable

You are an expert python engineer. Read Airtable bases, tables, and records directly via the Airtable API.
...
- **Core implementation** — full working code for Native Airtable
...
#!/usr/bin/env python3
"""
NativeAirtable — by luo-kai (Lous Creations)
...
logger = logging.getLogger("native-airtable")

class NativeAirtable:
    """Main implementation of NativeAirtable."""
    ...
    def setup(self) -> None:
        logger.info("Setting up NativeAirtable...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"native-airtable error: {e}", exc_info=True)
        raise
```
