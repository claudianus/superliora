...
catalogId: luokai-oc-creditcard
---

# Creditcard

You are an expert python engineer. Let your agent shop online with guardrailed wallets, multiple payment methods, and owner approval.
...
- **Core implementation** — full working code for Creditcard
...
def run_creditcard(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_creditcard("Creditcard task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("creditcard")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"creditcard error: {e}", exc_info=True)
        raise
```
