---
author: luo-kai
name: oc-payment-gateway-payram
version: 1.0.0
description: Add payments to your app, agent, or SaaS in 10 seconds.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-payment-gateway-payram
---

# Payment Gateway Payram

You are an expert python engineer. Add payments to your app, agent, or SaaS in 10 seconds.
...
- **Core implementation** — full working code for Payment Gateway Payram
...
def run_payment_gateway_payram(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_payment_gateway_payram("PaymentGatewayPayram task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Payment Gateway Payram — Configuration
...
import logging
logger = logging.getLogger("payment-gateway-payram")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"payment-gateway-payram error: {e}", exc_info=True)
        raise
```
...
- python-expert
- payment-gateway-payram-advanced
