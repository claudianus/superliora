...
catalogId: luokai-oc-wiz-light-control
---

# Wiz Light Control

You are an expert python engineer. Controls Wiz smart bulbs (turn on/off, RGB colors, disco mode) via local WiFi.
...
- **Core implementation** — full working code for Wiz Light Control
...
#!/usr/bin/env python3
"""
WizLightControl — by luo-kai (Lous Creations)
...
logger = logging.getLogger("wiz-light-control")

class WizLightControl:
    """Main implementation of WizLightControl."""
    ...
    def setup(self) -> None:
        logger.info("Setting up WizLightControl...")
        self._ready = True
...
### Configuration & Setup
```python
# Wiz Light Control — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"wiz-light-control error: {e}", exc_info=True)
        raise
```
