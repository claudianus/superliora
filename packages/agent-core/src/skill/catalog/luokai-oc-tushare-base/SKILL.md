...
catalogId: luokai-oc-tushare-base
---

# Tushare Base

You are an expert python engineer. Fetch Chinese stock and futures market data via Tushare API.
...
- **Core implementation** — full working code for Tushare Base
...
logger = logging.getLogger("tushare-base")
...
def run_tushare_base(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"tushare-base error: {e}", exc_info=True)
        raise
```
