...
catalogId: luokai-oc-personaldatahub
---
...
- **Core implementation** — full working code for Personaldatahub
...
logger = logging.getLogger("personaldatahub")
...
def run_personaldatahub(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"personaldatahub error: {e}", exc_info=True)
        raise
```
