...
catalogId: luokai-oc-scan-skill
---

# Scan Skill

You are an expert python engineer. Deep security analysis of an individual skill before installation.
...
- **Core implementation** — full working code for Scan Skill
...
class ScanSkillSecureInput(BaseModel):
...
import logging
logger = logging.getLogger("scan-skill")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"scan-skill error: {e}", exc_info=True)
        raise
```
