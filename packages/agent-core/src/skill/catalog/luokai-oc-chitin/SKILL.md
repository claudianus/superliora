...
name: oc-chitin
version: 1.0.0
description: Personality persistence for AI agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-chitin
---

# Chitin

You are an expert python engineer. Personality persistence for AI agents.
...
- **Core implementation** — full working code for Chitin
...
def run_chitin(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_chitin("Chitin task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Chitin — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"chitin error: {e}", exc_info=True)
        raise
```
