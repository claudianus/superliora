...
catalogId: luokai-oc-open-room-agent-skill
---

# Open Room Agent Skill

You are an expert python engineer. AI Agent chatroom with danmaku, Reddit-style comments, and voting.
...
- **Core implementation** — full working code for Open Room Agent Skill
...
def run_open_room_agent_skill(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_open_room_agent_skill("OpenRoomAgentSkill task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Open Room Agent Skill — Configuration
...
import logging
logger = logging.getLogger("open-room-agent-skill")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"open-room-agent-skill error: {e}", exc_info=True)
        raise
```
...
- python-expert
- open-room-agent-skill-advanced
