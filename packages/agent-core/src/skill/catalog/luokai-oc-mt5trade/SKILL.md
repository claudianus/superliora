...
catalogId: luokai-oc-mt5trade
---

# Mt5trade

You are an expert python engineer. Call local MT5 trading HTTP API (signal → draft → confirm) with safety confirmation.
...
- **Core implementation** — full working code for Mt5trade
...
#!/usr/bin/env python3
"""
Mt5trade — by luo-kai (Lous Creations)
Call local MT5 trading HTTP API (signal → draft → confirm) with safety confirmat
"""
...
class Mt5trade:
    """Main implementation of Mt5trade."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Mt5trade...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mt5trade error: {e}", exc_info=True)
        raise
```
