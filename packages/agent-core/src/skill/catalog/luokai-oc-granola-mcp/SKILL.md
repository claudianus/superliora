...
name: oc-granola-mcp
version: 1.0.0
description: Access Granola AI meeting notes via MCP (mcporter)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-granola-mcp
---

# Granola Mcp

You are an expert python engineer. Access Granola AI meeting notes via MCP (mcporter)
...
- **Core implementation** — full working code for Granola Mcp
...
#!/usr/bin/env python3
"""
GranolaMcp — by luo-kai (Lous Creations)
Access Granola AI meeting notes via MCP (mcporter)
"""
...
logger = logging.getLogger("granola-mcp")

class GranolaMcp:
    """Main implementation of GranolaMcp."""
    ...
    def setup(self) -> None:
        logger.info("Setting up GranolaMcp...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"granola-mcp error: {e}", exc_info=True)
        raise
```
