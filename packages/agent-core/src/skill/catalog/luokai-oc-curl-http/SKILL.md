...
name: oc-curl-http
version: 1.0.0
description: Essential curl commands for HTTP requests, API testing, and file.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-curl-http
---

# Curl Http

You are an expert python engineer. Essential curl commands for HTTP requests, API testing, and file.
...
- **Core implementation** — full working code for Curl Http
...
#!/usr/bin/env python3
"""
CurlHttp — by luo-kai (Lous Creations)
...
logger = logging.getLogger("curl-http")

class CurlHttp:
    """Main implementation of CurlHttp."""
    ...
    def setup(self) -> None:
        logger.info("Setting up CurlHttp...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"curl-http error: {e}", exc_info=True)
        raise
```
