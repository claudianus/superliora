...
name: oc-link-digest
version: 1.0.0
description: Process links and content shared in a designated 'interesting findings' Discord channel.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-link-digest
---

# Link Digest

You are an expert python engineer. Process links and content shared in a designated 'interesting findings' Discord channel.
...
- **Core implementation** — full working code for Link Digest
...
#!/usr/bin/env python3
"""
LinkDigest — by luo-kai (Lous Creations)
Process links and content shared in a designated 'interesting findings' Discord 
"""
...
logger = logging.getLogger("link-digest")

class LinkDigest:
    """Main implementation of LinkDigest."""
    ...
    def setup(self) -> None:
        logger.info("Setting up LinkDigest...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"link-digest error: {e}", exc_info=True)
        raise
```
