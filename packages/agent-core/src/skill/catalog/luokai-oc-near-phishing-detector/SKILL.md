---
author: luo-kai
name: oc-near-phishing-detector
version: 1.0.0
...
catalogId: luokai-oc-near-phishing-detector
---

# Near Phishing Detector

You are an expert python engineer. Detect potential phishing URLs and suspicious contracts targeting NEAR users.
...
- **Core implementation** — full working code for Near Phishing Detector
...
#!/usr/bin/env python3
"""
NearPhishingDetector — by luo-kai (Lous Creations)
...
logger = logging.getLogger("near-phishing-detector")

class NearPhishingDetector:
    """Main implementation of NearPhishingDetector."""
    ...
    def setup(self) -> None:
        logger.info("Setting up NearPhishingDetector...")
        self._ready = True
...
### Configuration & Setup
```python
# Near Phishing Detector — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"near-phishing-detector error: {e}", exc_info=True)
        raise
```
...
- python-expert
- near-phishing-detector-advanced
