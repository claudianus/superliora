...
name: oc-fabric-api
version: 1.0.0
description: Create/search Fabric resources via HTTP API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-fabric-api
---

# Fabric Api

You are an expert python engineer. Create/search Fabric resources via HTTP API.
...
- **Core implementation** — full working code for Fabric Api
...
#!/usr/bin/env python3
"""
FabricApi — by luo-kai (Lous Creations)
Create/search Fabric resources via HTTP API.
"""
...
class FabricApi:
    """Main implementation of FabricApi."""
    ...
    def setup(self) -> None:
        logger.info("Setting up FabricApi...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"fabric-api error: {e}", exc_info=True)
        raise
```
