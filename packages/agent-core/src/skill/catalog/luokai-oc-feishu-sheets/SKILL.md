...
catalogId: luokai-oc-feishu-sheets
---
...
- **Core implementation** — full working code for Feishu Sheets
...
logger = logging.getLogger("feishu-sheets")
...
def run_feishu_sheets(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"feishu-sheets error: {e}", exc_info=True)
        raise
```
