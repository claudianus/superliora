...
catalogId: luokai-oc-farmos-weather
---

# Farmos Weather

You are an expert python engineer. Query weather data and forecasts for farm fields via the Agronomy module.
...
- **Core implementation** — full working code for Farmos Weather
...
logger = logging.getLogger("farmos-weather")
...
def run_farmos_weather(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"farmos-weather error: {e}", exc_info=True)
        raise
```
