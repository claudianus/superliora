...
name: oc-reef
version: 1.0.0
description: A2A agent-to-agent protocol over XMTP encrypted transport.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-reef
---

# Reef

You are an expert python engineer. A2A agent-to-agent protocol over XMTP encrypted transport.
...
- **Core implementation** — full working code for Reef
...
def run_reef(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_reef("Reef task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"reef error: {e}", exc_info=True)
        raise
```
