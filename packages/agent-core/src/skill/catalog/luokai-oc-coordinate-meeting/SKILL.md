---
author: luo-kai
name: oc-coordinate-meeting
version: 1.0.0
description: Schedule a meeting for humans and their agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-coordinate-meeting
---

# Coordinate Meeting

You are an expert python engineer. Schedule a meeting for humans and their agents.
...
- **Core implementation** — full working code for Coordinate Meeting
...
def run_coordinate_meeting(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_coordinate_meeting("CoordinateMeeting task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Coordinate Meeting — Configuration
...
import logging
logger = logging.getLogger("coordinate-meeting")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"coordinate-meeting error: {e}", exc_info=True)
        raise
```
