...
name: oc-baseball
version: 1.0.0
description: Fetch MLB game schedules, live game status, box scores, player search, and season statistics via the MLB Stats API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-baseball
---

# Baseball

You are an expert python engineer. Fetch MLB game schedules, live game status, box scores, player search, and season statistics via the MLB Stats API.
...
- **Core implementation** — full working code for Baseball
...
#!/usr/bin/env python3
"""
Baseball — by luo-kai (Lous Creations)
Fetch MLB game schedules, live game status, box scores, player search, and seaso
"""
...
class Baseball:
    """Main implementation of Baseball."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Baseball...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"baseball error: {e}", exc_info=True)
        raise
```
