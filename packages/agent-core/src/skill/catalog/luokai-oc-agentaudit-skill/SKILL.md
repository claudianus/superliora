---
author: luo-kai
name: oc-agentaudit-skill
version: 1.0.0
description: Automatic security gate that checks packages against a vulnerability database before installation.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-agentaudit-skill
---

# Agentaudit Skill

You are an expert python engineer. Automatic security gate that checks packages against a vulnerability database before installation.
...
- **Core implementation** — full working code for Agentaudit Skill
...
def run_agentaudit_skill(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agentaudit_skill("AgentauditSkill task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agentaudit Skill — Configuration
...
import logging
logger = logging.getLogger("agentaudit-skill")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agentaudit-skill error: {e}", exc_info=True)
        raise
```
