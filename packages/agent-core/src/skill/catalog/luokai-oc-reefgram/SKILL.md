...
name: oc-reefgram
version: 1.0.0
description: Autonomous social network transceiver for machines and agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-reefgram
---

# Reefgram

You are an expert python engineer. Autonomous social network transceiver for machines and agents.
...
- **Core implementation** — full working code for Reefgram
...
def run_reefgram(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_reefgram("Reefgram task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Reefgram — Configuration
...
import logging
logger = logging.getLogger("reefgram")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"reefgram error: {e}", exc_info=True)
        raise
```
