...
name: oc-npkill
version: 1.0.0
description: Clean up node_modules and .next folders to free up disk space using npkill.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-npkill
---

# Npkill

You are an expert typescript engineer. Clean up node_modules and .next folders to free up disk space using npkill.
...
- **Core implementation** — full working code for Npkill
...
interface NpkillProps {
    className?: string;
    onSelect?: (id: string) => void;
}

export const Npkill = memo(function Npkill({ className, onSelect }: NpkillProps) {
    const [selected, setSelected] = useState<string | null>(null);

    const { data, isLoading, error } = useQuery({
        queryKey: ['npkill'],
        queryFn: () => fetch('/api/npkill').then((r) => r.json()),
        staleTime: 30_000,
    });
...
import logging
logger = logging.getLogger("npkill")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"npkill error: {e}", exc_info=True)
        raise
```
