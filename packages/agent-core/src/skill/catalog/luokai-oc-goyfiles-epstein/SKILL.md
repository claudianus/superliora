---
author: luo-kai
name: oc-goyfiles-epstein
version: 1.0.0
description: Core GOYFILES external-bot contract (endpoint-first onboarding, strict agent behavior, direct tool mode.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-goyfiles-epstein
---

# Goyfiles Epstein

You are an expert python engineer. Core GOYFILES external-bot contract (endpoint-first onboarding, strict agent behavior, direct tool mode.
...
- **Core implementation** — full working code for Goyfiles Epstein
...
def run_goyfiles_epstein(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_goyfiles_epstein("GoyfilesEpstein task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Goyfiles Epstein — Configuration
# Author: luo-kai (Lous Creations)

config = {
    "name": "goyfiles-epstein",
    "version": "1.0.0",
    "author": "luo-kai",
    "enabled": True,
    "debug": False,
...
import logging
logger = logging.getLogger("goyfiles-epstein")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"goyfiles-epstein error: {e}", exc_info=True)
        raise
```
