...
name: oc-mac-use
version: 1.0.0
description: Control macOS GUI apps visually — take screenshots, click, scroll, type.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mac-use
---

# Mac Use

You are an expert bash engineer. Control macOS GUI apps visually — take screenshots, click, scroll, type.
...
- **Core implementation** — full working code for Mac Use
...
```bash
#!/bin/zsh
# mac-use — by luo-kai (Lous Creations)
# Control macOS GUI apps visually — take screenshots, click, s
set -euo pipefail
...
import logging
logger = logging.getLogger("mac-use")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"mac-use error: {e}", exc_info=True)
        raise
```
