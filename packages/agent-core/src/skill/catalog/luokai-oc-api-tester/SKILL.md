...
name: oc-api-tester
version: 1.0.0
description: Perform structured HTTP/HTTPS requests (GET, POST, PUT, DELETE) with custom headers and JSON body support.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-api-tester
---

# Api Tester

You are an expert python engineer. Perform structured HTTP/HTTPS requests (GET, POST, PUT, DELETE) with custom headers and JSON body support.
...
- **Core implementation** — full working code for Api Tester
...
#!/usr/bin/env python3
"""
ApiTester — by luo-kai (Lous Creations)
Perform structured HTTP/HTTPS requests (GET, POST, PUT, DELETE) with custom head
"""
...
class ApiTester:
    """Main implementation of ApiTester."""
    ...
    def setup(self) -> None:
        logger.info("Setting up ApiTester...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"api-tester error: {e}", exc_info=True)
        raise
```
