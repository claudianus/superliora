...
catalogId: luokai-oc-skill-cleaner
---

# Skill Cleaner

You are an expert python engineer. Automatically verify "suspicious" skills via VirusTotal and add them to the security allowlist via the Bridge.
...
- **Core implementation** — full working code for Skill Cleaner
...
class SkillCleanerSecureInput(BaseModel):
...
import logging
logger = logging.getLogger("skill-cleaner")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"skill-cleaner error: {e}", exc_info=True)
        raise
```
