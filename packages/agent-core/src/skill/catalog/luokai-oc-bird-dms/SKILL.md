...
catalogId: luokai-oc-bird-dms
---

# Bird Dms

You are an expert python engineer. An add-on to the Bird skill that lets your agent check its X/Twitter DM.
...
- **Core implementation** — full working code for Bird Dms
...
def run_bird_dms(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_bird_dms("BirdDms task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Bird Dms — Configuration
...
import logging
logger = logging.getLogger("bird-dms")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"bird-dms error: {e}", exc_info=True)
        raise
```
