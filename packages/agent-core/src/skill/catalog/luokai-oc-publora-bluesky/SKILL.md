---
author: luo-kai
name: oc-publora-bluesky
version: 1.0.0
description: Post or schedule content to Bluesky using the Publora API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-publora-bluesky
---

# Publora Bluesky

You are an expert python engineer. Post or schedule content to Bluesky using the Publora API.
...
- **Core implementation** — full working code for Publora Bluesky
...
#!/usr/bin/env python3
"""
PubloraBluesky — by luo-kai (Lous Creations)
Post or schedule content to Bluesky using the Publora API.
"""
...
logger = logging.getLogger("publora-bluesky")

class PubloraBluesky:
    """Main implementation of PubloraBluesky."""
    ...
    def setup(self) -> None:
        logger.info("Setting up PubloraBluesky...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"publora-bluesky error: {e}", exc_info=True)
        raise
```
