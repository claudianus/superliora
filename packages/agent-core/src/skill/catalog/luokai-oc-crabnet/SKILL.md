...
catalogId: luokai-oc-crabnet
---

# Crabnet

You are an expert python engineer. Interact with the CrabNet cross-agent collaboration registry.
...
- **Core implementation** — full working code for Crabnet
...
def run_crabnet(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_crabnet("Crabnet task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("crabnet")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"crabnet error: {e}", exc_info=True)
        raise
```
