...
name: oc-groupme-cli
version: 1.0.0
description: Send and read GroupMe messages via the groupme CLI.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-groupme-cli
---

# Groupme Cli

You are an expert python engineer. Send and read GroupMe messages via the groupme CLI.
...
- **Core implementation** — full working code for Groupme Cli
...
#!/usr/bin/env python3
"""
GroupmeCli — by luo-kai (Lous Creations)
Send and read GroupMe messages via the groupme CLI.
"""
...
logger = logging.getLogger("groupme-cli")

class GroupmeCli:
    """Main implementation of GroupmeCli."""
    ...
    def setup(self) -> None:
        logger.info("Setting up GroupmeCli...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"groupme-cli error: {e}", exc_info=True)
        raise
```
