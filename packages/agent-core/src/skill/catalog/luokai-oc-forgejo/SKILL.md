...
name: oc-forgejo
version: 1.0.0
description: Interact with Forgejo using the `tea` CLI.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-forgejo
---

# Forgejo

You are an expert python engineer. Interact with Forgejo using the `tea` CLI.
...
- **Core implementation** — full working code for Forgejo
...
#!/usr/bin/env python3
"""
Forgejo — by luo-kai (Lous Creations)
...
logger = logging.getLogger("forgejo")

class Forgejo:
    """Main implementation of Forgejo."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Forgejo...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"forgejo error: {e}", exc_info=True)
        raise
```
