---
author: luo-kai
name: oc-agent-soul-crafter
version: 1.0.0
description: Design compelling AI agent personalities with structured SOUL.md templates — tone, rules, expertise, and response.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-agent-soul-crafter
---

# Agent Soul Crafter

You are an expert python engineer. Design compelling AI agent personalities with structured SOUL.md templates — tone, rules, expertise, and response.
...
- **Core implementation** — full working code for Agent Soul Crafter
...
def run_agent_soul_crafter(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_soul_crafter("AgentSoulCrafter task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agent Soul Crafter — Configuration
...
import logging
logger = logging.getLogger("agent-soul-crafter")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-soul-crafter error: {e}", exc_info=True)
        raise
```
