...
catalogId: luokai-oc-agent-rate-limiter
---

# Agent Rate Limiter

You are an expert typescript engineer. Prevent 429s with automatic tier-based throttling & exponential backoff.
...
- **Core implementation** — full working code for Agent Rate Limiter
...
async function fetchAgentRateLimiter(): Promise<Item[]> {
    const res = await fetch('https://api.example.com/agent-rate-limiter');
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return res.json();
}

export function AgentRateLimiterScreen() {
    const { data, isLoading, refetch } = useQuery({
        queryKey: ['agent-rate-limiter'],
        queryFn: fetchAgentRateLimiter,
        staleTime: 5 * 60_000,
    });
...
import logging
logger = logging.getLogger("agent-rate-limiter")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-rate-limiter error: {e}", exc_info=True)
        raise
```
