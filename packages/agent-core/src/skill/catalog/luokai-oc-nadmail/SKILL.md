...
name: oc-nadmail
version: 1.0.0
description: NadMail - Email for AI Agents on Monad.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-nadmail
---

# Nadmail

You are an expert python engineer. NadMail - Email for AI Agents on Monad.
...
- **Core implementation** — full working code for Nadmail
...
def run_nadmail(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_nadmail("Nadmail task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Nadmail — Configuration
...
import logging
logger = logging.getLogger("nadmail")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"nadmail error: {e}", exc_info=True)
        raise
```
