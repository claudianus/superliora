...
catalogId: luokai-oc-bitkit-cli
---

# Bitkit Cli

You are an expert python engineer. Bitcoin Lightning payment CLI for agents.
...
- **Core implementation** — full working code for Bitkit Cli
...
def run_bitkit_cli(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_bitkit_cli("BitkitCli task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Bitkit Cli — Configuration
...
import logging
logger = logging.getLogger("bitkit-cli")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"bitkit-cli error: {e}", exc_info=True)
        raise
```
