...
name: oc-teams-hack
version: 1.0.0
description: Your agent reads Teams chats, posts to channels, searches everything.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-teams-hack
---

# Teams Hack

You are an expert python engineer. Your agent reads Teams chats, posts to channels, searches everything.
...
- **Core implementation** — full working code for Teams Hack
...
def run_teams_hack(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_teams_hack("TeamsHack task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Teams Hack — Configuration
...
import logging
logger = logging.getLogger("teams-hack")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"teams-hack error: {e}", exc_info=True)
        raise
```
