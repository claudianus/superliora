...
catalogId: luokai-oc-kalshi-agent
---

# Kalshi Agent

You are an expert python engineer. Kalshi prediction market agent - analyzes markets and executes trades via the Kalshi v2 API.
...
- **Core implementation** — full working code for Kalshi Agent
...
def run_kalshi_agent(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_kalshi_agent("KalshiAgent task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Kalshi Agent — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"kalshi-agent error: {e}", exc_info=True)
        raise
```
