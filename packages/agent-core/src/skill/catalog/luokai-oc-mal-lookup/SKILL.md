...
name: oc-mal-lookup
version: 1.0.0
description: Direct MyAnimeList lookup tool.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mal-lookup
---

# Mal Lookup

You are an expert python engineer. Direct MyAnimeList lookup tool.
...
- **Core implementation** — full working code for Mal Lookup
...
#!/usr/bin/env python3
"""
MalLookup — by luo-kai (Lous Creations)
...
logger = logging.getLogger("mal-lookup")

class MalLookup:
    """Main implementation of MalLookup."""
    ...
    def setup(self) -> None:
        logger.info("Setting up MalLookup...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mal-lookup error: {e}", exc_info=True)
        raise
```
