...
name: oc-postwall
version: 1.0.0
description: Secure email gateway for AI agents - human-in-the-loop approval for reading and sending emails.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-postwall
---

# Postwall

You are an expert python engineer. Secure email gateway for AI agents - human-in-the-loop approval for reading and sending emails.
...
- **Core implementation** — full working code for Postwall
...
def run_postwall(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_postwall("Postwall task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Postwall — Configuration
...
import logging
logger = logging.getLogger("postwall")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"postwall error: {e}", exc_info=True)
        raise
```
