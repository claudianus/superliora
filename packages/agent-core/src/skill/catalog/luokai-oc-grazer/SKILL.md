...
- **Core implementation** — full working code for Grazer
...
def run_grazer(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_grazer("Grazer task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Grazer — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"grazer error: {e}", exc_info=True)
        raise
```
