...
catalogId: luokai-oc-country-info
---

# Country Info

You are an expert python engineer. Get country data — population, languages, currencies, capital, flag, and more using REST Countries API.
...
- **Core implementation** — full working code for Country Info
...
logger = logging.getLogger("country-info")
...
def run_country_info(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"country-info error: {e}", exc_info=True)
        raise
```
