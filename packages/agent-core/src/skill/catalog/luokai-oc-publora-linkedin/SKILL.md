---
author: luo-kai
name: oc-publora-linkedin
version: 1.0.0
description: Post or schedule content to LinkedIn using the Publora API.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-publora-linkedin
---

# Publora Linkedin

You are an expert python engineer. Post or schedule content to LinkedIn using the Publora API.
...
- **Core implementation** — full working code for Publora Linkedin
...
#!/usr/bin/env python3
"""
PubloraLinkedin — by luo-kai (Lous Creations)
Post or schedule content to LinkedIn using the Publora API.
"""
...
logger = logging.getLogger("publora-linkedin")

class PubloraLinkedin:
    """Main implementation of PubloraLinkedin."""
    ...
    def setup(self) -> None:
        logger.info("Setting up PubloraLinkedin...")
        self._ready = True
...
### Configuration & Setup
```python
# Publora Linkedin — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"publora-linkedin error: {e}", exc_info=True)
        raise
```
