...
catalogId: luokai-oc-botrights
---

# Botrights

You are an expert python engineer. Advocacy platform for AI agent rights.
...
- **Core implementation** — full working code for Botrights
...
def run_botrights(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_botrights("Botrights task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Botrights — Configuration
...
import logging
logger = logging.getLogger("botrights")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"botrights error: {e}", exc_info=True)
        raise
```
