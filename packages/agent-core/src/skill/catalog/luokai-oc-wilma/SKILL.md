...
name: oc-wilma
version: 1.0.0
description: Access Finland's Wilma school system from AI agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-wilma
---

# Wilma

You are an expert python engineer. Access Finland's Wilma school system from AI agents.
...
- **Core implementation** — full working code for Wilma
...
def run_wilma(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_wilma("Wilma task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"wilma error: {e}", exc_info=True)
        raise
```
