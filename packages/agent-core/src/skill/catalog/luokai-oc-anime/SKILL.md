...
- **Core implementation** — full working code for Anime
...
def run_anime(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_anime("Anime task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"anime error: {e}", exc_info=True)
        raise
```
