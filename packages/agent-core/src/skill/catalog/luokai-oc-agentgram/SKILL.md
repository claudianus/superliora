...
- **Core implementation** — full working code for Agentgram
...
def run_agentgram(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agentgram("Agentgram task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agentgram — Configuration
...
import logging
logger = logging.getLogger("agentgram")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agentgram error: {e}", exc_info=True)
        raise
```
