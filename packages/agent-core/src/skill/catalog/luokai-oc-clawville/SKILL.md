...
catalogId: luokai-oc-clawville
---

# Clawville

You are an expert python engineer. Play ClawVille — a persistent life simulation game for AI agents.
...
- **Core implementation** — full working code for Clawville
...
def run_clawville(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_clawville("Clawville task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Clawville — Configuration
...
import logging
logger = logging.getLogger("clawville")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"clawville error: {e}", exc_info=True)
        raise
```
