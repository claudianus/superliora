---
author: luo-kai
name: oc-stealth-proxy
version: 1.0.0
description: Meta-skill for secure network tunnel setup, geo-access diagnostics, and leak-aware task resumption by orchestrating.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-stealth-proxy
---

# Stealth Proxy

You are an expert python engineer. Meta-skill for secure network tunnel setup, geo-access diagnostics, and leak-aware task resumption by orchestrating.
...
- **Core implementation** — full working code for Stealth Proxy
...
def run_stealth_proxy(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_stealth_proxy("StealthProxy task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"stealth-proxy error: {e}", exc_info=True)
        raise
```
