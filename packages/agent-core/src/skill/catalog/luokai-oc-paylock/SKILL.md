...
name: oc-paylock
version: 1.0.0
description: Non-custodial SOL escrow for AI agent deals.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-paylock
---

# Paylock

You are an expert python engineer. Non-custodial SOL escrow for AI agent deals.
...
- **Core implementation** — full working code for Paylock
...
def run_paylock(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_paylock("Paylock task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Paylock — Configuration
...
import logging
logger = logging.getLogger("paylock")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"paylock error: {e}", exc_info=True)
        raise
```
