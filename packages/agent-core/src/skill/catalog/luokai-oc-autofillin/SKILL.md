...
name: oc-autofillin
version: 1.0.0
description: Automated web form filling and file uploading skill.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-autofillin
---

# Autofillin

You are an expert typescript engineer. Automated web form filling and file uploading skill.
...
- **Core implementation** — full working code for Autofillin
...
interface AutofillinProps {
    className?: string;
    onSelect?: (id: string) => void;
}

export const Autofillin = memo(function Autofillin({ className, onSelect }: AutofillinProps) {
    const [selected, setSelected] = useState<string | null>(null);

    const { data, isLoading, error } = useQuery({
        queryKey: ['autofillin'],
        queryFn: () => fetch('/api/autofillin').then((r) => r.json()),
        staleTime: 30_000,
    });
...
import logging
logger = logging.getLogger("autofillin")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"autofillin error: {e}", exc_info=True)
        raise
```
