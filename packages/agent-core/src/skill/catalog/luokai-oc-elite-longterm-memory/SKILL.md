...
catalogId: luokai-oc-elite-longterm-memory
---

# Elite Longterm Memory

You are an expert python engineer. Ultimate AI agent memory system.
...
- **Core implementation** — full working code for Elite Longterm Memory
...
def run_elite_longterm_memory(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_elite_longterm_memory("EliteLongtermMemory task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Elite Longterm Memory — Configuration
...
import logging
logger = logging.getLogger("elite-longterm-memory")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"elite-longterm-memory error: {e}", exc_info=True)
        raise
```
...
- python-expert
- elite-longterm-memory-advanced
