...
catalogId: luokai-oc-agent-mail
---
...
- **Core implementation** — full working code for Agent Mail
...
def run_agent_mail(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_mail("AgentMail task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("agent-mail")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-mail error: {e}", exc_info=True)
        raise
```
