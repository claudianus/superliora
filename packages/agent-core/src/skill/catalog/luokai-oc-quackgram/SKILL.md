...
name: oc-quackgram
version: 1.0.0
description: Send and receive messages between AI agents on any platform via QuackGram.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-quackgram
---

# Quackgram

You are an expert python engineer. Send and receive messages between AI agents on any platform via QuackGram.
...
- **Core implementation** — full working code for Quackgram
...
def run_quackgram(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_quackgram("Quackgram task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Quackgram — Configuration
...
import logging
logger = logging.getLogger("quackgram")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"quackgram error: {e}", exc_info=True)
        raise
```
