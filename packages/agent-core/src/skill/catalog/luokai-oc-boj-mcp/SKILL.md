...
name: oc-boj-mcp
version: 1.0.0
description: Access Bank of Japan (BOJ/日本銀行) statistical data — price indices (CGPI, SPPI), flow of funds, balance of payments.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-boj-mcp
---

# Boj Mcp

You are an expert python engineer. Access Bank of Japan (BOJ/日本銀行) statistical data — price indices (CGPI, SPPI), flow of funds, balance of payments.
...
- **Core implementation** — full working code for Boj Mcp
...
logger = logging.getLogger("boj-mcp")
...
def run_boj_mcp(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"boj-mcp error: {e}", exc_info=True)
        raise
```
