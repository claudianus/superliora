...
catalogId: luokai-oc-apple-music
---

# Apple Music

You are an expert bash engineer. Apple Music integration via AppleScript (macOS) or MusicKit API.
...
- **Core implementation** — full working code for Apple Music
...
```bash
#!/bin/zsh
# apple-music — by luo-kai (Lous Creations)
# Apple Music integration via AppleScript (macOS) or MusicKit 
set -euo pipefail
...
import logging
logger = logging.getLogger("apple-music")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"apple-music error: {e}", exc_info=True)
        raise
```
