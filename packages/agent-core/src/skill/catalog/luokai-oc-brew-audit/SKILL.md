...
name: oc-brew-audit
version: 1.0.0
description: Audit Homebrew installation — outdated packages, cleanup opportunities, and health checks.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-brew-audit
---

# Brew Audit

You are an expert bash engineer. Audit Homebrew installation — outdated packages, cleanup opportunities, and health checks.
...
- **Core implementation** — full working code for Brew Audit
...
```bash
#!/bin/zsh
# brew-audit — by luo-kai (Lous Creations)
# Audit Homebrew installation — outdated packages, cleanup opp
set -euo pipefail
...
import logging
logger = logging.getLogger("brew-audit")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"brew-audit error: {e}", exc_info=True)
        raise
```
