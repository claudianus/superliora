...
name: oc-ipcam
version: 1.0.0
description: ONVIF PTZ control + RTSP capture + camera discovery.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-ipcam
---

# Ipcam

You are an expert python engineer. ONVIF PTZ control + RTSP capture + camera discovery.
...
- **Core implementation** — full working code for Ipcam
...
#!/usr/bin/env python3
"""
Ipcam — by luo-kai (Lous Creations)
ONVIF PTZ control + RTSP capture + camera discovery.
"""
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"ipcam error: {e}", exc_info=True)
        raise
```
