---
author: luo-kai
name: oc-sfsymbol-generator
version: 1.0.0
description: Generate an Xcode SF Symbol asset catalog .symbolset.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-sfsymbol-generator
---

# Sfsymbol Generator

You are an expert swift engineer. Generate an Xcode SF Symbol asset catalog .symbolset.
...
- **Core implementation** — full working code for Sfsymbol Generator
...
@Observable
class SfsymbolGeneratorViewModel {
    var isLoading = false
    var items: [Item] = []
    var error: String?
    ...
    func load() async {
        ...
        do {
            let url = URL(string: "https://api.example.com/sfsymbol-generator")!
...
struct SfsymbolGeneratorView: View {
    @State private var vm = SfsymbolGeneratorViewModel()
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("SfsymbolGenerator")
            .task { await vm.load() }
        }
    }
}
...
### Configuration & Setup
```swift
# Sfsymbol Generator — Configuration
...
import logging
logger = logging.getLogger("sfsymbol-generator")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"sfsymbol-generator error: {e}", exc_info=True)
        raise
```
