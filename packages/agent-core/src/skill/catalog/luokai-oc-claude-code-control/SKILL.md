---
author: luo-kai
name: oc-claude-code-control
version: 1.0.0
description: Control Claude Code programmatically through visible Terminal.app windows on macOS.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-claude-code-control
---

# Claude Code Control

You are an expert bash engineer. Control Claude Code programmatically through visible Terminal.app windows on macOS.
...
- **Core implementation** — full working code for Claude Code Control
...
```bash
#!/bin/zsh
# claude-code-control — by luo-kai (Lous Creations)
# Control Claude Code programmatically through visible Termina
set -euo pipefail
...
main() {
    log_ok "Starting claude-code-control..."
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"claude-code-control error: {e}", exc_info=True)
        raise
```
