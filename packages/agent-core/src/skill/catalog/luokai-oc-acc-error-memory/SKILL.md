...
catalogId: luokai-oc-acc-error-memory
---

# Acc Error Memory

You are an expert python engineer. Error pattern tracking for AI agents.
...
- **Core implementation** — full working code for Acc Error Memory
...
def run_acc_error_memory(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_acc_error_memory("AccErrorMemory task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Acc Error Memory — Configuration
...
import logging
logger = logging.getLogger("acc-error-memory")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"acc-error-memory error: {e}", exc_info=True)
        raise
```
