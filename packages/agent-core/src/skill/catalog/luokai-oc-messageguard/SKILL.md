---
author: luo-kai
name: oc-messageguard
version: 1.0.0
description: **Purpose**: MessageGuard filters outgoing text to prevent secret leaks and sensitive data exposure.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-messageguard
---

# Messageguard

You are an expert typescript engineer. **Purpose**: MessageGuard filters outgoing text to prevent secret leaks and sensitive data exposure.
...
- **Core implementation** — full working code for Messageguard
...
async function fetchMessageguard(): Promise<Item[]> {
    const res = await fetch('https://api.example.com/messageguard');
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return res.json();
}

export function MessageguardScreen() {
    const { data, isLoading, refetch } = useQuery({
        queryKey: ['messageguard'],
        queryFn: fetchMessageguard,
        staleTime: 5 * 60_000,
    });
...
import logging
logger = logging.getLogger("messageguard")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"messageguard error: {e}", exc_info=True)
        raise
```
