---
author: luo-kai
name: oc-upsurge-searxng
version: 1.0.0
description: Private Intelligence Radar for Agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-upsurge-searxng
---

# Upsurge Searxng

You are an expert python engineer. Private Intelligence Radar for Agents.
...
- **Core implementation** — full working code for Upsurge Searxng
...
def run_upsurge_searxng(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_upsurge_searxng("UpsurgeSearxng task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Upsurge Searxng — Configuration
...
import logging
logger = logging.getLogger("upsurge-searxng")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"upsurge-searxng error: {e}", exc_info=True)
        raise
```
