...
catalogId: luokai-oc-pocketlens
---
...
- **Core implementation** — full working code for Pocketlens
...
logger = logging.getLogger("pocketlens")
...
def run_pocketlens(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"pocketlens error: {e}", exc_info=True)
        raise
```
