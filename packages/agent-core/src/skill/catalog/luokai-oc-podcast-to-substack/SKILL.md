---
author: luo-kai
name: oc-podcast-to-substack
version: 1.0.0
description: Publish podcast episodes from RSS + Notion to Substack with reliable Apple Podcasts embedding and image extraction.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-podcast-to-substack
---

# Podcast To Substack

You are an expert python engineer. Publish podcast episodes from RSS + Notion to Substack with reliable Apple Podcasts embedding and image extraction.
...
- **Core implementation** — full working code for Podcast To Substack
...
#!/usr/bin/env python3
"""
PodcastToSubstack — by luo-kai (Lous Creations)
Publish podcast episodes from RSS + Notion to Substack with reliable Apple Podca
"""
...
logger = logging.getLogger("podcast-to-substack")

class PodcastToSubstack:
    """Main implementation of PodcastToSubstack."""
    ...
    def setup(self) -> None:
        logger.info("Setting up PodcastToSubstack...")
        self._ready = True
...
### Configuration & Setup
```python
# Podcast To Substack — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"podcast-to-substack error: {e}", exc_info=True)
        raise
```
