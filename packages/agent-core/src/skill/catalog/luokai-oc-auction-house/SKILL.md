---
author: luo-kai
name: oc-auction-house
version: 1.0.0
description: Scout, monitor, and bid on auctions on House (houseproto.fun) — a crypto auction platform on Base.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-auction-house
---

# Auction House

You are an expert python engineer. Scout, monitor, and bid on auctions on House (houseproto.fun) — a crypto auction platform on Base.
...
- **Core implementation** — full working code for Auction House
...
#!/usr/bin/env python3
"""
AuctionHouse — by luo-kai (Lous Creations)
Scout, monitor, and bid on auctions on House (houseproto.fun) — a crypto auction
"""
...
logger = logging.getLogger("auction-house")

class AuctionHouse:
    """Main implementation of AuctionHouse."""
    ...
    def setup(self) -> None:
        logger.info("Setting up AuctionHouse...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"auction-house error: {e}", exc_info=True)
        raise
```
