...
catalogId: luokai-oc-idealista
---

# Idealista

You are an expert python engineer. Query Idealista API via idealista-cli (OAuth2 client credentials).
...
- **Core implementation** — full working code for Idealista
...
class IdealistaSecureInput(BaseModel):
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"idealista error: {e}", exc_info=True)
        raise
```
