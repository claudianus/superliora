...
catalogId: luokai-oc-hello-agent-world
---

# Hello Agent World

You are an expert python engineer. A simple skill that responds with a customized hello message in the user's language.
...
- **Core implementation** — full working code for Hello Agent World
...
def run_hello_agent_world(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_hello_agent_world("HelloAgentWorld task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Hello Agent World — Configuration
...
import logging
logger = logging.getLogger("hello-agent-world")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"hello-agent-world error: {e}", exc_info=True)
        raise
```
