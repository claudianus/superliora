...
catalogId: luokai-oc-v2ex
---

# V2ex

You are an expert typescript engineer. V2EX API 2.0 integration for accessing V2EX forum data, notifications, topics, nodes, and member profiles.
...
- **Core implementation** — full working code for V2ex
...
app.post<{ Body: Omit<Item, 'id'> }>('/v2ex', {
    schema: {
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"v2ex error: {e}", exc_info=True)
        raise
```
