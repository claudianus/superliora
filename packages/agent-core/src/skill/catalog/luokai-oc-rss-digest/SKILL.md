...
name: oc-rss-digest
version: 1.0.0
description: Agentic RSS digest using the feed CLI.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-rss-digest
---

# Rss Digest

You are an expert python engineer. Agentic RSS digest using the feed CLI.
...
- **Core implementation** — full working code for Rss Digest
...
def run_rss_digest(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_rss_digest("RssDigest task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("rss-digest")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"rss-digest error: {e}", exc_info=True)
        raise
```
