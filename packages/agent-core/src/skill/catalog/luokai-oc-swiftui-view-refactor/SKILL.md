---
author: luo-kai
name: oc-swiftui-view-refactor
version: 1.0.0
description: Refactor and review SwiftUI view files.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-swiftui-view-refactor
---

# Swiftui View Refactor

You are an expert swift engineer. Refactor and review SwiftUI view files.
...
- **Core implementation** — full working code for Swiftui View Refactor
...
@Observable
class SwiftuiViewRefactorViewModel {
    var isLoading = false
    var items: [Item] = []
    var error: String?
    ...
    func load() async {
        ...
        do {
            let url = URL(string: "https://api.example.com/swiftui-view-refactor")!
...
struct SwiftuiViewRefactorView: View {
    @State private var vm = SwiftuiViewRefactorViewModel()
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("SwiftuiViewRefactor")
            .task { await vm.load() }
        }
    }
}
...
### Configuration & Setup
```swift
# Swiftui View Refactor — Configuration
...
import logging
logger = logging.getLogger("swiftui-view-refactor")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"swiftui-view-refactor error: {e}", exc_info=True)
        raise
```
...
- swift-expert
- swiftui-view-refactor-advanced
