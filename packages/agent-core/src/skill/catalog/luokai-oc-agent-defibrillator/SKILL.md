---
author: luo-kai
name: oc-agent-defibrillator
version: 1.0.0
description: Watchdog that monitors your AI agent gateway and restarts it when it crashes.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-agent-defibrillator
---

# Agent Defibrillator

You are an expert python engineer. Watchdog that monitors your AI agent gateway and restarts it when it crashes.
...
- **Core implementation** — full working code for Agent Defibrillator
...
def run_agent_defibrillator(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_defibrillator("AgentDefibrillator task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agent Defibrillator — Configuration
...
import logging
logger = logging.getLogger("agent-defibrillator")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-defibrillator error: {e}", exc_info=True)
        raise
```
