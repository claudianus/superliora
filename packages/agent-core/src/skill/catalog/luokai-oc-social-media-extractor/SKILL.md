---
name: oc-social-media-extractor
version: 1.0.0
description: This skill enables Claude to extract public data from **Instagram**, **TikTok**, and **Reddit**.
author: luo-kai
tags: [social-media-extractor, openclaw, community]
source: https://clawskills.sh/skills/social-media-extractor
catalogSource: luokai
catalogId: luokai-oc-social-media-extractor
---

# Social Media Extractor

## Overview
This skill enables Claude to extract public data from **Instagram**, **TikTok**, and **Reddit**.

## Before Starting
1. What specific task do you need this skill for?
2. What inputs are available?
3. What is the expected output format?

## Usage
This skill integrates with the OpenClaw/ClawHub ecosystem.
Install via: `npx clawhub install social-media-extractor`

## Core Functionality
Refer to the original skill documentation at: https://clawskills.sh/skills/social-media-extractor

This skill enables: This skill enables Claude to extract public data from **Instagram**, **TikTok**, and **Reddit**.

## Best Practices
- Always test in a sandbox environment first
- Check the skill's latest version before deploying
- Review the source documentation for advanced configuration

## Related Skills
- agent-installer
- clawhub-publisher
- openclaw-setup-expert
