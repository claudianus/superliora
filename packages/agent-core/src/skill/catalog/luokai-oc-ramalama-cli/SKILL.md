---
name: oc-ramalama-cli
version: 1.0.0
description: Run and interact with AI agents.
author: luo-kai
tags: [ramalama-cli, openclaw, community]
source: https://clawskills.sh/skills/ramalama-cli
catalogSource: luokai
catalogId: luokai-oc-ramalama-cli
---

# Ramalama Cli

## Overview
Run and interact with AI agents.

## Before Starting
1. What specific task do you need this skill for?
2. What inputs are available?
3. What is the expected output format?

## Usage
Install via: `npx clawhub install ramalama-cli`
Documentation: https://clawskills.sh/skills/ramalama-cli

## Core Functionality
Run and interact with AI agents.

## Best Practices
- Test in sandbox environment first
- Check latest version before production use
- Review source docs for advanced configuration

## Related Skills
- agent-installer
- clawhub-publisher
