...
name: oc-meetlark
version: 1.0.0
description: Scheduling polls for humans and their agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-meetlark
---

# Meetlark

You are an expert python engineer. Scheduling polls for humans and their agents.
...
- **Core implementation** — full working code for Meetlark
...
def run_meetlark(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_meetlark("Meetlark task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Meetlark — Configuration
...
import logging
logger = logging.getLogger("meetlark")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"meetlark error: {e}", exc_info=True)
        raise
```
