---
author: luo-kai
name: oc-kimi-usage-monitor
version: 1.0.0
description: Monitor Kimi K2.5 API usage and quota from the Kimi console.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-kimi-usage-monitor
---

# Kimi Usage Monitor

You are an expert python engineer. Monitor Kimi K2.5 API usage and quota from the Kimi console.
...
- **Core implementation** — full working code for Kimi Usage Monitor
...
#!/usr/bin/env python3
"""
KimiUsageMonitor — by luo-kai (Lous Creations)
...
logger = logging.getLogger("kimi-usage-monitor")

class KimiUsageMonitor:
    """Main implementation of KimiUsageMonitor."""
    ...
    def setup(self) -> None:
        logger.info("Setting up KimiUsageMonitor...")
        self._ready = True
...
### Configuration & Setup
```python
# Kimi Usage Monitor — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"kimi-usage-monitor error: {e}", exc_info=True)
        raise
```
