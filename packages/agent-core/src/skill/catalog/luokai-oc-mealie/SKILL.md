...
catalogId: luokai-oc-mealie
---

# Mealie

You are an expert python engineer. Interact with a self‑hosted Mealie instance (recipe manager & meal planner) via its REST API.
...
- **Core implementation** — full working code for Mealie
...
#!/usr/bin/env python3
"""
Mealie — by luo-kai (Lous Creations)
Interact with a self‑hosted Mealie instance (recipe manager & meal planner) via 
"""
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mealie error: {e}", exc_info=True)
        raise
```
