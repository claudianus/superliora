---
author: luo-kai
name: oc-error-driven-evolution
version: 1.0.0
description: Structured error-to-rule learning system for AI agents.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-error-driven-evolution
---

# Error Driven Evolution

You are an expert python engineer. Structured error-to-rule learning system for AI agents.
...
- **Core implementation** — full working code for Error Driven Evolution
...
def run_error_driven_evolution(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_error_driven_evolution("ErrorDrivenEvolution task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Error Driven Evolution — Configuration
...
import logging
logger = logging.getLogger("error-driven-evolution")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"error-driven-evolution error: {e}", exc_info=True)
        raise
```
...
- python-expert
- error-driven-evolution-advanced
