...
catalogId: luokai-oc-drafts
---

# Drafts

You are an expert bash engineer. Manage Drafts app notes via CLI on macOS.
...
- **Core implementation** — full working code for Drafts
...
```bash
#!/bin/zsh
# drafts — by luo-kai (Lous Creations)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"drafts error: {e}", exc_info=True)
        raise
```
