...
catalogId: luokai-oc-agent-hardening
---

# Agent Hardening

You are an expert python engineer. Test your agent's input sanitization against common injection attacks.
...
- **Core implementation** — full working code for Agent Hardening
...
def run_agent_hardening(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_hardening("AgentHardening task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("agent-hardening")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-hardening error: {e}", exc_info=True)
        raise
```
