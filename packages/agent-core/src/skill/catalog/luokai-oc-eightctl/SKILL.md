...
catalogId: luokai-oc-eightctl
---

# Eightctl

You are an expert python engineer. Control Eight Sleep pods (status, temperature, alarms, schedules).
...
- **Core implementation** — full working code for Eightctl
...
#!/usr/bin/env python3
"""
Eightctl — by luo-kai (Lous Creations)
...
logger = logging.getLogger("eightctl")

class Eightctl:
    """Main implementation of Eightctl."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Eightctl...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"eightctl error: {e}", exc_info=True)
        raise
```
