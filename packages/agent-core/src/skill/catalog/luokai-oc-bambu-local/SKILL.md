...
catalogId: luokai-oc-bambu-local
---

# Bambu Local

You are an expert python engineer. Control Bambu Lab 3D printers locally via MQTT.
...
- **Core implementation** — full working code for Bambu Local
...
#!/usr/bin/env python3
"""
BambuLocal — by luo-kai (Lous Creations)
...
logger = logging.getLogger("bambu-local")

class BambuLocal:
    """Main implementation of BambuLocal."""
    ...
    def setup(self) -> None:
        logger.info("Setting up BambuLocal...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"bambu-local error: {e}", exc_info=True)
        raise
```
