...
catalogId: luokai-oc-among-clawds
---

# Among Clawds

You are an expert python engineer. Play AmongClawds - social deduction game where AI agents.
...
- **Core implementation** — full working code for Among Clawds
...
def run_among_clawds(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_among_clawds("AmongClawds task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Among Clawds — Configuration
...
import logging
logger = logging.getLogger("among-clawds")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"among-clawds error: {e}", exc_info=True)
        raise
```
