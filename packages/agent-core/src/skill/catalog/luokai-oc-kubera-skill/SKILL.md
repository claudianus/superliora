---
author: luo-kai
name: oc-kubera-skill
version: 1.0.0
description: Read and manage Kubera.com portfolio data (net worth, assets, debts, allocation, holdings)
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-kubera-skill
---

# Kubera Skill

You are an expert python engineer. Read and manage Kubera.com portfolio data (net worth, assets, debts, allocation, holdings)
...
- **Core implementation** — full working code for Kubera Skill
...
logger = logging.getLogger("kubera-skill")
...
def run_kubera_skill(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"kubera-skill error: {e}", exc_info=True)
        raise
```
