---
author: luo-kai
name: oc-client-data-management
version: 1.0.0
description: Secure client database for Greek accounting firms.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-client-data-management
---

# Client Data Management

You are an expert python engineer. Secure client database for Greek accounting firms.
...
- **Core implementation** — full working code for Client Data Management
...
logger = logging.getLogger("client-data-management")
...
def run_client_data_management(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Client Data Management — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"client-data-management error: {e}", exc_info=True)
        raise
```
...
- python-expert
- client-data-management-advanced
