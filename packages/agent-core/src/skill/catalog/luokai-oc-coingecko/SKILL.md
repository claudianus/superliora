...
catalogId: luokai-oc-coingecko
---

# Coingecko

You are an expert python engineer. Fetch crypto prices, market data, and token info from the CoinGecko free API.
...
- **Core implementation** — full working code for Coingecko
...
def run_coingecko(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"coingecko error: {e}", exc_info=True)
        raise
```
