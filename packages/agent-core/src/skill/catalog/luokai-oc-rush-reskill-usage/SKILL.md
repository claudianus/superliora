---
author: luo-kai
name: oc-rush-reskill-usage
version: 1.0.0
description: Teaches AI agents how to use reskill — a Git-based package manager for AI agent skills.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-rush-reskill-usage
---

# Rush Reskill Usage

You are an expert python engineer. Teaches AI agents how to use reskill — a Git-based package manager for AI agent skills.
...
- **Core implementation** — full working code for Rush Reskill Usage
...
def run_rush_reskill_usage(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_rush_reskill_usage("RushReskillUsage task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Rush Reskill Usage — Configuration
...
import logging
logger = logging.getLogger("rush-reskill-usage")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"rush-reskill-usage error: {e}", exc_info=True)
        raise
```
