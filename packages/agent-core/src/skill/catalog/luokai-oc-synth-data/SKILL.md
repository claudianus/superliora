...
name: oc-synth-data
version: 1.0.0
description: Query volatility forecasts from Synthdata.co for crypto, commodities, and stocks.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-synth-data
---

# Synth Data

You are an expert python engineer. Query volatility forecasts from Synthdata.co for crypto, commodities, and stocks.
...
- **Core implementation** — full working code for Synth Data
...
logger = logging.getLogger("synth-data")
...
def run_synth_data(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"synth-data error: {e}", exc_info=True)
        raise
```
