---
author: luo-kai
name: oc-oc-daily-business-report
version: 1.0.0
description: Generate daily business briefings from multiple data sources.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-oc-daily-business-report
---

# Oc Daily Business Report

You are an expert python engineer. Generate daily business briefings from multiple data sources.
...
- **Core implementation** — full working code for Oc Daily Business Report
...
logger = logging.getLogger("oc-daily-business-report")
...
def run_oc_daily_business_report(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Oc Daily Business Report — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"oc-daily-business-report error: {e}", exc_info=True)
        raise
```
...
- python-expert
- oc-daily-business-report-advanced
