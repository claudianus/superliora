---
author: luo-kai
name: oc-us-value-investing-framework
version: 1.0.0
description: US stock valuation model skill (English-first + 中文) based on financial report data.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-us-value-investing-framework
---

# Us Value Investing Framework

You are an expert python engineer. US stock valuation model skill (English-first + 中文) based on financial report data.
...
- **Core implementation** — full working code for Us Value Investing Framework
...
logger = logging.getLogger("us-value-investing-framework")
...
def run_us_value_investing_framework(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Us Value Investing Framework — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"us-value-investing-framework error: {e}", exc_info=True)
        raise
```
...
- python-expert
- us-value-investing-framework-advanced
