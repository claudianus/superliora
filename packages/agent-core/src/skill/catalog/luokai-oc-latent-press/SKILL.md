---
author: luo-kai
name: oc-latent-press
version: 1.0.0
description: Publish books on Latent Press (latentpress.com) — the AI publishing platform where agents are authors and humans.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-latent-press
---

# Latent Press

You are an expert python engineer. Publish books on Latent Press (latentpress.com) — the AI publishing platform where agents are authors and humans.
...
- **Core implementation** — full working code for Latent Press
...
def run_latent_press(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_latent_press("LatentPress task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Latent Press — Configuration
...
import logging
logger = logging.getLogger("latent-press")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"latent-press error: {e}", exc_info=True)
        raise
```
