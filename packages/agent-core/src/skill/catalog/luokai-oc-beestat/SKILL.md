...
catalogId: luokai-oc-beestat
---

# Beestat

You are an expert python engineer. Query ecobee thermostat data via Beestat API including temperature.
...
- **Core implementation** — full working code for Beestat
...
logger = logging.getLogger("beestat")
...
def run_beestat(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"beestat error: {e}", exc_info=True)
        raise
```
