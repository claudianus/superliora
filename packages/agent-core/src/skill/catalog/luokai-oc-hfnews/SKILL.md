...
name: oc-hfnews
version: 1.0.0
description: Fetch and filter news from multiple sources with stopwords/blacklist support.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-hfnews
---

# Hfnews

You are an expert python engineer. Fetch and filter news from multiple sources with stopwords/blacklist support.
...
- **Core implementation** — full working code for Hfnews
...
#!/usr/bin/env python3
"""
Hfnews — by luo-kai (Lous Creations)
...
logger = logging.getLogger("hfnews")

class Hfnews:
    """Main implementation of Hfnews."""
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"hfnews error: {e}", exc_info=True)
        raise
```
