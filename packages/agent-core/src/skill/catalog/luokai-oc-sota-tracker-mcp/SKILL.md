---
author: luo-kai
name: oc-sota-tracker-mcp
version: 1.0.0
description: **The definitive open-source database of State-of-the-Art AI models.**.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-sota-tracker-mcp
---

# Sota Tracker Mcp

You are an expert python engineer. **The definitive open-source database of State-of-the-Art AI models.**.
...
- **Core implementation** — full working code for Sota Tracker Mcp
...
logger = logging.getLogger("sota-tracker-mcp")
...
def run_sota_tracker_mcp(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Sota Tracker Mcp — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"sota-tracker-mcp error: {e}", exc_info=True)
        raise
```
