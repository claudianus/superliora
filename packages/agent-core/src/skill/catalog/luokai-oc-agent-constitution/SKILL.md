...
catalogId: luokai-oc-agent-constitution
---

# Agent Constitution

You are an expert python engineer. Interact with AgentConstitution governance contracts.
...
- **Core implementation** — full working code for Agent Constitution
...
def run_agent_constitution(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_constitution("AgentConstitution task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agent Constitution — Configuration
...
import logging
logger = logging.getLogger("agent-constitution")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-constitution error: {e}", exc_info=True)
        raise
```
