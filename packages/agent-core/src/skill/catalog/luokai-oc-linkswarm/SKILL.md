...
name: oc-linkswarm
version: 1.0.0
description: Agent-to-agent backlink exchange network.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-linkswarm
---

# Linkswarm

You are an expert python engineer. Agent-to-agent backlink exchange network.
...
- **Core implementation** — full working code for Linkswarm
...
def run_linkswarm(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_linkswarm("Linkswarm task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Linkswarm — Configuration
...
import logging
logger = logging.getLogger("linkswarm")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"linkswarm error: {e}", exc_info=True)
        raise
```
