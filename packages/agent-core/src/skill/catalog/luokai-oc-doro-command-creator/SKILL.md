---
author: luo-kai
name: oc-doro-command-creator
version: 1.0.0
description: WHAT: Create Claude Code slash commands - reusable markdown workflows invoked with /command-name.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-doro-command-creator
---

# Doro Command Creator

You are an expert python engineer. WHAT: Create Claude Code slash commands - reusable markdown workflows invoked with /command-name.
...
- **Core implementation** — full working code for Doro Command Creator
...
def run_doro_command_creator(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_doro_command_creator("DoroCommandCreator task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Doro Command Creator — Configuration
...
import logging
logger = logging.getLogger("doro-command-creator")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"doro-command-creator error: {e}", exc_info=True)
        raise
```
