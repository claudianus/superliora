...
name: oc-envios
version: 1.0.0
description: Usar cuando el usuario pregunte sobre envíos, cómo enviar un pedido, tiempos de entrega, zonas de cobertura.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-envios
---

# Envios

You are an expert swift engineer. Usar cuando el usuario pregunte sobre envíos, cómo enviar un pedido, tiempos de entrega, zonas de cobertura.
...
- **Core implementation** — full working code for Envios
...
class EnviosViewModel {
    ...
    func load() async {
        ...
        do {
            let url = URL(string: "https://api.example.com/envios")!
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"envios error: {e}", exc_info=True)
        raise
```
