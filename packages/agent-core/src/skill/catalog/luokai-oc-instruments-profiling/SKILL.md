---
author: luo-kai
name: oc-instruments-profiling
version: 1.0.0
description: Use when profiling native macOS or iOS apps.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-instruments-profiling
---

# Instruments Profiling

You are an expert swift engineer. Use when profiling native macOS or iOS apps.
...
- **Core implementation** — full working code for Instruments Profiling
...
@Observable
class InstrumentsProfilingViewModel {
    var isLoading = false
    var items: [Item] = []
    var error: String?
    ...
    func load() async {
        ...
        do {
            let url = URL(string: "https://api.example.com/instruments-profiling")!
...
struct InstrumentsProfilingView: View {
    @State private var vm = InstrumentsProfilingViewModel()
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("InstrumentsProfiling")
            .task { await vm.load() }
        }
    }
}
...
### Configuration & Setup
```swift
# Instruments Profiling — Configuration
...
import logging
logger = logging.getLogger("instruments-profiling")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"instruments-profiling error: {e}", exc_info=True)
        raise
```
