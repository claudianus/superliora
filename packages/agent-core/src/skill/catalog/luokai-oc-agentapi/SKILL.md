...
- **Core implementation** — full working code for Agentapi
...
def run_agentapi(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agentapi("Agentapi task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"agentapi error: {e}", exc_info=True)
        raise
```
