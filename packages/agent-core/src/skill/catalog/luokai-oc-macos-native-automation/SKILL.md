---
author: luo-kai
name: oc-macos-native-automation
version: 1.0.0
description: Hardware-level mouse, keyboard & dialog automation on macOS via CGEvent + AppleScript.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-macos-native-automation
---

# Macos Native Automation

You are an expert bash engineer. Hardware-level mouse, keyboard & dialog automation on macOS via CGEvent + AppleScript.
...
- **Core implementation** — full working code for Macos Native Automation
...
```bash
#!/bin/zsh
# macos-native-automation — by luo-kai (Lous Creations)
# Hardware-level mouse, keyboard & dialog automation on macOS 
set -euo pipefail
...
main() {
    log_ok "Starting macos-native-automation..."
...
### Configuration & Setup
```bash
# Macos Native Automation — Configuration
...
import logging
logger = logging.getLogger("macos-native-automation")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"macos-native-automation error: {e}", exc_info=True)
        raise
```
...
- bash-expert
- macos-native-automation-advanced
