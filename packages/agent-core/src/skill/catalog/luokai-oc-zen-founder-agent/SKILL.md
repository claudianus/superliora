...
catalogId: luokai-oc-zen-founder-agent
---

# Zen Founder Agent

You are an expert python engineer. Analyze startup pitch decks and get matched with VCs from Zen.GP's investor database.
...
- **Core implementation** — full working code for Zen Founder Agent
...
def run_zen_founder_agent(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_zen_founder_agent("ZenFounderAgent task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Zen Founder Agent — Configuration
...
import logging
logger = logging.getLogger("zen-founder-agent")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"zen-founder-agent error: {e}", exc_info=True)
        raise
```
