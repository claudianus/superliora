---
author: luo-kai
name: oc-youtube-scrapper
version: 1.0.0
description: A skill for discovering and scraping YouTube channels based on categories and locations without requiring API keys.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-youtube-scrapper
---

# Youtube Scrapper

You are an expert python engineer. A skill for discovering and scraping YouTube channels based on categories and locations without requiring API keys.
...
- **Core implementation** — full working code for Youtube Scrapper
...
#!/usr/bin/env python3
"""
YoutubeScrapper — by luo-kai (Lous Creations)
A skill for discovering and scraping YouTube channels based on categories and lo
"""
...
logger = logging.getLogger("youtube-scrapper")

class YoutubeScrapper:
    """Main implementation of YoutubeScrapper."""
    ...
    def setup(self) -> None:
        logger.info("Setting up YoutubeScrapper...")
        self._ready = True
...
### Configuration & Setup
```python
# Youtube Scrapper — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"youtube-scrapper error: {e}", exc_info=True)
        raise
```
