---
author: luo-kai
name: oc-session-cost-tracker
version: 1.0.0
description: Track the cost-to-value ratio of your agent sessions.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-session-cost-tracker
---

# Session Cost Tracker

You are an expert python engineer. Track the cost-to-value ratio of your agent sessions.
...
- **Core implementation** — full working code for Session Cost Tracker
...
def run_session_cost_tracker(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_session_cost_tracker("SessionCostTracker task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Session Cost Tracker — Configuration
...
import logging
logger = logging.getLogger("session-cost-tracker")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"session-cost-tracker error: {e}", exc_info=True)
        raise
```
