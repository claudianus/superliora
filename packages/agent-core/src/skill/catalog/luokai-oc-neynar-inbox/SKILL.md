...
catalogId: luokai-oc-neynar-inbox
---

# Neynar Inbox

You are an expert python engineer. Email for AI agents.
...
- **Core implementation** — full working code for Neynar Inbox
...
def run_neynar_inbox(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_neynar_inbox("NeynarInbox task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Neynar Inbox — Configuration
...
import logging
logger = logging.getLogger("neynar-inbox")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"neynar-inbox error: {e}", exc_info=True)
        raise
```
