...
name: oc-mcp-chrome
version: 1.0.0
description: Control Chrome browser with AI using MCP protocol.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mcp-chrome
---

# Mcp Chrome

You are an expert python engineer. Control Chrome browser with AI using MCP protocol.
...
- **Core implementation** — full working code for Mcp Chrome
...
#!/usr/bin/env python3
"""
McpChrome — by luo-kai (Lous Creations)
...
logger = logging.getLogger("mcp-chrome")

class McpChrome:
    """Main implementation of McpChrome."""
    ...
    def setup(self) -> None:
        logger.info("Setting up McpChrome...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mcp-chrome error: {e}", exc_info=True)
        raise
```
