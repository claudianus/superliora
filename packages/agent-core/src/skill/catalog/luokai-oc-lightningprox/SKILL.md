---
author: luo-kai
name: oc-lightningprox
version: 1.0.0
description: Pay-per-request AI model access via Bitcoin Lightning using prepaid spend tokens.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-lightningprox
---

# Lightningprox

You are an expert python engineer. Pay-per-request AI model access via Bitcoin Lightning using prepaid spend tokens.
...
- **Core implementation** — full working code for Lightningprox
...
#!/usr/bin/env python3
"""
Lightningprox — by luo-kai (Lous Creations)
Pay-per-request AI model access via Bitcoin Lightning using prepaid spend tokens
"""
...
logger = logging.getLogger("lightningprox")

class Lightningprox:
    """Main implementation of Lightningprox."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Lightningprox...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"lightningprox error: {e}", exc_info=True)
        raise
```
