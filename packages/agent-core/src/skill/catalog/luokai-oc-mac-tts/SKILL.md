...
catalogId: luokai-oc-mac-tts
---

# Mac Tts

You are an expert bash engineer. Text-to-speech using macOS built-in `say` command.
...
- **Core implementation** — full working code for Mac Tts
...
```bash
#!/bin/zsh
# mac-tts — by luo-kai (Lous Creations)
...
import logging
logger = logging.getLogger("mac-tts")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"mac-tts error: {e}", exc_info=True)
        raise
```
