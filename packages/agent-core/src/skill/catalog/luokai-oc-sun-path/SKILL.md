...
name: oc-sun-path
version: 1.0.0
description: Generates a sun path diagram, calculates solar position, performs building shadow analysis, and analyzes thermal.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-sun-path
---

# Sun Path

You are an expert python engineer. Generates a sun path diagram, calculates solar position, performs building shadow analysis, and analyzes thermal.
...
- **Core implementation** — full working code for Sun Path
...
#!/usr/bin/env python3
"""
SunPath — by luo-kai (Lous Creations)
Generates a sun path diagram, calculates solar position, performs building shado
"""
...
logger = logging.getLogger("sun-path")

class SunPath:
    """Main implementation of SunPath."""
    ...
    def setup(self) -> None:
        logger.info("Setting up SunPath...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"sun-path error: {e}", exc_info=True)
        raise
```
