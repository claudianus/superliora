...
name: oc-coda-io
version: 1.0.0
description: Interact with Coda.io docs, tables, rows, pages, and automations via the Coda REST API v1.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-coda-io
---

# Coda Io

You are an expert python engineer. Interact with Coda.io docs, tables, rows, pages, and automations via the Coda REST API v1.
...
- **Core implementation** — full working code for Coda Io
...
#!/usr/bin/env python3
"""
CodaIo — by luo-kai (Lous Creations)
Interact with Coda.io docs, tables, rows, pages, and automations via the Coda RE
"""
...
logger = logging.getLogger("coda-io")

class CodaIo:
    """Main implementation of CodaIo."""
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"coda-io error: {e}", exc_info=True)
        raise
```
