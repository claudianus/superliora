---
author: luo-kai
name: oc-heartbeat-scanner
version: 1.0.0
description: Validate your agent nature through SHACL-based heartbeat analysis.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-heartbeat-scanner
---

# Heartbeat Scanner

You are an expert python engineer. Validate your agent nature through SHACL-based heartbeat analysis.
...
- **Core implementation** — full working code for Heartbeat Scanner
...
def run_heartbeat_scanner(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_heartbeat_scanner("HeartbeatScanner task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Heartbeat Scanner — Configuration
...
import logging
logger = logging.getLogger("heartbeat-scanner")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"heartbeat-scanner error: {e}", exc_info=True)
        raise
```
