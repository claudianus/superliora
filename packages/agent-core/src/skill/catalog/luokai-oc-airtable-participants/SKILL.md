...
catalogId: luokai-oc-airtable-participants
---

# Airtable Participants

You are an expert python engineer. Read and query retreat participant data from the Ceremonia Airtable base.
...
- **Core implementation** — full working code for Airtable Participants
...
logger = logging.getLogger("airtable-participants")
...
def run_airtable_participants(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Airtable Participants — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"airtable-participants error: {e}", exc_info=True)
        raise
```
...
- python-expert
- airtable-participants-advanced
