---
author: luo-kai
name: oc-agent-reputation
version: 1.0.0
description: summary: Cross-platform AI agent reputation checker with trust scoring and PayLock escrow recommendations.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-agent-reputation
---

# Agent Reputation

You are an expert python engineer. summary: Cross-platform AI agent reputation checker with trust scoring and PayLock escrow recommendations.
...
- **Core implementation** — full working code for Agent Reputation
...
def run_agent_reputation(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_reputation("AgentReputation task: analyze and report")
    print(result)
```
...
import logging
logger = logging.getLogger("agent-reputation")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-reputation error: {e}", exc_info=True)
        raise
```
