---
author: luo-kai
name: oc-clawdbot-skill-voice-wake-say
version: 1.0.0
...
catalogId: luokai-oc-clawdbot-skill-voice-wake-say
---

# Clawdbot Skill Voice Wake Say

You are an expert bash engineer. Speak responses aloud on macOS.
...
- **Core implementation** — full working code for Clawdbot Skill Voice Wake Say
...
```bash
#!/bin/zsh
# clawdbot-skill-voice-wake-say — by luo-kai (Lous Creations)
...
main() {
    log_ok "Starting clawdbot-skill-voice-wake-say..."
...
### Configuration & Setup
```bash
# Clawdbot Skill Voice Wake Say — Configuration
...
import logging
logger = logging.getLogger("clawdbot-skill-voice-wake-say")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"clawdbot-skill-voice-wake-say error: {e}", exc_info=True)
        raise
```
...
- bash-expert
- clawdbot-skill-voice-wake-say-advanced
