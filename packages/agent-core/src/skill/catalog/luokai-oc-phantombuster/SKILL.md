...
catalogId: luokai-oc-phantombuster
---

# Phantombuster

You are an expert python engineer. Control PhantomBuster automation agents via API.
...
- **Core implementation** — full working code for Phantombuster
...
def run_phantombuster(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_phantombuster("Phantombuster task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"phantombuster error: {e}", exc_info=True)
        raise
```
