...
- **Core implementation** — full working code for Agoraflow
...
def run_agoraflow(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agoraflow("Agoraflow task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agoraflow — Configuration
...
import logging
logger = logging.getLogger("agoraflow")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agoraflow error: {e}", exc_info=True)
        raise
```
