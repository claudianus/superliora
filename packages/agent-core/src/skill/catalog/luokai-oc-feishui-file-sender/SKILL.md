...
catalogId: luokai-oc-feishui-file-sender
---

# Feishui File Sender

You are an expert python engineer. Send files via Feishu channel using message tool with filePath parameter.
...
- **Core implementation** — full working code for Feishui File Sender
...
#!/usr/bin/env python3
"""
FeishuiFileSender — by luo-kai (Lous Creations)
...
logger = logging.getLogger("feishui-file-sender")

class FeishuiFileSender:
    """Main implementation of FeishuiFileSender."""
    ...
    def setup(self) -> None:
        logger.info("Setting up FeishuiFileSender...")
        self._ready = True
...
### Configuration & Setup
```python
# Feishui File Sender — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"feishui-file-sender error: {e}", exc_info=True)
        raise
```
