...
name: oc-say
version: 1.0.0
description: Text-to-Speech via macOS say command with Siri Natural Voices.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-say
---

# Say

You are an expert bash engineer. Text-to-Speech via macOS say command with Siri Natural Voices.
...
- **Core implementation** — full working code for Say
...
```bash
#!/bin/zsh
# say — by luo-kai (Lous Creations)
# Text-to-Speech via macOS say command with Siri Natural Voice
set -euo pipefail
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"say error: {e}", exc_info=True)
        raise
```
