---
author: luo-kai
name: oc-mobb-vulnerabilities-fixer
version: 1.0.0
description: Scan, fix, and remediate security vulnerabilities in a local code repository using Mobb MCP/CLI.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mobb-vulnerabilities-fixer
---

# Mobb Vulnerabilities Fixer

You are an expert python engineer. Scan, fix, and remediate security vulnerabilities in a local code repository using Mobb MCP/CLI.
...
- **Core implementation** — full working code for Mobb Vulnerabilities Fixer
...
class MobbVulnerabilitiesFixerSecureInput(BaseModel):
...
### Configuration & Setup
```python
# Mobb Vulnerabilities Fixer — Configuration
...
import logging
logger = logging.getLogger("mobb-vulnerabilities-fixer")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"mobb-vulnerabilities-fixer error: {e}", exc_info=True)
        raise
```
...
- python-expert
- mobb-vulnerabilities-fixer-advanced
