---
author: luo-kai
name: oc-voice-wake-say
version: 1.0.0
description: Speak responses aloud on macOS using the built-in `say`.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-voice-wake-say
---

# Voice Wake Say

You are an expert bash engineer. Speak responses aloud on macOS using the built-in `say`.
...
- **Core implementation** — full working code for Voice Wake Say
...
```bash
#!/bin/zsh
# voice-wake-say — by luo-kai (Lous Creations)
...
main() {
    log_ok "Starting voice-wake-say..."
...
import logging
logger = logging.getLogger("voice-wake-say")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"voice-wake-say error: {e}", exc_info=True)
        raise
```
