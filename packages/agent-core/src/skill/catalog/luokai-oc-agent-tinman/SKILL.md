...
catalogId: luokai-oc-agent-tinman
---

# Agent Tinman

You are an expert python engineer. AI security scanner with active prevention - 168 detection.
...
- **Core implementation** — full working code for Agent Tinman
...
def run_agent_tinman(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_tinman("AgentTinman task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agent Tinman — Configuration
...
import logging
logger = logging.getLogger("agent-tinman")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-tinman error: {e}", exc_info=True)
        raise
```
