...
catalogId: luokai-oc-agent-zero
---

# Agent Zero

You are an expert python engineer. Delegate complex coding, research, or autonomous tasks.
...
- **Core implementation** — full working code for Agent Zero
...
def run_agent_zero(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_zero("AgentZero task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"agent-zero error: {e}", exc_info=True)
        raise
```
