---
author: luo-kai
name: oc-permission-creep-scanner
version: 1.0.0
description: Helps detect permission creep in AI agent skills — flags when a skill's actual code accesses resources far.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-permission-creep-scanner
---

# Permission Creep Scanner

You are an expert python engineer. Helps detect permission creep in AI agent skills — flags when a skill's actual code accesses resources far.
...
- **Core implementation** — full working code for Permission Creep Scanner
...
def run_permission_creep_scanner(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_permission_creep_scanner("PermissionCreepScanner task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Permission Creep Scanner — Configuration
...
import logging
logger = logging.getLogger("permission-creep-scanner")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"permission-creep-scanner error: {e}", exc_info=True)
        raise
```
...
- python-expert
- permission-creep-scanner-advanced
