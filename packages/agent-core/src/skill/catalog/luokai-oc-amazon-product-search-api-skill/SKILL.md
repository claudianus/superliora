---
author: luo-kai
name: oc-amazon-product-search-api-skill
version: 1.0.0
description: This skill is designed to help users automatically extract product data from Amazon search results.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-amazon-product-search-api-skill
---

# Amazon Product Search Api Skill

You are an expert python engineer. This skill is designed to help users automatically extract product data from Amazon search results.
...
- **Core implementation** — full working code for Amazon Product Search Api Skill
...
logger = logging.getLogger("amazon-product-search-api-skill")
...
def run_amazon_product_search_api_skill(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
### Configuration & Setup
```python
# Amazon Product Search Api Skill — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"amazon-product-search-api-skill error: {e}", exc_info=True)
        raise
```
...
- python-expert
- amazon-product-search-api-skill-advanced
