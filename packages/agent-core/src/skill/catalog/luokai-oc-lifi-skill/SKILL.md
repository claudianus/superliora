...
name: oc-lifi-skill
version: 1.0.0
description: v4 - Use LI.FI API for cross-chain and same-chain swaps, bridges, and contract calls.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-lifi-skill
---

# Lifi Skill

You are an expert python engineer. v4 - Use LI.FI API for cross-chain and same-chain swaps, bridges, and contract calls.
...
- **Core implementation** — full working code for Lifi Skill
...
#!/usr/bin/env python3
"""
LifiSkill — by luo-kai (Lous Creations)
v4 - Use LI.FI API for cross-chain and same-chain swaps, bridges, and contract c
"""
...
logger = logging.getLogger("lifi-skill")

class LifiSkill:
    """Main implementation of LifiSkill."""
    ...
    def setup(self) -> None:
        logger.info("Setting up LifiSkill...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"lifi-skill error: {e}", exc_info=True)
        raise
```
