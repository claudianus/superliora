---
author: luo-kai
name: oc-agent-device
version: 1.0.0
description: Automates interactions for iOS simulators/devices and Android emulators/devices.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-agent-device
---

# Agent Device

You are an expert swift engineer. Automates interactions for iOS simulators/devices and Android emulators/devices.
...
- **Core implementation** — full working code for Agent Device
...
class AgentDeviceViewModel {
    ...
    func load() async {
        ...
        do {
            let url = URL(string: "https://api.example.com/agent-device")!
...
struct AgentDeviceView: View {
    @State private var vm = AgentDeviceViewModel()
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"agent-device error: {e}", exc_info=True)
        raise
```
