...
catalogId: luokai-oc-agent-teleport
---

# Agent Teleport

You are an expert python engineer. Seamlessly migrate your agent's configuration and memory to a new machine using TiDB Zero.
...
- **Core implementation** — full working code for Agent Teleport
...
def run_agent_teleport(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_agent_teleport("AgentTeleport task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Agent Teleport — Configuration
...
import logging
logger = logging.getLogger("agent-teleport")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"agent-teleport error: {e}", exc_info=True)
        raise
```
