...
catalogId: luokai-oc-paperpod
---

# Paperpod

You are an expert python engineer. Isolated agent runtime for code execution, live preview URLs, browser automation, 50+ tools (ffmpeg, sqlite.
...
- **Core implementation** — full working code for Paperpod
...
def run_paperpod(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_paperpod("Paperpod task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Paperpod — Configuration
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"paperpod error: {e}", exc_info=True)
        raise
```
