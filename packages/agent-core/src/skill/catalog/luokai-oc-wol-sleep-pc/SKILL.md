---
author: luo-kai
name: oc-wol-sleep-pc
version: 1.0.0
description: Send Wake-on-LAN (magic packet) and Sleep-on-LAN (inverted MAC) packets for a specific PC.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-wol-sleep-pc
---

# Wol Sleep Pc

You are an expert python engineer. Send Wake-on-LAN (magic packet) and Sleep-on-LAN (inverted MAC) packets for a specific PC.
...
- **Core implementation** — full working code for Wol Sleep Pc
...
#!/usr/bin/env python3
"""
WolSleepPc — by luo-kai (Lous Creations)
Send Wake-on-LAN (magic packet) and Sleep-on-LAN (inverted MAC) packets for a sp
"""
...
logger = logging.getLogger("wol-sleep-pc")

class WolSleepPc:
    """Main implementation of WolSleepPc."""
    ...
    def setup(self) -> None:
        logger.info("Setting up WolSleepPc...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"wol-sleep-pc error: {e}", exc_info=True)
        raise
```
