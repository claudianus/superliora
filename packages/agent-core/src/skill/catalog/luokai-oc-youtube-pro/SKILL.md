...
catalogId: luokai-oc-youtube-pro
---

# Youtube Pro

You are an expert python engineer. Advanced YouTube analysis, transcripts, and metadata extraction.
...
- **Core implementation** — full working code for Youtube Pro
...
logger = logging.getLogger("youtube-pro")
...
def run_youtube_pro(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"youtube-pro error: {e}", exc_info=True)
        raise
```
