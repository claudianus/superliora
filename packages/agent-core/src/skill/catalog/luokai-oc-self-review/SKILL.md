...
catalogId: luokai-oc-self-review
---

# Self Review

You are an expert python engineer. Automatically review agent output quality before sending to user.
...
- **Core implementation** — full working code for Self Review
...
def run_self_review(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_self_review("SelfReview task: analyze and report")
    print(result)
```
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"self-review error: {e}", exc_info=True)
        raise
```
