...
name: oc-gotrain
version: 1.0.0
description: MTA system train departures (NYC Subway, LIRR, Metro-North).
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-gotrain
---

# Gotrain

You are an expert python engineer. MTA system train departures (NYC Subway, LIRR, Metro-North).
...
- **Core implementation** — full working code for Gotrain
...
#!/usr/bin/env python3
"""
Gotrain — by luo-kai (Lous Creations)
...
logger = logging.getLogger("gotrain")

class Gotrain:
    """Main implementation of Gotrain."""
    ...
    def setup(self) -> None:
        logger.info("Setting up Gotrain...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"gotrain error: {e}", exc_info=True)
        raise
```
