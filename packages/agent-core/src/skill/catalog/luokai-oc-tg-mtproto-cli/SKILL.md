---
author: luo-kai
name: oc-tg-mtproto-cli
version: 1.0.0
description: Read-only Telegram CLI via MTProto.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-tg-mtproto-cli
---

# Tg Mtproto Cli

You are an expert python engineer. Read-only Telegram CLI via MTProto.
...
- **Core implementation** — full working code for Tg Mtproto Cli
...
#!/usr/bin/env python3
"""
TgMtprotoCli — by luo-kai (Lous Creations)
Read-only Telegram CLI via MTProto.
"""
...
logger = logging.getLogger("tg-mtproto-cli")

class TgMtprotoCli:
    """Main implementation of TgMtprotoCli."""
    ...
    def setup(self) -> None:
        logger.info("Setting up TgMtprotoCli...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"tg-mtproto-cli error: {e}", exc_info=True)
        raise
```
