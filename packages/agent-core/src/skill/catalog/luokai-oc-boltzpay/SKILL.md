...
catalogId: luokai-oc-boltzpay
---

# Boltzpay

You are an expert python engineer. Pay for API data automatically — multi-protocol (x402 + L402), multi-chain.
...
- **Core implementation** — full working code for Boltzpay
...
logger = logging.getLogger("boltzpay")
...
def run_boltzpay(source: str, dest: str) -> dict:
    raw = extract(source)
    clean = transform(raw)
    load(clean, dest)
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"boltzpay error: {e}", exc_info=True)
        raise
```
