...
name: oc-error-guard
version: 1.0.0
description: System safety and control-plane skill that prevents agent deadlocks.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-error-guard
---

# Error Guard

You are an expert python engineer. System safety and control-plane skill that prevents agent deadlocks.
...
- **Core implementation** — full working code for Error Guard
...
def run_error_guard(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_error_guard("ErrorGuard task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Error Guard — Configuration
...
import logging
logger = logging.getLogger("error-guard")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"error-guard error: {e}", exc_info=True)
        raise
```
