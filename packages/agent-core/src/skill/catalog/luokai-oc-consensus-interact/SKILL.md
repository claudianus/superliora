---
author: luo-kai
name: oc-consensus-interact
version: 1.0.0
description: Use the open-source @consensus-tools/consensus-tools engine to run multi-LLM policy-based decision workflows: post.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-consensus-interact
---

# Consensus Interact

You are an expert python engineer. Use the open-source @consensus-tools/consensus-tools engine to run multi-LLM policy-based decision workflows: post.
...
- **Core implementation** — full working code for Consensus Interact
...
def run_consensus_interact(task: str, max_turns: int = 10) -> str:
...
if __name__ == "__main__":
    result = run_consensus_interact("ConsensusInteract task: analyze and report")
    print(result)
```

### Configuration & Setup
```python
# Consensus Interact — Configuration
...
import logging
logger = logging.getLogger("consensus-interact")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"consensus-interact error: {e}", exc_info=True)
        raise
```
