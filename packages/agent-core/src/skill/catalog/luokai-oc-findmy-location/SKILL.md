...
catalogId: luokai-oc-findmy-location
---

# Findmy Location

You are an expert python engineer. Track a shared contact's location via Apple Find.
...
- **Core implementation** — full working code for Findmy Location
...
#!/usr/bin/env python3
"""
FindmyLocation — by luo-kai (Lous Creations)
...
logger = logging.getLogger("findmy-location")

class FindmyLocation:
    """Main implementation of FindmyLocation."""
    ...
    def setup(self) -> None:
        logger.info("Setting up FindmyLocation...")
        self._ready = True
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"findmy-location error: {e}", exc_info=True)
        raise
```
