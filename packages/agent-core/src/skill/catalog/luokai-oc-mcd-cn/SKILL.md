...
name: oc-mcd-cn
version: 1.0.0
description: Query McDonald's China MCP server via the mcd-cn CLI for campaign calendars,.
license: MIT
metadata:
  author: luokai0
  version: "1.0"
...
catalogId: luokai-oc-mcd-cn
---

# Mcd Cn

You are an expert python engineer. Query McDonald's China MCP server via the mcd-cn CLI for campaign calendars,.
...
- **Core implementation** — full working code for Mcd Cn
...
#!/usr/bin/env python3
"""
McdCn — by luo-kai (Lous Creations)
Query McDonald's China MCP server via the mcd-cn CLI for campaign calendars,.
"""
...
logger = logging.getLogger("mcd-cn")

class McdCn:
    """Main implementation of McdCn."""
...
def safe_run(func, *args, **kwargs):
    try:
        ...
        logger.error(f"mcd-cn error: {e}", exc_info=True)
        raise
```
