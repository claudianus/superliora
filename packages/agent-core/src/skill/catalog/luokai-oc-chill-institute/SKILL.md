...
catalogId: luokai-oc-chill-institute
---

# Chill Institute

You are an expert typescript engineer. Use chill.institute (web UI) to search for content and click.
...
- **Core implementation** — full working code for Chill Institute
...
interface ChillInstituteProps {
    className?: string;
    onSelect?: (id: string) => void;
}

export const ChillInstitute = memo(function ChillInstitute({ className, onSelect }: ChillInstituteProps) {
    const [selected, setSelected] = useState<string | null>(null);

    const { data, isLoading, error } = useQuery({
        queryKey: ['chill-institute'],
        queryFn: () => fetch('/api/chill-institute').then((r) => r.json()),
        staleTime: 30_000,
    });
...
import logging
logger = logging.getLogger("chill-institute")

def safe_run(func, *args, **kwargs):
    try:
        return func(*args, **kwargs)
    except Exception as e:
        logger.error(f"chill-institute error: {e}", exc_info=True)
        raise
```
