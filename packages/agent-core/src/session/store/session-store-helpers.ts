...
export function metadataFromState(state: SessionSummaryState | undefined): JsonObject | undefined {
  if (state === undefined || state.custom === undefined) return undefined;
  return state.custom as JsonObject;
}

export function forkCustomMetadata(source: unknown, metadata: JsonObject | undefined): Record<string, unknown> {
  return {
...
export async function dropForkedSessionFiles(sessionDir: string): Promise<void> {
  await Promise.all(
...
export async function readOptionalState(sessionDir: string): Promise<SessionSummaryState | undefined> {
  try {
...
export function normalizeRequiredWorkDir(workDir: string): string {
  if (workDir.trim() === '') {
...
export function normalizeOptionalSessionId(sessionId: string | undefined): string | undefined {
  return sessionId === undefined ? undefined : sessionId.trim();
}

export function normalizeForkTitle(title: string | undefined, fallback: unknown): string {
  if (title !== undefined) {
...
export function remapSessionPath(value: string, sourceDir: string, targetDir: string): string {
...
export async function statIfExists(path: string): Promise<{ readonly mtimeMs: number } | undefined> {
  try {
...
export function timestampOrFallback(value: number, fallback: number): number {
...
export function compareSessionSummary(a: SessionSummary, b: SessionSummary): number {
