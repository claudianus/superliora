...
In-process service layer for the SuperLiora server, now hosted inside
`@superliora/agent-core` (merged from the former `@superliora/services`
...
from `index.ts` so importing `@superliora/agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@superliora/agent-core` loads the package barrel, whose impl
