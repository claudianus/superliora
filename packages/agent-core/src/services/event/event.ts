 ...
 * `LioraCore` (and synthetic events from daemon-side services) to all
...
import type { Event as ProtocolEvent } from '@superliora/protocol';

/**
 * Naming convention inside this file:
 *
 * - `Event` (from `@superliora/agent-core/base/common/event`) — the generic
 *   VSCode-style emitter accessor type. `Event<T>` is the listener-tuple
 *   type used to declare `readonly onDidXxx: Event<T>`.
 * - `ProtocolEvent` (alias of `@superliora/protocol`'s `Event`) — the
