...
export function buildToolWorkflowGuidance(cap: ToolWorkflowCapability): string {
  const lines: string[] = [
    ...
    '- File content: use Write/Edit — do not cat/echo/printf redirect, heredoc, tee, dd if=/of=, install src dest, empty redirects (`: > file`), or python/node/ruby/php/perl/lua -c/-e/-r writeFile/open-write one-liners through Bash (runtime-blocked).',
    '- File reads: use Read — do not python/node/ruby/php/perl/lua -c/-e/-r open/readFile one-liners, or bat/tac/rev/paste/sed -n/awk/base64/fmt/pr/fold dumps, for file contents (runtime-blocked).',
