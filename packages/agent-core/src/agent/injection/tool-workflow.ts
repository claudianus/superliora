...
export function buildToolWorkflowGuidance(cap: ToolWorkflowCapability): string {
  const lines: string[] = [
    ...
    '- File reads: use Read — do not python/node/ruby/php/perl/lua -c/-e/-r open/readFile one-liners, or bat/tac/sed -n/awk/base64/fmt/pr/fold dumps, for file contents (runtime-blocked).',
