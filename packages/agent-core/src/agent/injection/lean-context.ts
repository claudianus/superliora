export function buildLeanContextGuidance(): string {
  return [
    ...
    '- Prefer dedicated file tools over Bash cat/sed/grep/redirect/heredoc when they fit; use Bash for real shell semantics only. Never shell-read secret files (.env, SSH keys).',
  ].join('\n');
}
