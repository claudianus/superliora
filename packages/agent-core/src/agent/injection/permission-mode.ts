...
const AUTO_MODE_ENTER_REMINDER = [
  ...
  '  - Ultra Plan starts with a read-only Research phase; do not ask the user there. Outside Ultra Plan interview, avoid AskUserQuestion unless a missing decision blocks correctness. Inside Ultra Plan interview, research/read first when needed, then ask only after blockers are narrowed; NextPhase enforces the gate.',
].join('\n');
