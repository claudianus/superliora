...
const AUTO_MODE_ENTER_REMINDER = [
  ...
  '  - AskUserQuestion auto-answers structured questions by upgrading quality (prefer visual rubric / VerifySurface / cinematic / complete scope / tests over skip, later, minimal, or bug-only; open-ended proceeds toward that higher bar and refines if blocked). Destructive or irreversible decisions (delete, force push, deploy, sharing secrets) are NEVER auto-answered — they always wait for the human. Ask mode also disables auto-answer so the question dialog waits for the human. Land-to-main uses MergeJob trust (not AskUserQuestion): in auto mode size/danger confirm holds are waived; conflict, missing checks, and visual proof still block. Ultra Plan interview still runs the same question flow.',
