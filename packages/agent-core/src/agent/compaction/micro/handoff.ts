...
import { estimateTokens, estimateTokensForContentParts } from '../../../utils/tokens';
import type { PromptOrigin } from '../../context/types';
import summaryPrefixTemplate from '../prompts/compaction-summary-prefix.md?raw';
...
function estimateTokensForMessageLike(message: MessageLike): number {
  return estimateTokens(message.role) + estimateTokensForContentParts(message.content);
}
...
export function selectRecentUserMessages<T extends MessageLike>(
  ...
  for (let i = messages.length - 1; i >= 0 && remaining > 0; i--) {
    ...
    const tokens = estimateTokensForMessageLike(message);
    if (tokens <= remaining) {
...
export function selectCompactionUserMessages<T extends MessageLike>(
  ...
  for (const message of messages) {
    totalTokens += estimateTokensForMessageLike(message);
  }
  ...
  let keptTokens = 0;
  for (const message of head) keptTokens += estimateTokensForMessageLike(message);
  for (const message of tail) keptTokens += estimateTokensForMessageLike(message);
