 ...
 * Compact before attention rot: 0.10 sits between async pre-rot (~0.01) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.10;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.08, this.config.triggerRatio - this.qualityTriggerBias);
  }
