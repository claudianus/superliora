 ...
 * Compact before attention rot: 0.35 sits between async pre-rot (~0.24) /
 * swarm handoff (~0.36) and hard block (~0.64), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.35;
/** Hard block near the window; leaves headroom for compaction summary output. */
export const DEFAULT_COMPACTION_BLOCK_RATIO = 0.64;
...
export const SWARM_HANDOFF_COMPACTION_RATIO = 0.36;
/**
 ...
 * for cost and fidelity; start at async pre-rot (~0.24) before soft trigger.
 */
export const SWARM_MICRO_PRESSURE_RATIO = 0.24;
/** Default ratio at which async background compaction may start (pre-rot). */
export const DEFAULT_ASYNC_COMPACTION_TRIGGER_RATIO = 0.24;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.33, this.config.triggerRatio - this.qualityTriggerBias);
  }
