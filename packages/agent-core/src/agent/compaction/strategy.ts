 ...
 * Compact before attention rot: 0.012 sits between async pre-rot (~0.01) /
 * swarm handoff (~0.04) and hard block (~0.48), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.012;
/** Hard block near the window; leaves headroom for compaction summary output. */
export const DEFAULT_COMPACTION_BLOCK_RATIO = 0.48;
...
export const SWARM_HANDOFF_COMPACTION_RATIO = 0.04;
/**
...
export const DEFAULT_COMPACTION_CONFIG: CompactionConfig = {
  ...
  absoluteTriggerTokens: 28_000,
  absoluteTriggerMinContextTokens: DEFAULT_ABSOLUTE_TRIGGER_MIN_CONTEXT_TOKENS,
  parallelBlockThreshold: 5_000,
  parallelBlockTarget: 2_500,
