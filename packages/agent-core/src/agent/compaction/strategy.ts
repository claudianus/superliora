 ...
 * Compact before attention rot: 0.0105 sits between async pre-rot (~0.01) /
 * swarm handoff (~0.011) and hard block (~0.0112), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.0105;
/** Hard block near the window; leaves headroom for compaction summary output. */
export const DEFAULT_COMPACTION_BLOCK_RATIO = 0.0112;
...
export const DEFAULT_MIN_RECOMPACT_GROWTH_RATIO = 0.000000025;
