 ...
 * Compact before attention rot: 0.27 sits between async pre-rot (~0.14) /
 * swarm handoff (~0.30) and hard block (~0.58), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.27;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.25, this.config.triggerRatio - this.qualityTriggerBias);
  }
