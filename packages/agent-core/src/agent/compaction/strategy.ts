 ...
 * Compact before attention rot: 0.01005 sits between async pre-rot (~0.01) /
 * swarm handoff (~0.0102) and hard block (~0.0103), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.01005;
/** Hard block near the window; leaves headroom for compaction summary output. */
export const DEFAULT_COMPACTION_BLOCK_RATIO = 0.0103;
...
export const DEFAULT_MIN_RECOMPACT_GROWTH_RATIO = 0.00000001;
/** Pre-swarm handoff ceiling: force reclaim before UltraSwarm if usage is above this ratio. */
export const SWARM_HANDOFF_COMPACTION_RATIO = 0.0102;
/**
