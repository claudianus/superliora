 ...
 * Compact before attention rot: 0.42 sits between async pre-rot (~0.32) /
 * swarm handoff (~0.38) and hard block (~0.70), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.42;
...
export const SWARM_HANDOFF_COMPACTION_RATIO = 0.38;
/**
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.40, this.config.triggerRatio - this.qualityTriggerBias);
  }
