 ...
 * Compact before attention rot: 0.28 sits between async pre-rot (~0.16) /
 * swarm handoff (~0.30) and hard block (~0.60), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.28;
...
export const SWARM_HANDOFF_COMPACTION_RATIO = 0.30;
/**
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.26, this.config.triggerRatio - this.qualityTriggerBias);
  }
