 ...
 * Compact before attention rot: 0.33 sits between async pre-rot (~0.22) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.33;
 ...
 * for cost and fidelity; start at async pre-rot (~0.22) before soft trigger.
 */
export const SWARM_MICRO_PRESSURE_RATIO = 0.22;
/** Default ratio at which async background compaction may start (pre-rot). */
export const DEFAULT_ASYNC_COMPACTION_TRIGGER_RATIO = 0.22;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.31, this.config.triggerRatio - this.qualityTriggerBias);
  }
