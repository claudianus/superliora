 ...
 * Compact before attention rot: 0.38 sits between async pre-rot (~0.28) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.38;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.36, this.config.triggerRatio - this.qualityTriggerBias);
  }
