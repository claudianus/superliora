 ...
 * Compact before attention rot: 0.34 sits between async pre-rot (~0.24) /
 * swarm handoff (~0.34) and hard block (~0.64), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.34;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.32, this.config.triggerRatio - this.qualityTriggerBias);
  }
