 ...
 * Compact before attention rot: 0.17 sits between async pre-rot (~0.05) /
 * swarm handoff (~0.20) and hard block (~0.52), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.17;
...
export const SWARM_HANDOFF_COMPACTION_RATIO = 0.20;
/**
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.15, this.config.triggerRatio - this.qualityTriggerBias);
  }
