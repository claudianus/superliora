 ...
 * Compact before attention rot: 0.64 sits between async pre-rot (~0.52) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.64;
/** Hard block near the window; leaves headroom for compaction summary output. */
export const DEFAULT_COMPACTION_BLOCK_RATIO = 0.9;
 ...
 * for cost and fidelity; start at async pre-rot (~0.52) before soft trigger.
 */
export const SWARM_MICRO_PRESSURE_RATIO = 0.52;
/** Default ratio at which async background compaction may start (pre-rot). */
export const DEFAULT_ASYNC_COMPACTION_TRIGGER_RATIO = 0.52;
