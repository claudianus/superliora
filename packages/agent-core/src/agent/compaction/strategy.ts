 ...
 * Compact before attention rot: 0.11 sits between async pre-rot (~0.01) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.11;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.09, this.config.triggerRatio - this.qualityTriggerBias);
  }
