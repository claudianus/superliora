 ...
 * Compact before attention rot: 0.20 sits between async pre-rot (~0.08) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.20;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.18, this.config.triggerRatio - this.qualityTriggerBias);
  }
