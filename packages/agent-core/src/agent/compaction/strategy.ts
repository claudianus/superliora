 ...
 * Compact before attention rot: 0.19 sits between async pre-rot (~0.06) /
 * swarm handoff (~0.22) and hard block (~0.54), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.19;
...
export const SWARM_HANDOFF_COMPACTION_RATIO = 0.22;
/**
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.17, this.config.triggerRatio - this.qualityTriggerBias);
  }
