 ...
 * Compact before attention rot: 0.31 sits between async pre-rot (~0.20) /
 * swarm handoff (~0.32) and hard block (~0.62), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.31;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.29, this.config.triggerRatio - this.qualityTriggerBias);
  }
