 ...
 * Compact before attention rot: 0.55 sits between async pre-rot (~0.44) /
 * swarm handoff (~0.46) and hard block (~0.84), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.55;
/** Hard block near the window; leaves headroom for compaction summary output. */
export const DEFAULT_COMPACTION_BLOCK_RATIO = 0.84;
/** Estimated tokens the next agent step may add for speculative pre-turn compaction (lean default). */
export const DEFAULT_SPECULATIVE_STEP_BUFFER_TOKENS = 1_000;
...
export const SWARM_HANDOFF_COMPACTION_RATIO = 0.46;
/**
 ...
 * for cost and fidelity; start at async pre-rot (~0.44) before soft trigger.
 */
export const SWARM_MICRO_PRESSURE_RATIO = 0.44;
/** Default ratio at which async background compaction may start (pre-rot). */
export const DEFAULT_ASYNC_COMPACTION_TRIGGER_RATIO = 0.44;
...
export const DEFAULT_COMPACTION_CONFIG: CompactionConfig = {
  ...
  reservedContextSize: 26_000,
  maxCompactionPerTurn: Infinity,
  maxOverflowCompactionAttempts: 3,
  maxRecentMessages: 2,
  maxRecentUserMessages: Infinity,
  maxRecentSizeRatio: 0.06,
  minOverflowReductionRatio: 0.05,
  absoluteTriggerTokens: 100_000,
  absoluteTriggerMinContextTokens: DEFAULT_ABSOLUTE_TRIGGER_MIN_CONTEXT_TOKENS,
  parallelBlockThreshold: 14_000,
  parallelBlockTarget: 7_000,
