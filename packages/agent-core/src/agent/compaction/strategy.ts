 ...
 * Compact before attention rot: 0.36 sits between async pre-rot (~0.26) /
 * swarm handoff (~0.36) and hard block (~0.66), so summaries generate while
 * the model still attends well. Async path still starts earlier via asyncTriggerRatio.
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.36;
/** Hard block near the window; leaves headroom for compaction summary output. */
export const DEFAULT_COMPACTION_BLOCK_RATIO = 0.66;
 ...
 * for cost and fidelity; start at async pre-rot (~0.26) before soft trigger.
 */
export const SWARM_MICRO_PRESSURE_RATIO = 0.26;
/** Default ratio at which async background compaction may start (pre-rot). */
export const DEFAULT_ASYNC_COMPACTION_TRIGGER_RATIO = 0.26;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.34, this.config.triggerRatio - this.qualityTriggerBias);
  }
