 ...
 * Compact before attention rot: 0.05 sits between async pre-rot (~0.01) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.05;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.03, this.config.triggerRatio - this.qualityTriggerBias);
  }
