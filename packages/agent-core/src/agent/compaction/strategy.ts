 ...
 * Compact before attention rot: 0.14 sits between async pre-rot (~0.04) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.14;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.12, this.config.triggerRatio - this.qualityTriggerBias);
  }
