 ...
 * Compact before attention rot: 0.37 sits between async pre-rot (~0.26) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.37;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.35, this.config.triggerRatio - this.qualityTriggerBias);
  }
