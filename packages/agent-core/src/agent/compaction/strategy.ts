 ...
 * Compact before attention rot: 0.09 sits between async pre-rot (~0.01) /
 ...
 */
export const DEFAULT_COMPACTION_TRIGGER_RATIO = 0.09;
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  ...
  get effectiveTriggerRatio(): number {
    return Math.max(0.07, this.config.triggerRatio - this.qualityTriggerBias);
  }
