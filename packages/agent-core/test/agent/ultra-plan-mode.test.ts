...
describe('UltraPlanModeEngine', () => {
  describe('Seed Spec', () => {
    ...
    it('falls back to a heuristic Seed Spec when LLM extraction fails', async () => {
      // No working generate() — interview→design must still produce a usable Seed Spec
      // (mirrors ambiguity scoring's usedHeuristicFallback path).
      const engine = new UltraPlanModeEngine(mockAgent);
      engine.startInterview('Build a CLI tool');
      engine.addInterviewRound('What language?', 'TypeScript');
      engine.addInterviewRound(
        '완료 기준은?',
        'CLI prints --help and unit tests pass.',
      );
      const seed = await engine.autoGenerateSeedSpecFromInterview('CLI');
      expect(seed.goal).toContain('CLI');
      expect(seed.autoGenerated).toBe(true);
      expect(seed.acceptanceCriteria.length).toBeGreaterThan(0);
      expect(seed.constraints.length).toBeGreaterThan(0);
      expect(seed.ontology.name).toMatch(/heuristic/i);
    });
  });
