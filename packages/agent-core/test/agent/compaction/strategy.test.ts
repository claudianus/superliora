...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(28_998)).toBe(false);
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.16 → 16k; soft trigger=0.29 → 29k
    expect(strategy.shouldAsyncCompact(15_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(16_000)).toBe(true);
    expect(strategy.shouldCompact(16_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.29);
    expect(strategy.shouldCompact(28_998)).toBe(false);
    expect(strategy.shouldCompact(28_999)).toBe(true);
    expect(strategy.shouldBlock(59_999)).toBe(false);
    expect(strategy.shouldBlock(60_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(28_998)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(28_999)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.29);
