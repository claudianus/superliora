...
describe('DefaultCompactionStrategy', () => {
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.44 → 44k; soft trigger=0.55 → 55k
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.55);
    expect(strategy.shouldCompact(54_999)).toBe(false);
    expect(strategy.shouldCompact(55_000)).toBe(true);
    expect(strategy.shouldBlock(83_999)).toBe(false);
    expect(strategy.shouldBlock(84_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(54_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(55_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.55);
