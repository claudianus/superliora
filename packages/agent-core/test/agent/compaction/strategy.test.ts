...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(30_999)).toBe(false);
    expect(strategy.shouldBlock(55_000)).toBe(false);
    expect(strategy.shouldCompact(81_000)).toBe(true);
    expect(strategy.shouldBlock(61_999)).toBe(false);
    expect(strategy.shouldBlock(62_000)).toBe(true);
  });

  it('starts async compaction between the async threshold and soft trigger', () => {
    const strategy = new DefaultCompactionStrategy(() => 100_000);

    // asyncTriggerRatio=0.20 → 20k; soft trigger=0.31 → 31k
    expect(strategy.shouldAsyncCompact(19_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(20_000)).toBe(true);
    expect(strategy.shouldCompact(20_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.31);
    expect(strategy.shouldCompact(30_999)).toBe(false);
    expect(strategy.shouldCompact(31_000)).toBe(true);
    expect(strategy.shouldBlock(61_999)).toBe(false);
    expect(strategy.shouldBlock(62_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(30_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(31_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.31);
