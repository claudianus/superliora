...
describe('DefaultCompactionStrategy', () => {
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.48 → 48k; soft trigger=0.60 → 60k
    expect(strategy.shouldAsyncCompact(47_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(48_000)).toBe(true);
    expect(strategy.shouldCompact(48_000)).toBe(false);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(59_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(60_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.6);
