...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(21_999)).toBe(false);
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.10 → 10k; soft trigger=0.22 → 22k
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.22);
    expect(strategy.shouldCompact(21_999)).toBe(false);
    expect(strategy.shouldCompact(22_000)).toBe(true);
    expect(strategy.shouldBlock(55_999)).toBe(false);
    expect(strategy.shouldBlock(56_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(21_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(22_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.22);
