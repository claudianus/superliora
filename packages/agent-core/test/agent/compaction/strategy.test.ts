...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(34_999)).toBe(false);
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.24 → 24k; soft trigger=0.35 → 35k
    expect(strategy.shouldAsyncCompact(23_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(24_000)).toBe(true);
    expect(strategy.shouldCompact(24_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.35);
    expect(strategy.shouldCompact(34_999)).toBe(false);
    expect(strategy.shouldCompact(35_000)).toBe(true);
    expect(strategy.shouldBlock(63_999)).toBe(false);
    expect(strategy.shouldBlock(64_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(34_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(35_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.35);
