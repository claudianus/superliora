...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldBlock(83_999)).toBe(false);
    expect(strategy.shouldBlock(84_000)).toBe(true);
  });

  it('starts async compaction between the async threshold and soft trigger', () => {
    const strategy = new DefaultCompactionStrategy(() => 100_000);

    // asyncTriggerRatio=0.46 → 46k; soft trigger=0.56 → 56k
    expect(strategy.shouldAsyncCompact(45_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(46_000)).toBe(true);
    expect(strategy.shouldCompact(46_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.56);
    expect(strategy.shouldCompact(55_999)).toBe(false);
    expect(strategy.shouldCompact(56_000)).toBe(true);
    expect(strategy.shouldBlock(83_999)).toBe(false);
    expect(strategy.shouldBlock(84_000)).toBe(true);
  ...
  it('resolves block ratio above trigger when only trigger is configured', () => {
    expect(resolveCompactionBlockRatio(0.7)).toBe(0.84);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(55_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(56_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.56);
