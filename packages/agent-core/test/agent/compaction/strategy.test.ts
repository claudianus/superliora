...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(43_999)).toBe(false);
    expect(strategy.shouldBlock(55_000)).toBe(false);
    expect(strategy.shouldCompact(81_000)).toBe(true);
    expect(strategy.shouldBlock(71_999)).toBe(false);
    expect(strategy.shouldBlock(72_000)).toBe(true);
  });

  it('starts async compaction between the async threshold and soft trigger', () => {
    const strategy = new DefaultCompactionStrategy(() => 100_000);

    // asyncTriggerRatio=0.34 → 34k; soft trigger=0.44 → 44k
    expect(strategy.shouldAsyncCompact(33_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(34_000)).toBe(true);
    expect(strategy.shouldCompact(34_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.44);
    expect(strategy.shouldCompact(43_999)).toBe(false);
    expect(strategy.shouldCompact(44_000)).toBe(true);
    expect(strategy.shouldBlock(71_999)).toBe(false);
    expect(strategy.shouldBlock(72_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(43_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(44_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.44);
