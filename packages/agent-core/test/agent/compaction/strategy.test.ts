...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(46_999)).toBe(false);
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.36 → 36k; soft trigger=0.47 → 47k
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.47);
    expect(strategy.shouldCompact(46_999)).toBe(false);
    expect(strategy.shouldCompact(47_000)).toBe(true);
    expect(strategy.shouldBlock(73_999)).toBe(false);
    expect(strategy.shouldBlock(74_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(46_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(47_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.47);
