...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(35_999)).toBe(false);
    expect(strategy.shouldBlock(55_000)).toBe(false);
    expect(strategy.shouldCompact(81_000)).toBe(true);
    expect(strategy.shouldBlock(65_999)).toBe(false);
    expect(strategy.shouldBlock(66_000)).toBe(true);
  });

  it('starts async compaction between the async threshold and soft trigger', () => {
    const strategy = new DefaultCompactionStrategy(() => 100_000);

    // asyncTriggerRatio=0.26 → 26k; soft trigger=0.36 → 36k
    expect(strategy.shouldAsyncCompact(25_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(26_000)).toBe(true);
    expect(strategy.shouldCompact(26_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.36);
    expect(strategy.shouldCompact(35_999)).toBe(false);
    expect(strategy.shouldCompact(36_000)).toBe(true);
    expect(strategy.shouldBlock(65_999)).toBe(false);
    expect(strategy.shouldBlock(66_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(35_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(36_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.36);
