...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(20_999)).toBe(false);
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.08 → 8k; soft trigger=0.21 → 21k
    expect(strategy.shouldAsyncCompact(7_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(8_000)).toBe(true);
    expect(strategy.shouldCompact(8_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.21);
    expect(strategy.shouldCompact(20_999)).toBe(false);
    expect(strategy.shouldCompact(21_000)).toBe(true);
    expect(strategy.shouldBlock(53_999)).toBe(false);
    expect(strategy.shouldBlock(54_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(20_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(21_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.21);
