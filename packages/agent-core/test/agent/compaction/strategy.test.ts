...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(24_999)).toBe(false);
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.12 → 12k; soft trigger=0.25 → 25k
    expect(strategy.shouldAsyncCompact(11_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(12_000)).toBe(true);
    expect(strategy.shouldCompact(12_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.25);
    expect(strategy.shouldCompact(24_999)).toBe(false);
    expect(strategy.shouldCompact(25_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(24_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(25_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.25);
