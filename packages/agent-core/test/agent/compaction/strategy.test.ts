...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(38_999)).toBe(false);
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.28 → 28k; soft trigger=0.39 → 39k
    expect(strategy.shouldAsyncCompact(27_999)).toBe(false);
    expect(strategy.shouldAsyncCompact(28_000)).toBe(true);
    expect(strategy.shouldCompact(28_000)).toBe(false);
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.39);
    expect(strategy.shouldCompact(38_999)).toBe(false);
    expect(strategy.shouldCompact(39_000)).toBe(true);
    expect(strategy.shouldBlock(67_999)).toBe(false);
    expect(strategy.shouldBlock(68_000)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(38_999)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(39_000)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.39);
