...
describe('DefaultCompactionStrategy', () => {
  ...
  it('uses reserved context only for hard blocking on smaller windows', () => {
    ...
    expect(strategy.shouldCompact(1_004)).toBe(false);
  ...
  it('starts async compaction between the async threshold and soft trigger', () => {
    ...
    // asyncTriggerRatio=0.01 → 1k; soft trigger=0.01005 → 1.005k
  ...
  it('uses decoupled default trigger and block ratios', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBe(0.01005);
    expect(strategy.shouldCompact(1_004)).toBe(false);
    expect(strategy.shouldCompact(1_005)).toBe(true);
    expect(strategy.shouldBlock(1_029)).toBe(false);
    expect(strategy.shouldBlock(1_030)).toBe(true);
  ...
  it('speculatively compacts only when projected usage crosses the soft trigger', () => {
    ...
    expect(strategy.shouldSpeculativelyCompact(1_004)).toBe(false);
    expect(strategy.shouldSpeculativelyCompact(1_005)).toBe(true);
  ...
  it('tightens the trigger threshold only after emergency backstop compactions', () => {
    ...
    expect(strategy.effectiveTriggerRatio).toBeLessThan(0.01005);
