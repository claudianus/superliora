...
describe('MicroCompaction', () => {
  ...
  it('clears cutoff on full compaction', () => {
    const ctx = testAgent({
      microCompaction: {
        keepRecentMessages: 2,
        minContentTokens: 1,
    ...
    // Establish a micro cutoff; full apply must reset it so projection does not
    // keep masking the rewritten post-summary history.
    ctx.agent.microCompaction.apply(ctx.agent.context.history.length);
    expect((ctx.agent.microCompaction as { cutoff: number }).cutoff).toBeGreaterThan(0);

    ctx.agent.context.applyCompaction({
      summary: 'Summary.',
      contextSummary: '# SuperLiora Context Compaction v2 Memory\n\nSummary.',
      compactedCount: 2,
      tokensBefore: 100,
      tokensAfter: 20,
    });

    expect((ctx.agent.microCompaction as { cutoff: number }).cutoff).toBe(0);
  });
