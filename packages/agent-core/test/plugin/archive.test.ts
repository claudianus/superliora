...
describe('extractZip', () => {
  ...
  it('prefers a plugin manifest at the archive root', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: '.claude-plugin/plugin.json', data: '{"name":"root"}' },
      { name: 'examples/demo/.claude-plugin/plugin.json', data: '{"name":"demo"}' },
    ]);
  ...
  it('detects plugin root with .claude-plugin/plugin.json', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'my-plugin/.claude-plugin/plugin.json', data: '{"name":"test"}' },
  ...
  it('detects plugin root via autodiscover layout (skills/)', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
  ...
  it('detects a single wrapper directory before nested manifests', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'outer/.claude-plugin/plugin.json', data: '{"name":"outer"}' },
      { name: 'outer/inner/.claude-plugin/plugin.json', data: '{"name":"inner"}' },
    ]);
