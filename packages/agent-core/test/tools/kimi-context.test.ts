...
import {
  KimiContextInputSchema,
  KimiContextTool,
} from '../../src/tools/builtin/context/kimi-context';
...
describe('KimiContextTool', () => {
  it('builds a compact evidence packet instead of dumping whole files', async () => {
    ...
    const tool = new KimiContextTool(
      makeKaos({ '/workspace/apps/kimi-code/src/tui/commands/ultrawork-contract.ts': file }),
      workspace,
    );

    expect(KimiContextInputSchema.safeParse({ query: 'Kimi Lean Context' }).success).toBe(true);
  ...
  it('rejects scans that would read sensitive explicit files', async () => {
    const tool = new KimiContextTool(makeKaos({ '/workspace/.env': 'SECRET=1\n' }), workspace);
  ...
  it('keeps automatic query scans focused on source instead of workflow artifacts', async () => {
    const tool = new KimiContextTool(
      makeKaos({
        ...
        '/workspace/.changeset/ultrawork-flow.md': 'KimiContext release note\n',
        '/workspace/packages/agent-core/src/tools/builtin/context/kimi-context.ts': [
          'export class KimiContextTool {',
          '  readonly name = "KimiContext";',
          '}',
        ].join('\n'),
      }),
      workspace,
    ...
    const result = await executeTool(tool, context({ query: 'KimiContext', max_files: 1 }));
    const output = toolContentString(result);
