...
describe('GrepTool', () => {
  ...
  describe('column cap by output mode', () => {
    it('does not cap columns in content output mode so long matching lines are returned in full', async () => {
      ...
      await executeTool(tool, context({ pattern: 'hit', output_mode: 'content', head_limit: 0 }));
  ...
  it('does not prefilter public key files that the sensitive policy allows', async () => {
    ...
    const result = await executeTool(tool, context({ pattern: 'hit', output_mode: 'content', head_limit: 0 }));
  ...
  it('reconstructs null-delimited content context separators for display', async () => {
    ...
    const result = await executeTool(tool, context({ pattern: 'hit', output_mode: 'content', '-C': 1, head_limit: 0 }));
  ...
  it('uses the backend path class for Windows content output without line numbers', async () => {
    ...
    const result = await executeTool(tool,
      context({ pattern: 'hit', output_mode: 'content', '-n': false, head_limit: 0 }),
    );
  ...
  it('filters sensitive context lines when content output omits line numbers', async () => {
    ...
    const result = await executeTool(tool,
      context({ pattern: 'hit', output_mode: 'content', '-n': false, '-C': 1, head_limit: 0 }),
    );
  ...
  it('limits grep output to 20 lines by default', async () => {
    ...
    const bodyLines = lines.filter((line) => !/truncated|offset=/i.test(line));
    expect(bodyLines.slice(0, 20)).toEqual(displayPaths.slice(0, 20));
    expect(output).not.toContain(displayPaths[20]);
    expect(output).toMatch(/truncated|offset=/i);
  ...
  it('drops incomplete trailing null-delimited content records after timeout', async () => {
    ...
    const resultPromise = executeTool(tool, context({ pattern: 'slow', output_mode: 'content', head_limit: 0 }));
    await vi.advanceTimersByTimeAsync(20_000);
    const result = await resultPromise;
  ...
  it('summarizes count output before pagination and after sensitive filtering', async () => {
    ...
    // densify char-cap may clip pagination/rows; header-first total must survive
    const body = toolContentBody(result);
    expect(body).toContain('Found 11 total non-sensitive occurrences across 3 files.');
    expect(body).toMatch(/Results truncated|truncated/);
  });
  ...
  it('includes lines before the match for -B without -C', async () => {
    ...
    const result = await executeTool(tool,
      context({ pattern: 'TestClass', output_mode: 'content', '-B': 2, head_limit: 0 }),
    );
  ...
  it('includes lines after the match for -A without -C', async () => {
    ...
    const result = await executeTool(tool,
      context({ pattern: 'TestClass', output_mode: 'content', '-A': 2, head_limit: 0 }),
    );
  ...
  it('appends the count-mode summary and pagination to the model-visible output', async () => {
    ...
    const output = toolContentBody(result);
    // densify char-cap may clip path rows; header-first total + pagination cue must survive
    expect(output).toContain('Found 30 total occurrences across 10 files.');
    expect(output).toMatch(/Results truncated to 3 lines|truncated/);
