...
describe('GrepTool', () => {
  ...
  it('limits grep output to 50 lines by default', async () => {
    ...
    expect(lines.slice(0, 50)).toEqual(displayPaths.slice(0, 50));
    expect(output).not.toContain(displayPaths[50]);
    expect(output).toContain(
      'Results truncated to 50 lines (total: 251). Use offset=50 to see more.',
    );
  });

  it('treats head_limit zero as unlimited', async () => {
    // Keep under tool-result budget so unlimited head_limit is observable.
    const paths = Array.from({ length: 40 }, (_, index) => `/workspace/src/${String(index)}.ts`);
    const displayPaths = Array.from({ length: 40 }, (_, index) => `src/${String(index)}.ts`);
    const stdout = [...paths, ''].join('\n');
    const tool = new GrepTool(
      createFakeKaos({ exec: vi.fn().mockResolvedValue(processWithOutput(stdout)) }),
      { workspaceDir: '/workspace', additionalDirs: [] },
