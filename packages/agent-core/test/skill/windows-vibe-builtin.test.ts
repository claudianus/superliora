...
describe('windows-vibe builtin skill', () => {
  it('registers a playbook that points at /host-setup', () => {
    expect(WINDOWS_VIBE_SKILL.name).toBe('windows-vibe');
    expect(WINDOWS_VIBE_SKILL.content).toContain('/host-setup');
    ...
    expect(WINDOWS_VIBE_SKILL.content).toContain('~/.zshrc');
