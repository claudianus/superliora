#!/usr/bin/env node
/**
 * Fetches the external Agent Skill catalogs and materializes them into
 * `packages/agent-core/src/skill/catalog/` as a build-time artifact
 * (gitignored — the repo does not vendor the external skill content).
 *
 * Runs from packages/agent-core so `js-yaml` resolves from this package's
 * dependencies. Catalog absence is a supported state: SearchSkill falls back
 * to builtin skills until this script runs.
 */
...
const skillDir = join(import.meta.dirname, '..', 'src', 'skill');
const outDir = join(skillDir, 'catalog');
...
async function writeCatalogSearchIndex(targetDir) {
  ...
  for (const entry of entries.toSorted((a, b) => a.name.localeCompare(b.name))) {
    ...
    const skillMd = join(targetDir, entry.name, 'SKILL.md');
    try {
      const text = await readFile(skillMd, 'utf8');
  ...
  const outPath = join(skillDir, 'catalog-search-index.json');
