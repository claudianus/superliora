...
| `fleet/` | Swarm / subagent orchestration home (`#/collaboration` is an alias) |
...
`Manager` suffixes are allowed under `agent/` / `session/` / `fleet/` / `mcp/` / `plugin/`. They are **banned** under `services/` (use `Service`).
