...
describe('formatHostErrorMessage', () => {
  it('includes the rejected host and allow guidance', () => {
    expect(formatHostErrorMessage('APP.Example.com:443')).toBe(
      "Invalid Host header: app.example.com; allow this host with SUPERLIORA_ALLOWED_HOSTS=app.example.com or 'liora server run --allowed-host app.example.com'.",
    );
  });
});
...
describe('parseAllowedHosts', () => {
  it('splits, trims, and drops empties', () => {
    expect(parseAllowedHosts({ SUPERLIORA_ALLOWED_HOSTS: ' a, .b.com, ' })).toEqual(['a', '.b.com']);
  });
...
describe('isHostCheckDisabled', () => {
  it('is true when set to "1"', () => {
    expect(isHostCheckDisabled({ SUPERLIORA_DISABLE_HOST_CHECK: '1' })).toBe(true);
  });
...
describe('createHostCheck (onRequest hook)', () => {
  ...
  it('rejects a disallowed Host with the 40301 envelope', async () => {
    ...
    expect(body['msg']).toBe(
      "Invalid Host header: evil.com; allow this host with SUPERLIORA_ALLOWED_HOSTS=evil.com or 'liora server run --allowed-host evil.com'.",
    );
