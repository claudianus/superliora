 ...
 * `host: '0.0.0.0'` + `SUPERLIORA_PASSWORD` + `insecureNoTls: true` server:
...
beforeEach(() => {
  prevPassword = process.env['SUPERLIORA_PASSWORD'];
});
...
afterEach(async () => {
  ...
  if (prevPassword === undefined) {
    delete process.env['SUPERLIORA_PASSWORD'];
  } else {
    process.env['SUPERLIORA_PASSWORD'] = prevPassword;
  }
});
...
describe('non-loopback bind gate (M6.3)', () => {
  ...
  it('refuses to bind 0.0.0.0 with a password but without --insecure-no-tls', async () => {
    process.env['SUPERLIORA_PASSWORD'] = 'test-pw';
    const { lockPath, homeDir } = tmpPaths();
...
describe('public-bind §3.5 end-to-end (M6.7)', () => {
   ...
   * the password `verifyPassword` path is exercised. `SUPERLIORA_PASSWORD` is set
