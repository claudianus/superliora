...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'liora-server-meta-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'liora-server-meta-home-'));
});
...
describe('GET /api/v1/meta — envelope + metaResponseSchema', () => {
  ...
  it('two independent daemons get distinct daemon_ids', async () => {
    ...
    const homeA = mkdtempSync(join(tmpdir(), 'liora-server-meta-home-a-'));
    const homeB = mkdtempSync(join(tmpdir(), 'liora-server-meta-home-b-'));
