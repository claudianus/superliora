...
import type {
  RendererColorMode,
  RendererCursorMotionMode,
  RendererCursorMotionMetrics,
  RendererCursorMoveKind,
  RendererCursorShape,
  RendererCursorState,
  RendererTerminalOutputOptions,
} from './output';
...
export function cursorStateToAnsiFromPosition(
  cursor: RendererCursorState,
  originX: number,
  originY: number,
...
export function stripTerminalEscapeSequences(text: string): string {
...
export function normalizeOrigin(value: number | undefined): number {
...
export function createCursorMotionMetrics(): MutableRendererCursorMotionMetrics {
  return {
    absoluteMoves: 0,
    relativeMoves: 0,
...
export function snapshotCursorMotionMetrics(
  metrics: MutableRendererCursorMotionMetrics,
): RendererCursorMotionMetrics {
  return { ...metrics };
}

export function recordCursorMoveMetrics(
  metrics: MutableRendererCursorMotionMetrics,
...
export function cursorPositionForManagedCursor(
...
export function resolveEraseLineStartIndex(
...
export function rendererRunCellWidth(cells: readonly { readonly width?: number; readonly continuation?: boolean }[]): number {
...
export function hasTerminalOutput(
...
export function normalizeHyperlink(link: string | undefined): string | undefined {
  if (link === undefined) return undefined;
  const normalized = link.replaceAll(/[\u0000-\u001F\u007F]/g, '');
  return normalized.length === 0 ? undefined : normalized;
}

export function isSafePrintableCluster(cluster: string): boolean {
...
export function parseHexColor(color: string | undefined): { r: number; g: number; b: number } | undefined {
...
export function pushColorParams(
  params: string[],
  target: 'fg' | 'bg',
