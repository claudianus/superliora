...
import { createLioraHarness, type Event, type LioraError } from '#/index';
...
describe('Session plan, compact, usage, and resume APIs', () => {
  ...
  it.todo('marks resumed plan mode active when the restored plan has no plan data', async () => {
    ...
    await writeTestConfig(homeDir);
    const createdHarness = createLioraHarness({ homeDir, identity: TEST_IDENTITY });
    let sessionId = '';
    let sessionDir = '';
    ...
    const resumedHarness = createLioraHarness({ homeDir, identity: TEST_IDENTITY });
    try {
