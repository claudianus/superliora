...
import { createLioraHarness, type LioraHarness } from '@superliora/sdk';
...
async function main(): Promise<void> {
  ...
  try {
    if (forceLogin) {
      await harness.auth.logout(MANAGED_SUPERLIORA_PROVIDER);
    ...
    const status = await harness.auth.status(MANAGED_SUPERLIORA_PROVIDER);
    const usage = await harness.auth.getManagedUsage(MANAGED_SUPERLIORA_PROVIDER);
    ...
    if (config.providers[MANAGED_SUPERLIORA_PROVIDER]?.oauth?.key !== 'oauth/kimi-code') {
    ...
    if (!keepToken) {
      await harness.auth.logout(MANAGED_SUPERLIORA_PROVIDER).catch(() => {});
    }
    await harness.close();
...
function printUsage(usage: Awaited<ReturnType<LioraHarness['auth']['getManagedUsage']>>): void {
  if (usage.kind === 'error') {
