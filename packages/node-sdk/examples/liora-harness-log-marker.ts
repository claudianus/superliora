...
Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     Kimi home dir; defaults to SUPERLIORA_HOME or ~/.superliora
...
async function main(): Promise<void> {
  ...
  const resolvedHome = resolveLioraHome(options.homeDir);
  ...
  try {
    ...
    sessionLogPath = join(sessionDir, 'logs', 'liora.log');
    const payload = {
  ...
  if (!sessionMatched) {
    process.stderr.write(
      [
        ...
        'Check that KIMI_LOG_LEVEL is not "off" and that the session id exists in this SUPERLIORA_HOME.',
        '',
      ].join('\n'),
    );
    process.exitCode = 1;
