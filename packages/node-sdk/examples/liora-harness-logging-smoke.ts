...
import { createLioraHarness, log } from '@superliora/sdk';

const SESSION_LOG = 'logs/liora.log';
const GLOBAL_LOG = 'logs/global/liora.log';
const MAIN_WIRE = 'agents/main/wire.jsonl';
const TEST_HOME = join(homedir(), '.superliora-test');
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'liora.log' || file.startsWith('liora.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('liora.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('liora.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  process.env['SUPERLIORA_HOME'] = TEST_HOME;
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        '  - logs/liora.log',
        '  - logs/liora.log.1',
        `  - ${summary.sessionDir}/logs/liora.log`,
        `  - ${summary.sessionDir}/logs/liora.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - liora.log.1 exists',
        '  - liora.log.2 does not exist',
        '',
        'Export evidence:',
        '  - logging-session.zip includes session logs and wire.jsonl',
        '  - logging-session.zip does not include logs/global/liora.log',
        '  - logging-with-global.zip includes only the active global log at logs/global/liora.log',
        '  - global rotated logs such as ~/.superliora-test/logs/liora.log.1 are intentionally not bundled',
        '',
        'Redaction evidence:',
