...
- Updated `LioraCallgraph` and `Expand` descriptions to no longer reference `LioraSearch`.
- Removed or updated tests that expected `LioraSearch`/`LioraIndex` to be available.
- The remaining `LioraRead`, `LioraSymbol`, `LioraCallgraph`, `LioraTree`, and `Expand` tools continue to use the SQLite/tree-sitter index via `ensureWorkspaceIndexBudgeted`.
