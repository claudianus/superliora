...
::: tip Path note
Child processes launched from an IDE GUI on macOS typically do **not** inherit the terminal shell's `PATH`. If `liora` is not in a system directory like `/usr/local/bin`, use the absolute path in your IDE configuration. Run `which liora` in a terminal to find the active path.
:::
...
- `command`: path to the SuperLiora CLI executable. If `liora` is not on `PATH`, use the full path (e.g. `/Users/you/.local/bin/liora`).
...
JetBrains is strict about the `command` field — always use an **absolute path**, which you can get by running `which liora` in a terminal. After saving, `SuperLiora CLI` will appear in the AI chat's agent selector.
...
- [liora command reference](../reference/liora-command.md) — full subcommand list
