...
[Model Context Protocol（MCP）](https://modelcontextprotocol.io/) 是一个开放协议，让模型可以安全地调用外部进程或服务暴露的工具——例如读取 GitHub issues、查询数据库、操作本地文件系统。SuperLiora CLI 作为 MCP client 接入这些外部工具，并把它们与内置工具（`Read`、`Bash`、`Grep` 等）一起暴露给 Agent 使用，行为上没有差异。

## 接入方式

SuperLiora CLI 支持三种 MCP server 接入方式：
...
- **用户级**：`~/.superliora/mcp.json`（或 `$SUPERLIORA_HOME/mcp.json`），跨项目共享
- **项目级**：工作目录下的 `.superliora/mcp.json`，只对当前仓库生效
...
::: warning 注意
项目级 `.superliora/mcp.json` 中的 stdio 条目会在会话启动时执行本地命令，只在你信任的仓库里启用。
:::
