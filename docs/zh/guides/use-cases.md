...
本页收录 SuperLiora CLI 的典型使用场景和配套的 prompt 示例，可以直接复制使用或按需修改。

## 理解陌生项目

接手陌生仓库时，建议先用 `kimi --plan` 输出只读调研计划。在 TUI 中，只有当你希望进入 Ultrawork mode，让系统先访谈、设定可验证目标并推进到验证时，才按 `Shift-Tab`。
...
需要留档或复盘时，用 `liora export <sessionId>` 打包为 ZIP，或在 TUI 中用 `/export-md` 导出为可读的 Markdown 对话记录。
