...
日常操作优先用 **Command Hub** 仪表盘：按 `Ctrl-K`，或在空提示符按 `?`（或运行 `/help`）。输入即可模糊搜索所有操作、设置与斜杠命令，匹配的字母会在列表中高亮。在初始列表按 Space 就地切换所选模式；输入后 Space 与数字并入搜索词。Enter 应用并回到对话。嵌套选择器支持 Esc 返回（有筛选时 Esc 先清空）。重试在 Hub → Chat（或 `/retry`）。
...
| `/swarm talk [expert]` | — | 打开 War Room expert 选择器（或直接进入指定 expert）以查看 transcript 并发送消息。进行中的 expert 使用 steer，空闲则使用 prompt。 | War Room 活跃时可用 |
| `/swarm msg <expert> <text>` | — | 不打开选择器，直接向 War Room expert 发送消息。 | War Room 活跃时可用 |
| `/swarm pause\|restaff\|raw` | — | 控制当前 Fleet War Room（暂停转向、请求 restaff、切换原始 feed）。 | War Room 活跃时可用 |
...
`/goal` 用于开始或管理目标模式：SuperLiora 会在自动续跑的轮次中持续朝一个持久目标工作。使用指导和示例见[使用目标模式](../guides/goals.md)。
...
```sh
liora -p "/goal 修复 checkout 测试失败"
```

Prompt 模式在目标完成时以退出码 `0` 退出，在目标阻塞时以 `3` 退出，在目标暂停时以 `6` 退出。其它 `/goal` 子命令，包括 `next`，都是 TUI 控制命令，不由 `liora -p` 处理。
...
| `/version` | — | 显示 SuperLiora CLI 版本号 | 是 |

## 退出

| 命令 | 别名 | 说明 | 随时可用 |
| --- | --- | --- | --- |
| `/exit` | `/quit`、`/q` | 退出 SuperLiora CLI | 否 |

## 内置 Skill 命令

SuperLiora CLI 随包内置了一组 Skill，直接以 `/<name>` 形式出现在斜杠命令面板中。与外部 Skill 不同，它们不需要 `skill:` 前缀，开箱即用。
...
SuperLiora CLI 随包内置的 Skill 会直接以 `/<name>` 形式出现在斜杠命令面板中。例如，`/mcp-config` 用于配置 MCP server 和处理 MCP OAuth 登录，`/custom-theme [附加文本]` 用于进入自定义主题流程，创建或编辑 TUI 主题。
