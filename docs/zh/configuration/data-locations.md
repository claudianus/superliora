...
SuperLiora CLI 把所有运行时数据——配置文件、会话历史、登录凭据、诊断日志——集中存放在 `~/.superliora/` 下。本页帮你搞清楚每类数据在哪里、用来做什么，以及需要时怎么清理或搬迁。

## 数据根目录

默认数据根是 `~/.superliora/`，在不同平台的实际路径：

- macOS：`/Users/<name>/.superliora`
- Linux：`/home/<name>/.superliora`
- Windows：`C:\Users\<name>\.superliora`

如果你需要把数据目录挪到别处（比如用多个独立环境隔离不同项目的配置），设置 `SUPERLIORA_HOME` 即可：

```sh
export SUPERLIORA_HOME="$HOME/.config/kimi-code"
```

设置后，配置、会话、日志、OAuth 凭据、Kimi 专属用户级 Skills、全局 `AGENTS.md` 等 **SuperLiora 数据**都会落到新路径下。`SUPERLIORA_HOME` 的完整说明见[环境变量](./env-vars.md)。

::: tip 提示

**通用 `.agents` 资源**仍放在真实 OS home 下，以便跨工具共享。例如，用户级通用 Skills 仍位于 `~/.agents/skills/`，而 Kimi 专属用户级 Skills 会随 `SUPERLIORA_HOME` 移动到 `$SUPERLIORA_HOME/skills/`。
:::

## 目录结构

```
$SUPERLIORA_HOME  （默认 ~/.superliora）
├── config.toml             # 用户配置
...
- **`AGENTS.md`**：全局 Kimi 专属 Agent 指令。该文件会随 `SUPERLIORA_HOME` 移动；跨工具通用指令仍可放在 `~/.agents/AGENTS.md`。
- **`mcp.json`**：用户级 MCP server 声明，启动时与项目内的 `.superliora/mcp.json` 合并加载。详见 [MCP](../customization/mcp.md)。
...
- **`logs/liora.log`**：该会话的诊断日志，只有发生诊断事件时才存在。
...
- **`logs/liora.log`**（全局）：记录启动、登录、导出等跨会话事件。
- **`<sessionDir>/logs/liora.log`**（会话级）：记录单个会话内的诊断事件。

报 bug 时，优先用 `liora export` 导出相关会话（详见 [kimi 命令](../reference/liora-command.md)）；会话日志默认包含在导出包里。不想分享全局日志时加 `--no-include-global-log`。
...
删除数据根目录（`~/.superliora/` 或 `SUPERLIORA_HOME` 指定路径）可清除所有运行时数据。只需清理部分内容时：
...
| 强制重新下载托管 `rg` 和 `fd` | 删除 `~/.superliora/bin/` |
| 清除供应商 OAuth 登录态 | 运行 `/logout`，或删除对应的 `credentials/<name>.json` |
| 清除 MCP server OAuth 登录态 | 删除 `credentials/mcp/`（`/logout` 不会清理 MCP 凭据） |
| 移除用户级 MCP 声明 | 删除 `$SUPERLIORA_HOME/mcp.json`（默认为 `~/.superliora/mcp.json`） |
| 清理全局 Kimi 专属 Agent 指令 | 删除 `$SUPERLIORA_HOME/AGENTS.md`（默认为 `~/.superliora/AGENTS.md`） |
| 清理 plugin 安装记录 | 删除 `$SUPERLIORA_HOME/plugins/`（本地 plugin 源码不受影响） |
| 清空 Kimi 专属用户级 Skills | 删除 `$SUPERLIORA_HOME/skills/`（默认为 `~/.superliora/skills/`） |

## 下一步

- [配置文件](./config-files.md) — `config.toml` 各字段的完整说明
- [环境变量](./env-vars.md) — `SUPERLIORA_HOME` 等路径变量的详细用法
