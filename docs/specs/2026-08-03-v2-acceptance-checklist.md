...
> **상태 변경 (2026-08-04, 운영자 결정):** 전체화면 컨트롤 타워 테이크오버는 폐기. Job Desk는 트랜스크립트 내 칸반 패널(`JobDeskPanelComponent`, `jobs` 리전)로 대체됐고 V5-1의 "보드 기본 화면" 요건은 더 이상 적용되지 않는다. V5-3의 스토어 단일 소스(`JobBoardStore`)는 유지된다.
