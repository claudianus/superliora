...
> **상태 변경 (2026-08-04, 운영자 결정):** 전체화면 컨트롤 타워 테이크오버(부팅 기본 화면 포함)는 폐기됐다. Job Desk는 이제 트랜스크립트 화면 안의 칸반 패널(Todo 보드 아래 `jobs` 크롬 슬롯)로 렌더되며, 프롬프트 입력창과 트랜스크립트는 항상 유지된다. `/jobs board`는 패널 표시/숨김 토글이다. 아래 §5의 "전용 렌더 경로·보드 제1 서피스" 규정은 이 결정으로 대체된다.
