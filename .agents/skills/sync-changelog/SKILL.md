---
name: sync-changelog
description: Use after a release succeeds, when maintainers need to sync apps/liora/CHANGELOG.md into docs/en/release-notes/changelog.md and docs/zh/release-notes/changelog.md, then open a PR on a dedicated branch.
---
...
`kimi-code` uses changesets for versioning. Each package gets its own `CHANGELOG.md`. The user-facing CLI package, `@superliora/liora`, writes its changelog here:

```text
apps/liora/CHANGELOG.md
```
...
Note: `docs/` is an unpublished bilingual reference tree, not the deployed public site (`apps/site/` is — see `docs/AGENTS.md`). This skill keeps `docs/en/release-notes/changelog.md` / `docs/zh/release-notes/changelog.md` accurate for anyone reading the repo directly; it does not publish anything.
...
- The top of `apps/liora/CHANGELOG.md` contains version blocks that are not yet in `docs/en/release-notes/changelog.md`.
- The `gen-docs` flow does not run this automatically; maintainers must explicitly do it after release.

Do **not** run this before the Release PR is merged. At that point, changesets has not yet written the new version into `apps/liora/CHANGELOG.md`.

## Source And Targets

| File | Role | Edited by |
|---|---|---|
| `apps/liora/CHANGELOG.md` | **Only upstream source**, generated by changesets | Never edit manually |
...
- The released version exists on npm (`npm view @superliora/liora versions --json`) or has a matching GitHub Release tag.
- The top of `apps/liora/CHANGELOG.md` is that new version.
...
```bash
rg '^## ' apps/liora/CHANGELOG.md | head -5
...
- The upstream H1 `# @superliora/liora` because the docs page already has `# Changelog`.
...
```bash
git log -1 --format=%cs "@superliora/liora@<version>"
```
...
```text
docs(changelog): sync <version range> from apps/liora/CHANGELOG.md
```
...
- Synced `<version range>` from `apps/liora/CHANGELOG.md` into `docs/en/release-notes/changelog.md`
...
- Never edit upstream `apps/liora/CHANGELOG.md`.
...
| Mistake | Fix |
|---|---|
| Adding entries directly to the English docs page without reading upstream | Use `apps/liora/CHANGELOG.md` as the source |
...
- The top version in `apps/liora/CHANGELOG.md` is not published on npm or GitHub Releases.
- You are about to edit `apps/liora/CHANGELOG.md`.
