...
description: "One-pass job-desk wake routing for Conductor: inbox, inspect only decision states, trust ledger done-claims, chain follow-ups, stop."
...
2. **JobInspect 1** — inspect only needs_user / failed / blocked (not every unread / historical done, not a second pass on a stamped review).
3. **Route:**
   - `needs_user` → AskUserQuestion with evidence; stop.
   - done-claim vs brief: trust ledger fields (verification, result, sha, verifyVerdict). Do not JobInspect again or spawn a nested verify Job. If staffed wrong / no code / checks not run but claim is incomplete → JobCreate reframe with parent_job_id and smaller scope. Do not re-verify by running tests yourself.
