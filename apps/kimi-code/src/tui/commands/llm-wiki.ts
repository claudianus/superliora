...
export const LLM_WIKI_ROOT = '.super-kimi/wiki';
export const LLM_WIKI_INDEX_PATH = `${LLM_WIKI_ROOT}/index.md`;
export const LLM_WIKI_MANIFEST_PATH = `${LLM_WIKI_ROOT}/manifest.json`;
...
export function writeProjectLlmWikiSeed(workDir: string, input: LlmWikiSeedInput): LlmWikiArtifacts {
  const artifacts: LlmWikiArtifacts = {
    wikiRootPath: LLM_WIKI_ROOT,
    wikiIndexPath: LLM_WIKI_INDEX_PATH,
    wikiManifestPath: LLM_WIKI_MANIFEST_PATH,
    wikiRunPath: `${LLM_WIKI_ROOT}/runs/${input.runId}.md`,
  };
  mkdirSync(join(workDir, LLM_WIKI_ROOT, 'runs'), { recursive: true });
...
export function loadLlmWikiStatus(workDir: string): LlmWikiStatus {
  const rootPath = join(workDir, LLM_WIKI_ROOT);
  const indexPath = join(workDir, LLM_WIKI_INDEX_PATH);
  const manifestPath = join(workDir, LLM_WIKI_MANIFEST_PATH);
...
function readExistingManifest(workDir: string): LlmWikiManifest | undefined {
  const path = join(workDir, LLM_WIKI_MANIFEST_PATH);
...
This project-local wiki stores human-reviewable, source-backed Ultrawork knowledge. Code remains the source of truth. Kimi Recall remains the global searchable memory and should only receive concise durable facts, decisions, or user preferences.
...
- Store project-specific reviewable knowledge under ${LLM_WIKI_ROOT}.
...
- Kimi Knowledge Map: ${input.evidenceFiles.knowledgeMapPath}
