...
import type { MemorySearchResult, MemoryStats } from '@moonshot-ai/kimi-code-sdk';
...
const HUMAN_WRITING_CONTRACT_PHRASES = [
  ...
  'product UX microcopy uses friendly 해요체',
  'positive-first recovery',
  'specific CTAs',
  'institutional corporate copy uses formal 합니다/습니다',
  'proof before emotion',
  'future-facing continuity',
  'style-analysis inputs only',
  'do not copy source passages',
  ...
  'reread the result for changed meaning',
] as const;
...
export function loadPreflightRefreshRun(workDir: string): PreflightRefreshRun | undefined {
  const summaryPath = join(workDir, PREFLIGHT_REFRESH_EVIDENCE_ROOT, 'summary.json');
...
export function loadPreflightLoopRun(workDir: string): PreflightLoopRun | undefined {
  const loopEvidenceRoot = join(workDir, PREFLIGHT_BENCH_LOOP_EVIDENCE_ROOT);
...
function loopEvidenceSummary(status: PreflightStatus, nowMs = Date.now()): string {
  if (shouldRunBenchLoop(status, nowMs)) return PREFLIGHT_BENCH_LOOP_EVIDENCE_ROOT;
...
function loopRerunSummary(run: PreflightLoopRun | undefined): string {
  if (run === undefined) return `${PREFLIGHT_BENCH_LOOP_COMMAND} --evidence-root ${PREFLIGHT_BENCH_LOOP_EVIDENCE_ROOT}/latest`;
  if (run.warning !== undefined) return `inspect_loop_summary ${run.evidencePath}`;
...
function loopInspectSummary(run: PreflightLoopRun | undefined): string {
  if (run === undefined) return `${PREFLIGHT_BENCH_LOOP_EVIDENCE_ROOT}/latest/loop-summary.json`;
  if (run.warning !== undefined) return `inspect_loop_summary ${run.evidencePath}`;
