...
export function loadBenchStatus(workDir: string, args: string): BenchStatus {
  ...
  const root = requestedPath.length > 0
    ...
    : join(workDir, DEFAULT_BENCH_ROOT);
...
function loadCandidateStatus(workDir: string, path: string): CandidateStatus | null {
  ...
  if (record['benchmark'] === 'super-kimi-agent-bench-loop-replay') {
    return withSourceDisplayPath(workDir, normalizeReplaySummary(workDir, path, record));
  }

  if (record['benchmark'] === 'super-kimi-agent-bench-loop' || record['bestScore'] !== undefined) {
...
function normalizeQualityGate(workDir: string, path: string, data: unknown): CandidateStatus | null {
  ...
  if (gateStatus === null) {
    return {
      ...
      nextAction: 'Run node scripts/kimi-agent-bench.mjs to refresh local benchmark evidence.',
...
function budgetRerunCommand(record: Record<string, unknown>): string | undefined {
  ...
  return `node scripts/kimi-agent-bench.mjs --task-dir ${taskDir} --suite ${suite} --runner ${runner} --evidence-root ${evidenceRoot}`;
}
...
function loopReplayCommand(
  ...
  return `node scripts/kimi-agent-bench.mjs --replay-summary ${quoteBenchShellArg(sourceDisplayPath)} --evidence-root ${quoteBenchShellArg(replayRoot)}`;
}
...
function hasReplayableRerunArgv(value: unknown): boolean {
  ...
  if (value[0] !== 'node' || value[1] !== 'scripts/kimi-agent-bench.mjs') return false;
...
function nextActionFor(preflight: string | undefined, missingEnv: readonly string[]): string {
  if (preflight === 'blocked_missing_env' || missingEnv.length > 0) {
    return `Set ${missingEnv.join('/')} and rerun node scripts/kimi-agent-bench.mjs --suite holdout --runner provider.`;
  }
