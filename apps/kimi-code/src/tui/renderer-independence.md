...
- `KimiTUI` treats the root `TUI` as both the render tree and input/focus host.
- 84 source files under `apps/kimi-code/src/tui` and `apps/kimi-code/src/migration`
...
- `KimiTUI` no longer calls backend-specific root APIs directly except through
  the facade.
...
or creates a `TerminalRenderer` bundle, and `KimiTUI` startup/shutdown uses the
...
supply Super Kimi Code canvas painting/cache policy and preserve existing call
...
owns the native renderer core independently from the Kimi Code app, and the app
...
still reuse the existing editor `handleInput` path during migration. `KimiTUI`'s
...
(`KIMI_CODE_EXPERIMENTAL_NATIVE_RENDERER`): the renderer facade keeps the
...
`KIMI_CODE_NATIVE_RENDERER_DIAGNOSTICS=1` or toggled live with
