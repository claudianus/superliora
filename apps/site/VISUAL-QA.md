...
Run `pnpm -C apps/site run catalog:sync` when the provider numbers look stale, then
`pnpm -C apps/site run build && pnpm -C apps/site exec vite preview --host 127.0.0.1 --port 4176`,
