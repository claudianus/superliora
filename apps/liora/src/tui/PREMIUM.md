...
The session prompt editor (`native-tui-editor`) is the standing input
box. Under ambient motion its rounded `╭╮╰╯` perimeter runs a clockwise
comet chase (≥4-cell trail) and a slow primary↔accent breath, same
clock as Command Hub. `off` / SSH / `NO_COLOR` keep the static box.
