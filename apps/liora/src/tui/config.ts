...
export const AppearancePreferencesSchema = z.object({
  ...
  /**
   * Worker Dock visibility: `auto` appears while background workers exist,
   * `pinned` keeps the panel mounted even when idle, `hidden` disables it.
   */
  workerDock: z.enum(['auto', 'pinned', 'hidden']),
  /**
...
const AppearanceConfigFileFieldsSchema = z.object({
  ...
  worker_dock: z.enum(['auto', 'pinned', 'hidden']).optional(),
  /** @deprecated Read fallback for Worker Dock; writers emit `worker_dock`. */
...
export function normalizeTuiConfig(config: TuiConfigFileShape): TuiConfig {
  ...
  return TuiConfigSchema.parse({
    ...
    appearance: {
      ...
      workerDock:
        config.appearance?.worker_dock ??
        config.appearance?.mission_control ??
        DEFAULT_APPEARANCE_PREFERENCES.workerDock,
...
worker_dock = "${appearance.workerDock}" # "auto" | "pinned" | "hidden"
