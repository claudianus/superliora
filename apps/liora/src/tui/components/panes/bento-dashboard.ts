...
export class BentoDashboardComponent extends Container implements Focusable {
  ...
  handleInput(data: string): void {
    ...
    if (pinned) {
      ...
      // Gen 113: W → cycle the pin to the next quest with problems (triage).
      if (k === 'W') {
        this.pinController.pinNextProblem();
        return;
      }
      // Gen 114: A → cycle the pin to the next quest awaiting approval.
      if (k === 'A') {
    ...
    // Gen 116: F → focus the quest with the most code changes.
    if (k === 'F') {
      this.gridController.focusMostChanges();
      return;
    }
  ...
  private renderHelp(pinnedMode: boolean, width: number): string[] {
    ...
    const rows: ReadonlyArray<readonly [string, string]> = pinnedMode
      ...
      : [
          ...
          ['F', 'Focus the quest with the most code changes'],
  ...
  private renderDashboard(quests: readonly Quest[], width: number): string[] {
    ...
    // Gen 111: separate failed count so operators can see how many sessions
    // need investigation, distinct from approvals (which need a decision).
    const failedCount = quests.filter((q) => q.state === 'failed').length;
    const totalCost = quests.reduce((sum, q) => sum + (q.sessionCostUsd ?? 0), 0);
    ...
    // Gen 111: show failed count separately so sessions needing investigation
    // are obvious, distinct from approvals (which need a decision).
    if (failedCount > 0) {
