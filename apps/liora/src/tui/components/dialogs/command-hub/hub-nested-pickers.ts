...
const FLEET_MSG_PREFILL = 'prefill:/swarm msg ';

export function showHubFleetPicker(host: HubNestedPickerHost): void {
  mountPickerDialog(
    host,
    new ChoicePickerComponent({
      title: 'War Room',
      ...
      searchable: true,
      options: [
        { value: '/swarm talk', label: 'Talk', description: 'Open expert talk picker / transcript' },
        {
          value: FLEET_MSG_PREFILL,
          label: 'Message…',
          description: 'Prefill /swarm msg — type expert + message',
        },
        { value: '/swarm pause', label: 'Pause', description: 'Pause active swarm war room(s)' },
        {
          value: '/swarm restaff',
          label: 'Restaff',
          description: 'Request restaff on active war room(s)',
        },
        { value: '/swarm raw', label: 'Raw feed', description: 'Toggle raw war-room feed' },
        { value: '/fleet', label: 'Fleet status', description: 'Live Fleet status panel' },
      ],
      onSelect: (value) => {
        if (value.startsWith('prefill:')) {
          dismissPickerDialog(host);
          host.closeAllCenterModals?.();
          host.restoreInputText?.(value.slice('prefill:'.length));
          return;
        }
        runSlashAndCloseHub(host, value);
      },
