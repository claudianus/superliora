 ...
 * footer key hint.
 *
 * Mounted by `liora-tui.ts` via container swap rather than `showOverlay`
...
export class TasksBrowserApp extends Container implements Focusable {
  ...
  private renderDetailFrame(width: number, height: number): string[] {
    ...
    const task = this.selectedTask();
    if (task === undefined) {
      const empty = currentTheme.fg('textMuted', 'Select a task · Enter opens detail · s stops when running');
