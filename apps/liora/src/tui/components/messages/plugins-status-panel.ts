import type { PluginInfo, PluginSummary } from '@superliora/sdk';
...
export function buildPluginsInfoLines(input: PluginsInfoPanelInput): readonly string[] {
  ...
  const trustLine = (() => {
    ...
    if (label === 'official') {
      return `${muted('Trust:')}  ${success(OFFICIAL_BADGE)} ${muted('(SuperLiora-built and -maintained)')}`;
    }
    if (label === 'curated') {
      return `${muted('Trust:')}  ${primary(CURATED_BADGE)} ${muted('(SuperLiora-reviewed, upstream-maintained)')}`;
    }
