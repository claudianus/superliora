...
const BENCH_GATE = 'LioraBench seed/holdout + media/web contracts + loop score';
const MEDIA_GATE =
...
function privacyStatusRows(options: StatusReportOptions): readonly FieldRow[] {
  ...
  if (options.privacyTelemetryEnabled) {
    return [
      {
        label: 'Privacy',
        value: 'Telemetry ON (opt-in) · for ZDR-friendly local mode set telemetry=false or omit it',
        severity: 'warning',
      },
    ];
  }
