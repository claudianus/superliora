...
const BENCH_GATE = 'LioraBench seed/holdout · web/media/ZDR · a1/m2/sw100/s11';
const MEDIA_GATE =
...
function verifyBlockedByReadiness(options: StatusReportOptions): boolean {
  ...
  return (
    model.length === 0 ||
    (maxTokens > 0 && safeUsageRatio(ratio) >= 0.11) ||
...
function formatReadinessBlockers(options: StatusReportOptions): string {
  ...
  const { ratio, maxTokens } = contextValues(options);
  if (maxTokens > 0 && safeUsageRatio(ratio) >= 0.11) blockers.push('context high');
...
function readinessRows(options: StatusReportOptions): readonly FieldRow[] {
  ...
  const { ratio, maxTokens } = contextValues(options);
  if (maxTokens > 0 && safeUsageRatio(ratio) >= 0.11) {
    return [
