...
const BENCH_GATE = 'LioraBench seed/holdout + media/web/ZDR/token/TUI + async/micro3/sw750/soft62/dream';
const MEDIA_GATE =
...
function verifyBlockedByReadiness(options: StatusReportOptions): boolean {
  ...
  return (
    model.length === 0 ||
    (maxTokens > 0 && safeUsageRatio(ratio) >= 0.62) ||
...
function formatReadinessBlockers(options: StatusReportOptions): string {
  ...
  const { ratio, maxTokens } = contextValues(options);
  if (maxTokens > 0 && safeUsageRatio(ratio) >= 0.62) blockers.push('context high');
...
function readinessRows(options: StatusReportOptions): readonly FieldRow[] {
  ...
  const { ratio, maxTokens } = contextValues(options);
  if (maxTokens > 0 && safeUsageRatio(ratio) >= 0.62) {
    return [
