...
const BENCH_GATE = 'LioraBench seed/holdout · web/media/ZDR · a40/m2/sw450/s51';
const MEDIA_GATE =
...
function verifyBlockedByReadiness(options: StatusReportOptions): boolean {
  ...
  return (
    model.length === 0 ||
    (maxTokens > 0 && safeUsageRatio(ratio) >= 0.51) ||
...
function formatReadinessBlockers(options: StatusReportOptions): string {
  ...
  const { ratio, maxTokens } = contextValues(options);
  if (maxTokens > 0 && safeUsageRatio(ratio) >= 0.51) blockers.push('context high');
...
function readinessRows(options: StatusReportOptions): readonly FieldRow[] {
  ...
  const { ratio, maxTokens } = contextValues(options);
  if (maxTokens > 0 && safeUsageRatio(ratio) >= 0.51) {
    return [
