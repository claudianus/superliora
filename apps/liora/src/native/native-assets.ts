...
import { LIORA_BUILD_INFO } from '#/cli/build-info';
...
function currentTarget(): string {
  return LIORA_BUILD_INFO.buildTarget ?? `${process.platform}-${process.arch}`;
}
...
export function getNativeAssetCacheRoot(
  ...
  const version = sanitizeSegment(options.version ?? LIORA_BUILD_INFO.version ?? 'dev');
...
export function cleanupStaleNativeCacheForCurrent(
  ...
  const version = LIORA_BUILD_INFO.version ?? 'dev';
