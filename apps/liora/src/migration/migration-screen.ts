 ...
 * conventions in `apps/liora/src/tui/components/dialogs/choice-picker.ts`.
 *
 ...
 */
import { Container, matchesKey, Key, truncateToWidth, type Focusable } from '#/tui/renderer';
...
} from '@superliora/migration-legacy';
...
export interface MigrationScreenOptions {
   ...
   * now/later/never gate — used by the explicit `liora migrate` command, where
...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  constructor(opts: MigrationScreenOptions) {
    ...
    if (opts.skipDecisionStep === true) {
      // Explicit `liora migrate`: the now/later/never gate is meaningless, so
  ...
  private renderResult(width: number): string[] {
    ...
    if (this.migrationFailed) {
      ...
      lines.push('');
      lines.push(chalk.hex(colors.text)(' You can retry later by running "liora migrate".'));
      lines.push('');
