/**
 * `liora server run` daemon orchestration — parent (spawner) side.
 *
 ...
 *
 *   1. Read `~/.superliora/server/lock`. If it names a *live* daemon, reuse it
 *      (wait for it to be healthy) — never spawn a second one.
 *   2. Otherwise pick a free port (preferred port when available, else an
 *      OS-assigned one) and spawn `liora server run --daemon` as a detached
...
import { DEFAULT_LOCK_DIR, getLiveLock, type LockContents } from '@superliora/server';
...
export async function ensureDaemon(options: EnsureDaemonOptions = {}): Promise<EnsureDaemonResult> {
  ...
  throw new Error(
    `SuperLiora server daemon failed to start within ${String(SPAWN_TIMEOUT_MS)}ms.\n\n` +
...
function formatDaemonBootFailure(
  ...
  return `SuperLiora server daemon ${reason} during startup.\n\n${formatLogTail(logPath)}`;
}
