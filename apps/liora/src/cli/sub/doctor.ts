...
export interface DoctorDeps {
  ...
  readonly configRpc?: LioraConfigRpc;
...
function resolveDeps(deps: Partial<DoctorDeps> | DoctorDeps | undefined): ResolvedDoctorDeps {
  ...
  const getConfigRpc = (): LioraConfigRpc => {
    configRpc ??= createLioraConfigRpc();
    return configRpc;
  };
...
function findValidationIssues(error: unknown): readonly LioraConfigValidationIssue[] | undefined {
...
function isValidationIssueArray(value: unknown): value is readonly LioraConfigValidationIssue[] {
  return Array.isArray(value) && value.every(isValidationIssue);
}

function isValidationIssue(value: unknown): value is LioraConfigValidationIssue {
