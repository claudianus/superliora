...
// Fetched straight from fd's own GitHub releases (SHA256-verified below), not
// mirrored through any third-party CDN.
const FD_RELEASES_BASE_URL = 'https://github.com/sharkdp/fd/releases/download';
...
function fdAssetVersion(assetName: string): string {
  const match = /^fd-(v\d+\.\d+\.\d+)-/.exec(assetName);
  const version = match?.[1];
  if (version === undefined) throw new Error(`fd asset name has no version tag: ${assetName}`);
  return version;
}
...
async function downloadFd(): Promise<string | null> {
  ...
  try {
    const downloadUrl = `${FD_RELEASES_BASE_URL}/${fdAssetVersion(assetName)}/${assetName}`;
