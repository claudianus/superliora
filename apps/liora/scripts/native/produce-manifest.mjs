 ...
 *
 * Input dir must contain files matching: liora-<target>.zip.sha256
...
// Tag format is `@superliora/liora@x.y.z`, `vx.y.z`, or `x.y.z` — normalize to x.y.z.
const version = tag.replace(/^@superliora\/liora@/, '').replace(/^v/, '');

const entries = await readdir(inputDir);
const sumFiles = entries.filter((f) => /^liora-[a-z0-9-]+\.zip\.sha256$/.test(f));

if (sumFiles.length === 0) {
  console.error(`No liora-<target>.zip.sha256 files found in ${inputDir}`);
  process.exit(1);
}
...
for (const sumFile of sumFiles.sort()) {
  ...
  // liora-darwin-arm64.zip → darwin-arm64
  const target = filename.replace(/^liora-/, '').replace(/\.zip$/, '');
