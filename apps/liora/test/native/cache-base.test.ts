...
describe('getNativeCacheBase precedence', () => {
  ...
  it('uses SUPERLIORA_CACHE_DIR when set (highest precedence)', () => {
    const got = getNativeCacheBase({
      ...baseOptions,
      env: { SUPERLIORA_CACHE_DIR: '/custom/cache' },
    });
    expect(got).toBe('/custom/cache');
  });

  it('ignores SUPERLIORA_HOME (no longer affects native cache)', () => {
    const got = getNativeCacheBase({
      ...baseOptions,
      env: { SUPERLIORA_HOME: '/legacy' },
      platform: 'darwin',
    });
    expect(got).toBe('/home/u/Library/Caches/liora');
  });
  ...
  it('uses LOCALAPPDATA on Windows when set', () => {
    ...
    expect(got).toBe('C:\\Users\\u\\AppData\\Local\\liora');
  });
});
