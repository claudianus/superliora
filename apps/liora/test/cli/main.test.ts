...
vi.mock('@superliora/sdk', async () => {
  const actual = await vi.importActual<typeof import('@superliora/sdk')>(
    '@superliora/sdk',
  );
  class MockKimiHarness {
    ...
    constructor(...args: unknown[]) {
      mocks.LioraHarness(...args);
    }
  }
  return {
    ...actual,
    createLioraHarness: (...args: unknown[]) => {
      mocks.createLioraHarness(...args);
      return mocks.harness;
    },
    LioraHarness: MockKimiHarness,
    log: mocks.log,
  };
});
...
describe('main entry command handling', () => {
  ...
  it('sets the process title during startup', () => {
    ...
    try {
      ...
      expect(process.title).toBe('liora');
    } finally {
  ...
  it('initializes and flushes telemetry around the upgrade command', async () => {
    ...
    expect(mocks.createLioraHarness).toHaveBeenCalledWith(expect.objectContaining({
      homeDir: '/tmp/kimi-home',
      telemetry: {
        track: mocks.track,
