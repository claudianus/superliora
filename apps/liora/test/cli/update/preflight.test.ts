...
const mocks = vi.hoisted(() => ({
  ...
  detectSuperLioraGithubCheckout: vi.fn(),
...
vi.mock('../../../src/cli/update/git-checkout', async () => {
  ...
  return {
    ...actual,
    detectSuperLioraGithubCheckout: mocks.detectSuperLioraGithubCheckout,
...
describe('runUpdatePreflight', () => {
  ...
  it('refreshes a stale cached target before showing the foreground install prompt', async () => {
    ...
    expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        installCommand: 'npm install -g @superliora/liora@0.7.0',
      }),
    );
    expect(mocks.spawn).toHaveBeenCalledWith(
      expect.stringMatching(/^npm(\.cmd)?$/),
    ...
    try {
      ...
      expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          installCommand: 'npm install -g @superliora/liora@0.6.0',
        }),
      );
    } finally {
      vi.useRealTimers();
  ...
  it('native on darwin: spawns bash -c with pipefail-guarded curl|bash', async () => {
    ...
    try {
      ...
      expect(script).toContain('curl -fsSL https://raw.githubusercontent.com/claudianus/superliora/main/install.sh');
  ...
  describe('rollout gating', () => {
    ...
    it('SUPERLIORA_EXPERIMENTAL_FLAG bypasses the rollout: held devices still update', async () => {
    ...
    it('SUPERLIORA_NO_AUTO_UPDATE still wins over the experimental flag', async () => {
      vi.stubEnv('SUPERLIORA_EXPERIMENTAL_FLAG', '1');
      vi.stubEnv('SUPERLIORA_NO_AUTO_UPDATE', '1');
...
describe('upgrade install stages', () => {
  ...
  it('passes a short stderr summary into the failed stage', async () => {
    ...
    expect(failed?.detail).toMatch(/EACCES|permission denied/);
  });
