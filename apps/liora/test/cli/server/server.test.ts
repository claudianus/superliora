/**
 * Tests for `liora server` Commander wiring.
 *
 * These tests don't actually start the server — they verify the parsed shape
 * of server options and the API-only command surface.
 *
...
describe('liora server', () => {
  ...
  it('`server run` exposes local-only foreground options', () => {
    ...
    expect(longs).not.toContain('--no-open');
  });
  ...
  it('does not register the retired top-level `kimi web` alias', () => {
    const program = makeProgram();
...
describe('`liora server` lifecycle exits with ESERVICE_UNSUPPORTED on unsupported platforms', () => {
  it('the dispatcher returns a friendly error manager for unknown platforms', async () => {
    ...
    const { resolveServiceManager, ServiceUnsupportedError } = await import('@superliora/server');
...
describe('`liora server` lifecycle handles unavailable service managers', () => {
  it('prints a friendly JSON error and exits 2', async () => {
    const { ServiceUnavailableError } = await import('@superliora/server');
    const program = new Command('kimi').exitOverride();
...
describe('`liora server` lifecycle output', () => {
  it('install passes --force/--port and prints the server URL when running', async () => {
    const program = new Command('kimi').exitOverride();
    const server = program.command('server');
    let stdout = '';
    let stderr = '';
    ...
    addLifecycleCommands(server, {
      resolveManager: () => ({
        install: async (args) => {
          ...
          return {
            status: 'replaced',
            message: 'SuperLiora server LaunchAgent replaced at /tmp/kimi.plist (port 9999).',
        ...
        status: async () => ({
          ...
          label: 'com.superliora.liora-server',
        }),
      }),
      stdout: {
        write(chunk: string | Uint8Array) {
  ...
  it('start prints URL and diagnostics when launchd did not keep the service running', async () => {
    ...
    addLifecycleCommands(server, {
      resolveManager: () => ({
        ...
        uninstall: async () => ({ ok: true, message: 'unused' }),
        start: async () => ({ ok: true, message: 'SuperLiora server started (com.superliora.liora-server).' }),
        stop: async () => ({ ok: true, message: 'unused' }),
        restart: async () => ({ ok: true, message: 'unused' }),
        status: async () => ({
          platform: 'darwin',
...
describe('`liora server run` background start', () => {
  ...
  it('warns and uses the running server host when a daemon is reused', async () => {
    ...
    expect(plain).toContain('liora server kill');
  ...
  it('prints a TUI-style ready panel once the daemon is up', async () => {
    ...
    const plain = stripAnsi(stdout);
    expect(plain).toContain('SuperLiora server ready');
    ...
    expect(plain).toContain('▐█████▌');
    expect(plain).not.toContain('➜');
    expect(plain).not.toContain('SuperLiora server:');

    // Title is above the URLs; Logs/Stop are at the bottom.
    expect(plain.indexOf('SuperLiora server ready')).toBeLessThan(plain.indexOf('Local:'));
  ...
  it('uses the TUI dark palette for the ready banner', async () => {
    ...
    expect(stdout).toContain(color.bold.hex(darkColors.primary)('SuperLiora server ready'));
...
describe('`liora server run --foreground`', () => {
  ...
  it('prints the API ready banner once listening', async () => {
    const { handleRunCommand } = await import('#/cli/sub/server/run');
    let stdout = '';
...
describe('`liora server` does not register a legacy `daemon` command', () => {
...
describe('ready banner reflects the bind class (M6.3)', () => {
  it('lists Local + Network addresses for a 0.0.0.0 bind (Vite-style)', async () => {
    ...
    expect(raw).toContain('SuperLiora server ready');
  ...
  it('prints the Local URL and token for a 127.0.0.1 bind', async () => {
    ...
    expect(raw).toContain('SuperLiora server ready');
    expect(raw).toContain('Local:');
    expect(raw).toContain('http://127.0.0.1:58627/');
    expect(raw).toContain('Token:');
...
describe('resolveDaemonProgram', () => {
  ...
  it('returns execPath in SEA mode when argv[1] is a bare command name', async () => {
    ...
    const { resolveDaemonProgram } = await import('#/cli/sub/server/daemon');
    expect(resolveDaemonProgram(['/Users/x/.superliora/bin/kimi', 'kimi', 'server', 'run'], '/Users/x', '/Users/x/.superliora/bin/kimi', true)).toBe('/Users/x/.superliora/bin/kimi');
  });

  it('returns execPath in SEA mode for a spawned `server` child', async () => {
    const { resolveDaemonProgram } = await import('#/cli/sub/server/daemon');
    expect(resolveDaemonProgram(['/Users/x/.superliora/bin/kimi', 'server', 'run'], '/Users/x', '/Users/x/.superliora/bin/kimi', true)).toBe('/Users/x/.superliora/bin/kimi');
  });
});
...
describe('`liora server run` validation', () => {
...
describe('`liora server kill`', () => {
  const liveLock = { pid: 1234, started_at: '2026-06-17T00:00:00.000Z', port: 58627 };

  it('prints "No running SuperLiora server." and sends no signal when no live lock exists', async () => {
    ...
    expect(writes.join('')).toContain('No running SuperLiora server.');
...
describe('resolveServerToken', () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'liora-server-token-'));
  });
  afterEach(() => {
...
describe('`liora server kill` carries the bearer token', () => {
  const liveLock = { pid: 1234, started_at: '2026-06-17T00:00:00.000Z', port: 58627 };
...
describe('accessUrlLines', () => {
  it('returns Local + Network lines for a wildcard bind', async () => {
    ...
    const lines = accessUrlLines('0.0.0.0', 58627, [
      { address: '192.168.1.5', family: 'IPv4' },
    ]);
    expect(lines).toEqual([
      { label: 'Local:    ', url: 'http://localhost:58627/' },
      { label: 'Network:  ', url: 'http://192.168.1.5:58627/' },
    ]);
  });

  it('returns a single Local line for a loopback bind', async () => {
    const { accessUrlLines } = await import('#/cli/sub/server/access-urls');
    const lines = accessUrlLines('127.0.0.1', 58627);
    expect(lines).toEqual([
      { label: 'Local:    ', url: 'http://127.0.0.1:58627/' },
    ]);
  });
...
describe('`liora server rotate-token`', () => {
  let dir: string;
  let prevHome: string | undefined;
  ...
  it('re-prints the access links with the new token when a server is running', async () => {
    ...
    const token = readFileSync(join(dir, 'server.token'), 'utf8').trim();
    expect(stdout).toContain('New server token');
    expect(stdout).toContain('http://127.0.0.1:58627/');
    expect(stdout).toContain(token);
    ...
    expect(stdout.indexOf('New server token')).toBeLessThan(
      stdout.indexOf('http://127.0.0.1:58627/'),
    );
  });
});
