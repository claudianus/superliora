...
import type { createKimiDeviceId as createKimiDeviceIdFn } from '@superliora/oauth';
...
const mocks = vi.hoisted(() => {
  ...
  class TuiConfigParseError extends Error {
    ...
    constructor(fallback: TuiConfigFallback) {
      super('Invalid TUI config in ~/.superliora/tui.toml; using defaults.');
  ...
  return {
    ...
    resolveLioraHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/kimi-code-test-home'),
...
vi.mock('@superliora/sdk', async (importOriginal) => {
  ...
  return {
    ...
    createLioraHarness: (...args: unknown[]) => {
...
vi.mock('@superliora/oauth', async () => {
  const actual = await vi.importActual<typeof import('@superliora/oauth')>(
    '@superliora/oauth',
  );
  return {
    ...actual,
    createKimiDeviceId: mocks.createKimiDeviceId,
    SUPERLIORA_PROVIDER_NAME: 'kimi-code',
  };
});
...
describe('runShell', () => {
  afterEach(() => {
    ...
    mocks.resolveLioraHome.mockImplementation(
      (homeDir?: string) => homeDir ?? '/tmp/kimi-code-test-home',
    );
    mocks.harnessCreatesDeviceIdOnConstruction = false;
  });

  it('constructs LioraHarness and LioraTUI with startup input', async () => {
  ...
  it('detects auto theme and forwards config parse warnings as startup notice', async () => {
    ...
    expect(startupInput).toMatchObject({
      startupNotice: 'Invalid TUI config in ~/.superliora/tui.toml; using defaults.',
      tuiConfig: {
        theme: 'auto',
        editorCommand: 'vim',
        notifications: { enabled: true, condition: 'always' },
  ...
  it('tracks exit and prints resume instructions from the TUI exit handler', async () => {
    ...
    try {
      ...
      expect(stderr.text()).toContain(' To resume this session: liora -r ses-1');
    } finally {
      exitSpy.mockRestore();
      stdout.restore();
      stderr.restore();
  ...
  it('surfaces an invalid target config as an error for liora migrate, not silently', async () => {
    mocks.loadTuiConfig.mockResolvedValue({
      theme: 'dark',
      editorCommand: null,
      notifications: { enabled: true, condition: 'unfocused' },
    ...
    mocks.harnessGetConfig.mockRejectedValue(
      new Error('Invalid configuration in ~/.superliora/config.toml'),
    );

    // A broken config.toml must fail loudly — `liora migrate` must not swallow
