...
const executableName = process.platform === 'win32' ? 'liora.exe' : 'liora';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `liora-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `liora-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    await execFileAsync(process.execPath, [packageScript], {
      cwd: appRoot,
      env: { ...process.env, SUPERLIORA_BUILD_TARGET: target },
    });

    const archivePath = resolve(artifactsDir, `liora-${target}.zip`);
  ...
  it('produces a manifest from zip archive checksums', async () => {
    ...
    await writeFile(join(releaseDir, 'liora-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'liora-darwin-arm64.zip.sha256'),
      `${checksum}  liora-darwin-arm64.zip\n`,
    );

    await execFileAsync(process.execPath, [manifestScript, releaseDir, '@superliora/liora@0.5.0']);
    ...
    expect(manifest).toEqual({
      version: '0.5.0',
      tag: '@superliora/liora@0.5.0',
      platforms: {
        'darwin-arm64': {
          filename: 'liora-darwin-arm64.zip',
          checksum,
        },
      },
    });
