...
describe('commandForExecFile', () => {
  ...
  it('returns command as-is on Windows for non-batch files', () => {
    const result = commandForExecFile('postject.exe', ['liora.exe'], 'win32');
    expect(result).toEqual({ command: 'postject.exe', args: ['liora.exe'] });
  });

  it('wraps .cmd files through cmd.exe on Windows', () => {
    const result = commandForExecFile('postject.cmd', ['liora.exe', 'NODE_SEA_BLOB'], 'win32', {
    ...
    expect(result.args).toEqual([
      ...
      '/s',
      '/c',
      '""postject.cmd" "liora.exe" "NODE_SEA_BLOB""',
    ]);
