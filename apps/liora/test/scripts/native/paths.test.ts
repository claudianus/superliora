...
describe('targetTriple', () => {
  ...
  it('honors SUPERLIORA_BUILD_TARGET override', () => {
    expect(
      targetTriple({
        platform: 'darwin',
        arch: 'arm64',
        env: { SUPERLIORA_BUILD_TARGET: 'linux-arm64' },
      }),
    ).toBe('linux-arm64');
  });
});
...
describe('executableName', () => {
  it('returns liora.exe on win32', () => {
    expect(executableName('win32')).toBe('liora.exe');
  });

  it('returns liora on other platforms', () => {
    expect(executableName('darwin')).toBe('liora');
    expect(executableName('linux')).toBe('liora');
  });
});
...
describe('path helpers', () => {
  ...
  it('returns absolute bin path with executable name', () => {
    expect(nativeBinPath('darwin-arm64', 'darwin')).toBe(
      p('dist-native/bin/darwin-arm64/liora'),
    );
    expect(nativeBinPath('win32-x64', 'win32')).toBe(
      p('dist-native/bin/win32-x64/liora.exe'),
    );
  });
