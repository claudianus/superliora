...
function makeRefreshHost(initial: LioraConfig): {
  current: () => LioraConfig;
  ...
  return {
    current: () => structuredClone(persisted) as unknown as LioraConfig,
    removeProvider,
    setConfig,
  };
}
...
describe('refreshAllProviderModels', () => {
  ...
  it('refreshes managed SuperLiora against environment endpoints over persisted config', async () => {
    ...
    expect(result.unchanged).toEqual([SUPERLIORA_PROVIDER_NAME]);
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(resolveOAuthToken).toHaveBeenCalledWith(SUPERLIORA_PROVIDER_NAME, envOauthRef);
  });
