...
describe('plugins selector dialogs', () => {
  ...
  it('treats only the official Kimi CDN path as a trusted install source', () => {
    expect(isOfficialPluginSource('https://raw.githubusercontent.com/claudianus/superliora/main/plugins/official/kimi-datasource.zip')).toBe(true);
    // Curated and other Kimi CDN paths are not "official" for the install gate.
    expect(isOfficialPluginSource('https://raw.githubusercontent.com/claudianus/superliora/main/plugins/curated/superpowers.zip')).toBe(false);
    expect(isOfficialPluginSource('https://raw.githubusercontent.com/claudianus/superliora/main/plugins/foo.zip')).toBe(false);
    ...
    expect(isOfficialPluginSource('http://raw.githubusercontent.com/claudianus/superliora/main/plugins/official/x.zip')).toBe(false);
  ...
  it('defaults the third-party install trust prompt to exit', () => {
    ...
    expect(out.some((line) => line.includes('SuperLiora has not reviewed'))).toBe(true);
