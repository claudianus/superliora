...
describe('FooterComponent — goal badge', () => {
  ...
  it('surfaces the SuperLiora SOTA pulse for an Ultrawork goal', () => {
    const footer = new FooterComponent(
      baseState({
        goal: goal({
          objective: 'SuperLiora Ultrawork SOTA footer harness pass',
        }),
      }),
    );
    ...
    expect(out).toContain('[goal');
    expect(out).toContain('SuperLiora SOTA / active');
  });
