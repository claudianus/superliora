...
describe('status panel report lines', () => {
  it('formats runtime status, context, and managed usage without account or AGENTS.md rows', () => {
    ...
    expect(output).toMatch(/Bench\s+LioraBench · web\/media\/office\/ZDR · async70 soft80 hard92 · inject3/);
    ...
    expect(output).toMatch(/Memory\s+prefs \| recall inject≤6×480 \| long-run notes \| auto-dream/);
