...
describe('terminal notification helpers', () => {
  it('emits OSC 9 only when the terminal supports it', () => {
    ...
    emitTerminalNotification(
      terminal,
      { title: 'SuperLiora', body: 'Approval\nrequired' },
      { supportsOsc9: true, insideTmux: false },
    );

    expect(terminal.write).toHaveBeenCalledTimes(1);
    expect(terminal.write).toHaveBeenCalledWith(']9;SuperLiora: Approval required');
  });
  ...
  it('falls back to a bare BEL when the terminal does not support OSC 9', () => {
    ...
    emitTerminalNotification(
      terminal,
      { title: 'SuperLiora', body: 'Approval required' },
  ...
  it('wraps OSC 9 in a tmux DCS passthrough when running inside tmux', () => {
    ...
    expect(terminal.write).toHaveBeenCalledTimes(1);
    expect(terminal.write).toHaveBeenCalledWith('Ptmux;]9;SuperLiora: Approval required\\');
  });
  ...
  it('builds a Linux notify-send command', () => {
    expect(
      ...
      command: 'notify-send',
      args: ['-a', 'SuperLiora', 'Question', 'Pick one'],
    });
  });
  ...
  it('adds a terminal bell alongside Linux native notifications', () => {
    ...
    expect(spawn).toHaveBeenCalledWith('notify-send', ['-a', 'SuperLiora', 'Question', 'Pick one'], {
      detached: true,
      env: {},
      stdio: 'ignore',
    });
