...
describe('custom theme loader', () => {
  ...
  it('exposes bundled SuperLiora themes without a user themes directory', async () => {
  ...
  it('lists user themes alongside bundled themes and lets user themes win name collisions', async () => {
    ...
    writeTheme('superliora-neon-noir', {
      name: 'superliora-neon-noir',
    ...
    expect(entries).not.toContainEqual(
      expect.objectContaining({ name: 'superliora-neon-noir', source: 'bundled' }),
    );

    const merged = await loadCustomThemeMerged('superliora-neon-noir');
