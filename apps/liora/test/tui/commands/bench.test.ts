...
describe('bench slash command status surface', () => {
  it('builds a concise redacted status transcript from gate evidence', () => {
    const lines = buildBenchStatusLines({
      sourcePath: '/repo/.superliora/evidence/gate-summary.json',
      status: 'APPROVE',
      score: 1,
      passRate: 1,
      budget: 'FAIL',
      ...
      budgetInspect: '/repo/.superliora/evidence/run/tasks/seed.budget-overrun.v1/result.json',
      budgetRerun: 'node scripts/liora-agent-bench.mjs --suite seed --runner fixture',
      holdout: 'active',
    ...
    expect(lines.join('\n')).toContain('Budget inspect  /repo/.superliora/evidence/run/tasks/seed.budget-overrun.v1/result.json');
    expect(lines.join('\n')).toContain('Budget rerun  node scripts/liora-agent-bench.mjs --suite seed --runner fixture');
  ...
  it('loads the newest local evidence from the default bench root', () => {
    ...
    const evidenceRoot = join(
      workDir,
      '.superliora/evidence/superliora-provider-bench/final-quality-gate',
    );
    ...
    expect(status.sourceDisplayPath).toBe(
      '.superliora/evidence/superliora-provider-bench/final-quality-gate/newer-loop/loop-summary.json',
    );
  });
  ...
  it('surfaces bounded loop trend and rerun context from loop summaries', () => {
    ...
    const evidenceRoot = join(workDir, '.superliora/evidence/loop-trend');
    mkdirSync(evidenceRoot, { recursive: true });
    writeFileSync(
      ...
      JSON.stringify({
        ...
        rerun: {
          command:
            'node scripts/liora-agent-bench.mjs --loop --task-dir .superliora/bench/tasks --suite seed --runner fixture --max-total-ms 600000 --max-iterations 2 --evidence-root .superliora/evidence/loop-trend',
          argv: [
            'node',
            'scripts/liora-agent-bench.mjs',
            '--loop',
            '--task-dir',
            '.superliora/bench/tasks',
            '--suite',
            'seed',
            '--runner',
            'fixture',
            ...
            '2',
            '--evidence-root',
            '.superliora/evidence/loop-trend',
          ],
        },
        iterations: [
          {
              ...
              tasks: [
                {
                  ...
                  action: 'cat .superliora/evidence/loop-trend/iterations/02/tasks/seed.fix-target.v1/result.json',
                  displayPath: '.superliora/evidence/loop-trend/iterations/02/tasks/seed.fix-target.v1/result.json',
    ...
    const status = loadBenchStatus(workDir, '.superliora/evidence/loop-trend');
    const text = buildBenchStatusLines(status).join('\n');
    ...
    expect(text).toContain(
      'Loop action  cat .superliora/evidence/loop-trend/iterations/02/tasks/seed.fix-target.v1/result.json',
    );
    expect(text).toContain(
      'Loop inspect  .superliora/evidence/loop-trend/iterations/02/tasks/seed.fix-target.v1/result.json',
    );
    ...
    expect(text).toContain(
      'Loop rerun  node scripts/liora-agent-bench.mjs --loop --task-dir .superliora/bench/tasks --suite seed --runner fixture --max-total-ms 600000 --max-iterations 2 --evidence-root .superliora/evidence/loop-trend',
    );
    expect(text).toContain(
      'Loop replay  node scripts/liora-agent-bench.mjs --replay-summary .superliora/evidence/loop-trend/loop-summary.json --evidence-root .superliora/evidence/loop-trend/replay',
    );
    expect(text).toContain('Next  Tighten the next budget-sensitive task.');
    expect(text).toContain('Source  .superliora/evidence/loop-trend/loop-summary.json');
  ...
  it('shows replay only for local loop summaries with structured rerun argv', () => {
    ...
    const legacyRoot = join(workDir, '.superliora/evidence/legacy-loop');
    ...
    const loopSummary = {
      ...
      bestScore: 1,
      rerun: {
        command: 'node scripts/liora-agent-bench.mjs --loop --evidence-root .superliora/evidence/loop',
      },
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        rerun: {
          ...loopSummary.rerun,
          argv: ['node', 'scripts/liora-agent-bench.mjs', '--loop', '--evidence-root', 'old'],
        },
      }),
    );

    const legacyText = buildBenchStatusLines(loadBenchStatus(workDir, '.superliora/evidence/legacy-loop')).join('\n');
    const externalText = buildBenchStatusLines(loadBenchStatus(workDir, externalRoot)).join('\n');

    expect(legacyText).toContain('Loop rerun  node scripts/liora-agent-bench.mjs --loop --evidence-root .superliora/evidence/loop');
  ...
  it('quotes local replay paths that need shell escaping', () => {
    ...
    const specialRoot = join(workDir, '.superliora/evidence', "loop replay's path");
    ...
    const text = buildBenchStatusLines(loadBenchStatus(workDir, ".superliora/evidence/loop replay's path")).join('\n');

    expect(text).toContain(
      "Loop replay  node scripts/liora-agent-bench.mjs --replay-summary '.superliora/evidence/loop replay'\\''s path/loop-summary.json' --evidence-root '.superliora/evidence/loop replay'\\''s path/replay'",
    );
  });
  ...
  it('surfaces replay wrapper evidence before child loop summaries', () => {
    ...
    const replayRoot = join(workDir, '.superliora/evidence/replay-root');
    const sourceRoot = join(workDir, '.superliora/evidence/source-loop');
    ...
    writeFileSync(
      ...
      JSON.stringify({
        benchmark: 'liora-agent-bench-loop-replay',
        status: 'PASS',
        childExitCode: 0,
        childTimedOut: false,
    ...
    const text = buildBenchStatusLines(loadBenchStatus(workDir, '.superliora/evidence/replay-root')).join('\n');
    ...
    expect(text).toContain('Replay source  .superliora/evidence/source-loop/loop-summary.json');
    expect(text).toContain('Replay evidence  .superliora/evidence/replay-root');
    expect(text).toContain('Replay inspect  cat .superliora/evidence/replay-root/loop-summary.json');
    expect(text).toContain('Replay log  cat .superliora/evidence/replay-root/commands.jsonl');
    expect(text).toContain(
      'Replay diff  diff -u .superliora/evidence/source-loop/loop-summary.json .superliora/evidence/replay-root/loop-summary.json',
    );
    expect(text).toContain('Next  Replay verified; inspect child loop result PASS before the next improvement.');
    expect(text).toContain('Source  .superliora/evidence/replay-root/replay-summary.json');
    expect(text).not.toContain('Source  .superliora/evidence/replay-root/loop-summary.json');
  });
  ...
  it('classifies unsafe replay wrappers before trusting them', () => {
    ...
    const root = join(workDir, '.superliora/evidence/replay-verdict');
    ...
    function writeReplay(name: string, value: Record<string, unknown>): string {
      ...
      return `.superliora/evidence/replay-verdict/${name}`;
    }
  ...
  it('quotes replay inspect actions that need shell escaping', () => {
    ...
    const replayRoot = join(workDir, '.superliora/evidence', "replay root's path");
    const sourceRoot = join(workDir, '.superliora/evidence/source-loop');
    mkdirSync(replayRoot, { recursive: true });
    mkdirSync(sourceRoot, { recursive: true });
    const childLoopSummary = {
    ...
    const text = buildBenchStatusLines(loadBenchStatus(workDir, ".superliora/evidence/replay root's path")).join('\n');

    expect(text).toContain(
      "Replay inspect  cat '.superliora/evidence/replay root'\\''s path/loop-summary.json'",
    );
    expect(text).toContain(
      "Replay log  cat '.superliora/evidence/replay root'\\''s path/commands.jsonl'",
    );
    expect(text).toContain(
      "Replay diff  diff -u .superliora/evidence/source-loop/loop-summary.json '.superliora/evidence/replay root'\\''s path/loop-summary.json'",
    );
  });
  ...
  it('hides replay diff actions when either compare side is missing', () => {
    ...
    const missingSourceRoot = join(workDir, '.superliora/evidence/missing-source');
    const missingEvidenceRoot = join(workDir, '.superliora/evidence/missing-evidence');
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        onlyEvidenceRootChanged: true,
        shellParsing: false,
        sourceSummaryPath: join(workDir, '.superliora/evidence/source-loop/loop-summary.json'),
        replayedLoopSummary: childLoopSummary,
      }),
    );

    const missingSourceText = buildBenchStatusLines(loadBenchStatus(workDir, '.superliora/evidence/missing-source')).join('\n');
    const missingEvidenceText = buildBenchStatusLines(loadBenchStatus(workDir, '.superliora/evidence/missing-evidence')).join('\n');
  ...
  it('keeps clean and malformed loop focus evidence honest', () => {
    ...
    const cleanRoot = join(workDir, '.superliora/evidence/clean-loop');
    const malformedRoot = join(workDir, '.superliora/evidence/malformed-loop');
    const secretRoot = join(workDir, '.superliora/evidence/secret-loop');
    ...
    const cleanText = buildBenchStatusLines(loadBenchStatus(workDir, '.superliora/evidence/clean-loop')).join('\n');
    const malformedText = buildBenchStatusLines(loadBenchStatus(workDir, '.superliora/evidence/malformed-loop')).join('\n');
    const secretText = buildBenchStatusLines(loadBenchStatus(workDir, '.superliora/evidence/secret-loop')).join('\n');
  ...
  it('falls back to resultPath for older loop focus summaries', () => {
    ...
    const evidenceRoot = join(workDir, '.superliora/evidence/old-loop');
    ...
    const text = buildBenchStatusLines(loadBenchStatus(workDir, '.superliora/evidence/old-loop')).join('\n');
  ...
  it('loads actionable budget violation details from bench summary evidence', () => {
    ...
    const evidenceRoot = join(workDir, '.superliora/evidence/budget-fail');
    mkdirSync(evidenceRoot, { recursive: true });
    writeFileSync(
      join(evidenceRoot, 'summary.json'),
      JSON.stringify({
        ...
        completedAt: '2026-06-29T00:00:00.000Z',
        evidenceRoot,
        taskDir: '/repo/.superliora/bench/tasks',
        suite: 'seed',
        runner: 'fixture',
        metrics: { score: 0, passRate: 0 },
        budget: {
    ...
    const status = loadBenchStatus(workDir, '.superliora/evidence/budget-fail');
    const text = buildBenchStatusLines(status).join('\n');
    ...
    expect(text).toContain(
      `Budget rerun  node scripts/liora-agent-bench.mjs --task-dir /repo/.superliora/bench/tasks --suite seed --runner fixture --evidence-root ${evidenceRoot}`,
    );
