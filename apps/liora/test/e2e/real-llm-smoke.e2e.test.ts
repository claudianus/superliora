 ...
 * `LioraHarness` entry and round-trips one real prompt. Requires provider
 * credentials on disk.
 *
 * Env knobs:
...
import { createLioraHarness, type Event } from '@superliora/sdk';
...
describe.skipIf(!ENABLED)('SDK e2e — real LLM smoke', () => {
  test(
    'round-trips a single prompt through LioraHarness',
    async () => {
