...
beforeEach(async () => {
  ...
  oldHome = process.env['SUPERLIORA_HOME'];
...
afterEach(async () => {
  ...
  if (oldHome === undefined) {
    ...
    process.env['SUPERLIORA_HOME'] = oldHome;
  }
...
describe.skipIf(!ENABLED)('local logging export e2e', () => {
  it('exports session log and global log by default, and allows skipping global log', async () => {
    const harness = createLioraHarness({
      homeDir,
      identity: createLioraHostIdentity('0.1.1'),
    });
    try {
...
async function runKimiExport(args: string[]): Promise<void> {
  ...
  registerExportCommand(program, {
    ...
    exit: (code: number): never => {
      throw new Error(`liora export exited ${code}: ${stderr.join('')}`);
    },
  });
