...
import { resolveKimiCodeOAuthKey } from '@superliora/oauth';
...
describe('kimi-datasource MCP server', () => {
  ...
  it('uses env-scoped credentials and derives the datasource URL from SUPERLIORA_BASE_URL', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'kimi-datasource-plugin-'));
    const kimiHome = join(tempDir, 'kimi-home');
    const requests: unknown[] = [];
    let child: ChildProcessWithoutNullStreams | undefined;
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          SUPERLIORA_HOME: kimiHome,
          SUPERLIORA_BASE_URL: baseUrl,
          SUPERLIORA_OAUTH_HOST: oauthHost,
  ...
  it('registers yuandian_law in the get_data_source_desc enum', async () => {
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        cwd: REPO_ROOT,
        env: { ...process.env, SUPERLIORA_HOME: kimiHome },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
...
// truth in @superliora/oauth. The credential file name is the OAuth
